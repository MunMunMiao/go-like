...
الاستدعاء الداخلي الأحادي في LikeGo تركيب صغير وواضح. يمرر `@likego/client` لقطة `Discovery` إلى `Selector`، ثم ينفذ تبادلاً واحداً من `send` و`recv` عبر `Transport`. وتستخدم عملية الإنشاء خيارات وظيفية:
