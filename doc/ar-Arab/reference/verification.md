...
ينشئ ينشئ `test:unit:coverage` تقريراً اختيارياً فقط. الأوامر `fmt` و`typecheck` و`build` و`audit` و`doc:build` أوامر هندسية وليست أنواعاً إضافية من الاختبارات. ويفحص `doc:build` مسارات VitePress الإنجليزية والمحلية المكوّنة، لكنه لا يثبت تخطيط المتصفح أو تكافؤ الترجمة. وجود الأمر لا يثبت نجاحه؛ يجب مراجعة حالة الخروج والسجلات للتشغيل الحالي. توجد lanes الأدلة الكاملة وbaseline التاريخي وrun record الوثائق في [صفحة Verification الإنجليزية canonical](/reference/verification).
