...
يمكن للاتصالات داخل العملية والاختبارات استخدام `@likego/transport-memory`. يستخدم HTTP الداخلي `@likego/transport-http`؛ ويوفّر المسار `@likego/transport-http/node` تنفيذي Node للدالتين `dial` و`listen`، بما في ذلك TLS/mTLS من نوع PEM في جهة الخادم وHTTP/2 عبر ALPN. وتربط `@likego/hono` و`@likego/elysia` و`@likego/h3` أطر Web. وتشمل محوّلات دورة حياة بيئات التشغيل والمكتبات `@likego/croner` و`@likego/bullmq` و`@likego/nats` و`@likego/pino` و`@likego/winston`. أما محوّلات قابلية الرصد فهي `@likego/prometheus` و`@likego/otel`.
...
مزود Kubernetes للإعداد هو `@likego/config-kubernetes`، ومزودا Broker هما `@likego/broker-memory` و`@likego/broker-rabbitmq`. حزمة CLI الخاصة بإنشاء المشاريع هي `@likego/create`.
