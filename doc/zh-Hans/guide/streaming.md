...
对外 HTTP 请把框架的原生 Fetch Handler 交给 `@likego/web`。Hono、Elysia、H3 的 SSE、流响应或 runtime 专属 WebSocket 升级仍由原框架处理，LikeGo 只保证 `Request`、`Response`、stream 和错误 identity 不被破坏。
