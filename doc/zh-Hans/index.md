...
可移植代码只依赖标准 Web API，例如 `Request`、`Response`、`Headers`、`AbortSignal`、Web Streams 和注入的 `fetch`。Node.js、Bun、Deno 各自特有的能力会放到单独入口。已经导出 Fetch 的框架不需要 LikeGo 适配包：Hono、Elysia 与 H3 2.x 的 `app.fetch` 可直接交给 `@likego/web`；H3 1.x 使用 `toWebHandler(app)`。Croner、BullMQ、NATS、Pino、Winston 等资源才使用生命周期适配器。
