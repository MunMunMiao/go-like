...
可攜部分只會用標準 Web API，包括 `Request`、`Response`、`Headers`、`AbortSignal`、Web Streams 同由應用傳入嘅 `fetch`。Node.js、Bun、Deno 專屬能力會另開入口。原本用緊 Hono、Elysia、H3、Croner、BullMQ、NATS、Pino 或 Winston 都可以照用，LikeGo 只負責將資源接入同一套生命週期。
