...
呢頁係按應用想解答嘅問題整理，唔係照 `packages/` 目錄排。由真正擁有契約嘅最細 public package import；runtime-specific 行為會由明確 subpath 暴露。Provider 套件唔係有一套通用保證、可以任意互換嘅實作，揀之前要睇清楚 backend、runtime 同 ownership。
...
而家 source inventory 有 43 個非 private 嘅 `@go-like/*` package manifest 同 23 個 public source subpath；呢個 checkout 入面全部係 `0.0.1`。`examples/*` 底下 44 個 workspace 係 private application，唔係 public package。
