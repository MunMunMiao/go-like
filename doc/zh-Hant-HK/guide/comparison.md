...
公平比較要由 ownership 開始，唔係鬥功能數目。呢度講 ownership，即係邊個建立資源、邊個負責停佢、邊個觀察終態。NestJS、Fastify、Hono、Elysia、Koa 同 tRPC 處理 TypeScript application stack 嘅唔同層次；go-micro 同 go-kratos 就係 Go 生態嘅參考，transport 同 code generation 取向亦唔一樣。go-like 只係一組面向 TypeScript 嘅建構元件，重點係清楚 lifecycle、內部 unary call、provider contract 同跨 runtime 組合。
...
本軌道使用的 go-like source baseline 是 commit `9385dbf5b6a7d913be56a80ade359e1bf9be8675`。本地研究記錄中存在一個 go-micro commit 不一致：一筆比較記錄寫的是 `9d306dcfc1a912a8a9493f31fee0bb983475258d`，而詳細的固定版本備忘錄檢查的是 `v6.9.0` 的 `3c39d17fadaa9ec21b671be4afef3e63846406e6`。請當佢哋係要重新核對嘅比較輸入，唔好當成 upstream 而家嘅保證。
...
呢啲 URL 只係比較參考，唔代表今次文件階段重新抓過或者驗證過每一個 upstream 頁面。要改版本相關嘅比較內容之前，請重新核對 release tag 或 commit。
