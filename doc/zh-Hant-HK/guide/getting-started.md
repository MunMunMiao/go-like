...
服務用到咩先裝咩。一個普通 HTTP 服務通常會有 `@likego/context`、`@likego/core`、`@likego/web`，再揀一個可以匯出原生 Fetch Handler 嘅 Web 框架。內部服務呼叫先需要 `@likego/client`、`@likego/transport` 同 `@likego/transport-http`；註冊中心、設定來源同儲存後端全部由應用明確揀，唔會暗中塞入預設組合。
