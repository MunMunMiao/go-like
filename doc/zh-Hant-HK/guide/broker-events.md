...
`Broker.subscribe(ctx, topic, handler)` 會回傳帶有 `unsubscribe(ctx)` 嘅 provider `Subscriber`。`newBrokerServer(...)` 將 `Broker` 接入 Core `Server` 契約：`start(ctx)` 代表完整運行期，`stop(ctx)` 請求停止。go-like 負責停止已接納嘅 subscription，但從來唔會擁有 connection、stream 或 durable consumer；啟動取消會回滾已建立但尚未接納嘅 subscription。
