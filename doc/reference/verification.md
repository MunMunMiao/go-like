...
The root scripts are defined in `package.json`. `bun run fmt` repairs formatting. `bun run lint` applies safe Oxlint fixes, formats the result, and fails if any warning remains. `fmt:check` and `lint:check` do not modify files and are used by the canonical gate; `lint:check` also requires zero warnings. These commands do not typecheck or execute runtime behavior. `bun run test:unit` runs root tests and the workspace unit scripts sequentially. `test:unit:coverage` runs those scripts under Bun coverage, includes the `examples/payments-ledger` integration exception described above, and then checks the executable-source inventory plus per-file line and function coverage. It does not claim branch coverage. No statement threshold is configured because the repository probe showed that accepting the setting did not make an uncovered same-line statement fail. `bun run build` builds package output before E2E scopes.

`bun run test:stability` is a separate unit-test stability check, not part of `verify`. It randomizes each root and workspace run, repeats every test file twice, labels each run, and prints the seed needed to reproduce its order. It uses no retry: a failure remains a failure. Re-run the labeled root or workspace command with the printed `--seed` and `--rerun-each=2` arguments to reproduce it. This check targets order dependencies and intermittent failures; it does not replace `verify` or the 60-minute runtime behavior checked by `test:e2e:soak`.

`bun run test:parallel` is also separate from `verify`. It runs the same root and workspace unit scope once with two isolated Bun workers to expose file-level parallel-safety problems. It does not randomize, repeat, or retry failures.
