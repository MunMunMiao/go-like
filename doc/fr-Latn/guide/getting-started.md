...
Installez uniquement les briques utiles. Un service HTTP classique part de `@likego/context`, `@likego/core`, `@likego/web` et d’un framework qui expose un handler Fetch natif. Les appels internes ajoutent `@likego/client`, `@likego/transport` et `@likego/transport-http` ; registre, configuration et stockage sont des choix explicites.
