...
Для внутрипроцессных вызовов и тестов можно использовать `@likego/transport-memory`. Внутренний HTTP реализует `@likego/transport-http`; подпуть `@likego/transport-http/node` предоставляет реализации Node для `dial` и `listen`, включая серверные PEM TLS/mTLS и HTTP/2 с согласованием ALPN. Web-фреймворки передают свои нативные Fetch-обработчики напрямую в `@likego/web`; LikeGo не публикует отдельные пакеты-мосты для фреймворков. Адаптеры жизненного цикла включают `@likego/croner`, `@likego/bullmq`, `@likego/nats`, `@likego/pino`, `@likego/winston`, а наблюдаемость — `@likego/prometheus` и `@likego/otel`.
