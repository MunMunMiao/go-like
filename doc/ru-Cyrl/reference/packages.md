...
Точные имена Registry-провайдеров: `@likego/registry-mdns`, `@likego/registry-consul`, `@likego/registry-etcd`, `@likego/registry-kubernetes` и `@likego/registry-zookeeper`. Дополнительные Config-провайдеры — `@likego/config-consul`, `@likego/config-etcd` и `@likego/config-kubernetes`; Broker-провайдеры — `@likego/broker-memory` и `@likego/broker-rabbitmq`. CLI-пакет для создания проектов — `@likego/create`.
