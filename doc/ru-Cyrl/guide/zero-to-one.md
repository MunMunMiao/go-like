...
В этом checkout пакеты подключаются как workspace dependencies. В корневом репозитории указаны Bun `1.x`, Node.js `26.x`, Deno без фиксированного требования к версии, TypeScript `7.0.2` и k6 `2.1.0` как матрица проверки; подходит любой выпуск Bun 1.x или Node.js 26.x, а Deno по-прежнему проверяется без обязательной для репозитория фиксации версии. Текущая документация пакетов сообщает, что они ещё не опубликованы в npm.
