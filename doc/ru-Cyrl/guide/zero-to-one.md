...
В этом checkout пакеты подключаются как workspace dependencies. В корневом репозитории зафиксированы Bun `1.3.14`, Node.js `26.x`, Deno `2.9.4`, TypeScript `7.0.2` и k6 `2.1.0` как матрица проверки; подходит любой patch-релиз Node.js 26.x. Текущая документация пакетов сообщает, что они ещё не опубликованы в npm.
