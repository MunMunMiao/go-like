...
Устанавливайте только нужные части. Обычному HTTP-сервису достаточно `@likego/context`, `@likego/core`, `@likego/web` и адаптера выбранного Web-фреймворка. Для внутренних вызовов добавьте `@likego/client`, `@likego/transport` и `@likego/transport-http`; registry, источник конфигурации и Store выбираются явно.
