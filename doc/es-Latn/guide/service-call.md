...
Una llamada unary interna combina piezas pequeñas. `@likego/client` entrega una instantánea de `Discovery` a un `Selector` y después completa un intercambio `send`/`recv` a través de un `Transport`. La construcción usa opciones funcionales:
