...
Instala solo las piezas que necesites. Un servicio HTTP habitual parte de `@likego/context`, `@likego/core`, `@likego/web` y un framework que exponga un handler Fetch nativo. Para llamadas internas añade `@likego/client`, `@likego/transport` y `@likego/transport-http`; el registro, la configuración y el almacén se eligen de forma explícita.
