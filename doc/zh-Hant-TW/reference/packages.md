...
程序內呼叫與測試可以使用 `@likego/transport-memory`。內部 HTTP 是 `@likego/transport-http`；`@likego/transport-http/node` 提供 Node 的 `dial` 與 `listen` 實作，伺服器端支援 PEM TLS/mTLS 及透過 ALPN 協商 HTTP/2。Web 框架直接將原生 Fetch Handler 交給 `@likego/web`；LikeGo 不發布框架專用橋接套件。常見函式庫生命週期適配包含 `@likego/croner`、`@likego/bullmq`、`@likego/nats`、`@likego/pino`、`@likego/winston`，觀測工具則是 `@likego/prometheus` 與 `@likego/otel`。
...
Registry provider 的完整名稱是 `@likego/registry-mdns`、`@likego/registry-consul`、`@likego/registry-etcd`、`@likego/registry-kubernetes`、`@likego/registry-zookeeper`。Config provider 也包含 `@likego/config-consul`、`@likego/config-etcd`、`@likego/config-kubernetes`；Broker provider 是 `@likego/broker-memory` 與 `@likego/broker-rabbitmq`。
