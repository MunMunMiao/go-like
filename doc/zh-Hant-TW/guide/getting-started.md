...
只安裝服務用得到的套件。一個常見 HTTP 服務通常會用 `@likego/context`、`@likego/core`、`@likego/web`，再挑一個能匯出原生 Fetch Handler 的 Web 框架。內部服務呼叫才需要 `@likego/client`、`@likego/transport` 與 `@likego/transport-http`；註冊中心、設定來源和儲存後端都由應用程式明確選擇。
