...
bun run --filter @likego/example-cybersecurity-alert-triage test
bun run --filter @likego/example-cybersecurity-alert-triage test:docker
```

`test:docker` 使用固定 digest 的真实 etcd 3.7.1，验证 Config 读取、Registry 注册/注销、Store 写入与
