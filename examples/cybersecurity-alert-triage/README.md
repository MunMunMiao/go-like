...
`@go-like/health` readiness 确认规则已发布，再从 `@go-like/config` 当前不可变配置读取阈值并计算
处置队列。默认模式无需外部服务；设置 `ETCD_ADDRESS` 后，同一程序改用真实
`@go-like/config-etcd`、`@go-like/registry-etcd` 和 `@go-like/store-etcd`。
...
- `@go-like/context` 作为分诊和告警台账操作的首个参数。
...
- `src/main.ts`：组装健康接口并运行 go-like HTTP App；这是唯一直接执行入口。
...
```sh
bun run --filter @go-like/example-cybersecurity-alert-triage typecheck
bun run --filter @go-like/example-cybersecurity-alert-triage test:unit
...
```bash
bun run --filter @go-like/example-cybersecurity-alert-triage start
```

Core 的 `beforeStart` hook 加载 Config。看到 `GO_LIKE_EXAMPLE_READY` 后提交告警并检查 readiness：
...
`go-like/examples/security/triage/config`，再设置 `ETCD_ADDRESS`；可用 `ETCD_CONFIG_KEY` 覆盖 key。
