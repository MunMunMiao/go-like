...
`@likego/core` App 成功启动后才接受任务，并由 App 统一停止和等待 Server 的 `start(ctx)` Promise。

## 主要演示

- `@likego/core` 管理自定义 Worker Server 的启动和停止。
- `@likego/context` 作为任务提交和 Worker 生命周期操作的首个参数。
- `@likego/web` 输出可嵌入任意支持标准 Fetch API 的 runtime Handler。
- 任务规则、Worker、HTTP 路由和进程生命周期按真实职责拆分。

## 源码结构

- `src/transcode-jobs.ts`：转码任务、输出键和幂等规则。
- `src/transcode-worker.ts`：实现 LikeGo `Server` 的进程内转码 Worker。
...
bun run --cwd examples/media-transcoding-pipeline test
```

## 直接运行

```bash
HOST=127.0.0.1 PORT=3000 bun run --filter @likego/example-media-transcoding-pipeline start
```

`start` 会先构建本地 LikeGo 包，再由 `start:prepared` 把 `src/main.ts` 构建为
`.artifacts/main.mjs` 并启动。看到 `LIKEGO_EXAMPLE_READY` 后提交任务：
...
本示例不伪装 FFmpeg、对象存储、GPU 或消息队列；内存 Worker 只验证 LikeGo 对自定义
