...
bun run --filter @likego/example-saas-tenant-api test
bun run --filter @likego/example-saas-tenant-api test:docker
```

Docker E2E 使用固定 digest 的 Consul 2.0.2 和 Redis 8.8.1，验证 Config watch、Store
