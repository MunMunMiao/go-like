...
import { newCachedShipmentTrackingService } from "./cache"
import { newShipmentTrackingHandler } from "./http"
...
const handler = newShipmentTrackingHandler(service.track)
...
const app = newApp(
  ...
  afterStart(async function announceReady(ctx): Promise<void> {
    ...
    process.stdout.write(
      `GO_LIKE_EXAMPLE_READY=${JSON.stringify({ example: "logistics-shipment-tracking", origin })}\n`
    )
  })
)
