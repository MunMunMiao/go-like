...
import { newLearningEnrollmentService } from "./runtime"
...
const service = newLearningEnrollmentService({ "course-1": 10 })
...
const app = newApp(
  ...
  server(service.capacity.server, httpServer),
  afterStart(async function announceReady(ctx): Promise<void> {
    await httpServer.endpoint(ctx)
    process.stdout.write(
      `GO_LIKE_EXAMPLE_READY=${JSON.stringify({ example: "learning-enrollment", origin })}\n`
    )
  })
)
