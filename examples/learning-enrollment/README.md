...
演示一个拆成两个服务边界的选课系统：外部 Enrollment API 接受标准 Fetch 请求，应用层通过 `@likego/transport-memory` 调用内部 Capacity 服务预占课程席位，再保存选课结果。
...
本例实际使用 `@likego/transport-memory` 的 `listen`、`dial`、`send`、`recv` 和关闭流程，并由 `@likego/core` 承接内部 Server。测试通过真实内存传输执行席位预占，不是直接函数调用伪装 RPC。

## 验证

```bash
bun run --filter @likego/example-learning-enrollment typecheck
bun run --filter @likego/example-learning-enrollment test:unit
```

## 直接运行

```bash
HOST=127.0.0.1 PORT=3000 bun run --filter @likego/example-learning-enrollment start
```

看到 `LIKEGO_EXAMPLE_READY` 后选修预置的 `course-1`：
...
本例无需 Docker：Memory Transport 是 LikeGo 的真实进程内 provider，且没有外部中间件。它验证服务契约和生命周期，不宣称具备跨进程持久化能力。
