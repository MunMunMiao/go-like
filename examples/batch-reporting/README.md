...
> 状态：可执行基线。2026-07-23 已通过类型检查、业务单测、100% 示例源码覆盖率和固定 digest Redis Docker E2E。
...
| 组件            | 当前仓库真实 pin                                                                                                   | 用途                                        |
| --------------- | ------------------------------------------------------------------------------------------------------------------ | ------------------------------------------- |
| Redis           | `redis:8.8.1-alpine@sha256:8096655e437712b07503796fb64d81359256cfcff0ab29d95a7da72863786efb`，仓库门禁实测为 8.8.1 | BullMQ queue、lock、retry 与 stalled 状态。 |
| BullMQ          | `5.81.2`                                                                                                           | Queue 与 Worker 原生数据面。                |
| Croner          | `10.0.1`                                                                                                           | 定时调度。                                  |
...
以下命令均在 2026-07-23 fresh 执行并退出 0：
...
业务单测共 7 项，示例源码行与函数覆盖率均为 100%。真实 Docker E2E 已核对：Redis 8.8.1 与固定 image digest、
...
当前阶段不创建 TODO 文件。实现阶段若出现本文未解决且不阻断基线的不确定项，统一记录到本示例未来的
`TODO.md`，不得用散落代码注释或 README 中的含糊“以后支持”代替。

每条记录必须包含稳定 ID、状态、证据或阻塞原因、影响的不变量、最小完成条件和可运行验证命令。未完成能力
必须写成“未支持”，不能写成“已规划”或“可用”；只有代码、测试和 fresh 验证证据同时存在时才能关闭记录。
多副本 checkpoint、并行报表流等扩展只有在出现真实吞吐或可用性需求后才允许登记，不能削弱当前单 owner 基线。
