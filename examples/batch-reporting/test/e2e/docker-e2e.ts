...
interface Evidence {
  ...
  readonly cronerVersion: string
  readonly deterministicJobId: string
  readonly duplicateTickWaitingJobs: number
  readonly retryAttempts: readonly number[]
  readonly retryBackoffElapsedMs: readonly number[]
  readonly firstCheckpoint: number
  readonly rawWorkerExitCode: number
  readonly stalledAttemptsStarted: number
  readonly stalledCounter: number
  readonly finalCheckpoint: number
  readonly shutdownOrder: readonly string[]
  readonly persistentRedisConnectionsAfterStop: number
  readonly startPreparedEntrypoint: boolean
...
/** Runs the fixed-digest Redis reporting workflow and returns post-cleanup evidence. */
async function run(): Promise<Evidence> {
  const owner = `batch-reporting-${crypto.randomUUID()}`
  ...
  return Object.freeze({
    redisVersion,
    redisImageId,
    bullmqVersion: versions.bullmq,
    cronerVersion: versions.croner,
    deterministicJobId,
    duplicateTickWaitingJobs,
    retryAttempts: Object.freeze(retryAttempts.slice()),
    retryBackoffElapsedMs: Object.freeze([
      retryTimes[1]! - retryTimes[0]!,
      retryTimes[2]! - retryTimes[1]!
    ]),
    firstCheckpoint,
    rawWorkerExitCode,
    stalledAttemptsStarted,
    stalledCounter,
    finalCheckpoint,
    shutdownOrder: Object.freeze(shutdownOrder.slice()),
    persistentRedisConnectionsAfterStop: persistentAfterStop,
...
try {
  ...
  process.stdout.write(
    `LIKEGO_BATCH_REPORTING_E2E_RESULT=${JSON.stringify({ valid: true, evidence })}\n`
  )
} catch (error) {
  const message = error instanceof Error ? error.message : String(error)
  process.stderr.write(
    `LIKEGO_BATCH_REPORTING_E2E_RESULT=${JSON.stringify({ valid: false, error: message })}\n`
  )
  throw error
}
