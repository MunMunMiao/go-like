...
使用 `@likego/context` 贯穿预留仓储操作，使用 `@likego/web` 提供预留与释放的标准 Fetch 入口，并使用 `@likego/health` 在房型库存目录未加载时让 readiness 失败关闭。
...
```bash
bun run --filter @likego/example-hotel-room-reservation typecheck
bun run --filter @likego/example-hotel-room-reservation test:unit
```
...
`.artifacts/main.mjs` 并启动。看到 `LIKEGO_EXAMPLE_READY=...` 后预留一个房间：
