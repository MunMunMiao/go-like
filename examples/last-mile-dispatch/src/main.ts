...
import { newDispatchHandler } from "./http"
import { newDispatchService } from "./service"
...
const service = newDispatchService()
const origin = `http://${host}:${portNumber}`
const httpServer = newNodeServer(
  newDispatchHandler(service.dispatch),
  hostname(host),
  port(portNumber)
)
const app = newApp(
  ...
  afterStart(async function announceReady(ctx): Promise<void> {
    ...
    process.stdout.write(
      `GO_LIKE_EXAMPLE_READY=${JSON.stringify({ example: "last-mile-dispatch", origin })}\n`
    )
  })
)
