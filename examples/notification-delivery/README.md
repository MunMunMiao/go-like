...
`@likego/resilience` 的显式幂等 retry 重试瞬时 provider 故障，并用 circuit breaker
阻止持续故障继续冲击下游。成功 receipt 随后通过 `@likego/event` 编码并发布到
`@likego/broker-memory`，消费者把结果投影到 `@likego/store-memory`；`@likego/winston`
...
- 应用创建原生 Winston Logger，LikeGo 只包装请求日志和终态排空。
- `@likego/context` 作为投递应用和 provider 操作的首个参数。
...
```sh
bun run --filter @likego/example-notification-delivery typecheck
bun run --filter @likego/example-notification-delivery test:unit
```
...
```bash
bun run --filter @likego/example-notification-delivery start
```

看到 `LIKEGO_EXAMPLE_READY` 后发起一次真实 HTTP 请求：
