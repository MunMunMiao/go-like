...
test("declares and documents the Elysia bridge as the example boundary", async () => {
  ...
  expect(source).toContain('import { newElysiaHandler } from "@likego/elysia"')
  expect(source).not.toContain("function fetch(")
  expect(packageJson).toContain('"@likego/elysia": "workspace:*"')
  expect(readme).toContain("`@likego/elysia`")
  expect(readme).not.toContain("不需要 Elysia 专用 LikeGo 包")
})
