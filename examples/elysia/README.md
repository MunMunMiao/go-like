...
Elysia 继续拥有路由并处理传入的标准 `Request`。应用通过 `@likego/elysia` 的
`newElysiaHandler(app)` 稳定绑定单参数 Handler，再交给 LikeGo 受管 Node 生命周期宿主；框架适配包不复制
Elysia router 或 middleware。
...
- `src/app.ts`：组合 Elysia router 与 `@likego/elysia` Fetch bridge，创建可复用 Handler。
