...
本目录是 LikeGo 可执行案例入口。案例总数下限为 **40**，允许继续增加；当前共有 **44** 个，机器清单见
[`catalog.json`](./catalog.json)。其中 **43/44** 的服务进程是 Node.js，只有 `payments-ledger` **1/44**
直接使用 Bun 运行。`bun run start` 是所有案例统一的 workspace 命令，不代表每个案例的数据面 runtime 都是 Bun。

为避免一个案例重复计数，先单列唯一的 Bun runtime，再按主要入站形态划分其余 Node.js 案例：

| 分类               | Runtime | 数量 | 案例                                                                                                                                |
| ------------------ | ------- | ---: | ----------------------------------------------------------------------------------------------------------------------------------- |
| Web / API          | Node.js |   41 | 40 个行业案例中除 `payments-ledger`、`iot-telemetry`、`batch-reporting` 外的 37 个，加上 Elysia、H3、Hono、Vanilla Web 4 个接入案例 |
| Worker / scheduler | Node.js |    1 | `batch-reporting`                                                                                                                   |
| Message consumer   | Node.js |    1 | `iot-telemetry`                                                                                                                     |
| Bun runtime        | Bun     |    1 | `payments-ledger`；业务入口同时提供 Web API                                                                                         |
...
- `bun run start` 先构建仓库内 LikeGo 包；43 个 Node.js 案例随后构建 `--target=node` 并运行产物，
  `payments-ledger` 则直接运行 Bun 入口；
...
默认 HTTP 地址是 `http://127.0.0.1:3000`，可通过 `HOST`、`PORT` 覆盖。六个 `production`
案例（`commerce-catalog`、`saas-tenant-api`、`payments-ledger`、`iot-telemetry`、
...
Server，不伪装成 HTTP 服务。第七个 `production` 案例 `cybersecurity-alert-triage` 默认使用固定
Config、不注册 Registry，并以内存台账保存结果，可直接运行；其可选 etcd Config、Registry 与 Store
接入由固定 digest Docker 门禁验证。

仓库门禁会逐个真正启动、探测并停止所有无需外部服务的程序；真实中间件案例由 Docker 门禁验证：
...
Tier 不自动代表 Docker。只有表格明确列出真实服务的案例才拥有 Docker 证据；focused
...
| --: | ---------------------------------------------------------------------------- | ------------------------------------------ | ------------------------------------------------------------------------------------ | ----------- | ----------------------------------------------------- |
|   1 | [`commerce-catalog`](./commerce-catalog/README.md)                           | 电商：缓存失效时仍从 Pricing 获取权威价格  | Hono/Web → Cache → Client + Retry → HTTP Pricing；Docker 注入 Redis/Consul           | production  | 是：Consul 2.0.2、Redis 8.8.1                         |
|   2 | [`saas-tenant-api`](./saas-tenant-api/README.md)                             | SaaS：配置、缓存和限流不得跨租户泄漏       | Hono/Web → Config + Cache → Token Bucket；Consul Store 保存 runtime state            | production  | 是：Consul 2.0.2、Redis 8.8.1                         |
|   3 | [`payments-ledger`](./payments-ledger/README.md)                             | 支付：分录借贷平衡且幂等键只能生效一次     | Web + Core-owned outbox publisher Server → PostgreSQL ledger/outbox → NATS JetStream | production  | 是：PostgreSQL 18.4、NATS 2.14.3                      |
|   4 | [`iot-telemetry`](./iot-telemetry/README.md)                                 | IoT：重投和重连后至多产生一个业务结果      | NATS JetStream → Broker consumer → validation/dedup → Ack 或 DLQ                     | production  | 是：NATS JetStream 2.14.3                             |
|   5 | [`batch-reporting`](./batch-reporting/README.md)                             | 报表：从检查点恢复不得重复提交完成分片     | Croner/BullMQ E2E composition → report use case → Store checkpoint                   | production  | 是：Redis 8.8.1；BullMQ/Croner 在应用进程             |
...
|  38 | [`cybersecurity-alert-triage`](./cybersecurity-alert-triage/README.md)       | 安全：规则取最高严重度且重复上报幂等       | Config + Health → ledger；可选 etcd Config/Registry/Store                            | production  | 是：固定 digest etcd 3.7.1 Docker 门禁                |
...
| ---------------------------------------- | ------------------------------------------------- | ----------- | ----------------- |
| [`elysia`](./elysia/README.md)           | Elysia → `@likego/elysia` → `@likego/web` Handler | integration | 否：Node 接入测试 |
| [`h3`](./h3/README.md)                   | H3 → `@likego/h3` → `@likego/web` Handler         | integration | 否：Node 接入测试 |
| [`hono`](./hono/README.md)               | Hono → `@likego/hono` → `@likego/web` Handler     | integration | 否：Node 接入测试 |
