...
|   4 | [`iot-telemetry`](./iot-telemetry/README.md)                                 | IoT：稳定事件标识供下游幂等收敛重复        | NATS JetStream → Broker consumer → validation/dedup → Ack 或 DLQ                     | production  | 是：NATS JetStream 2.14.3                             |
|   5 | [`batch-reporting`](./batch-reporting/README.md)                             | 报表：重跑保持窗口标识且输出必须幂等       | Croner/BullMQ E2E composition → report use case → Store checkpoint                   | production  | 是：Redis 8.8.1；BullMQ/Croner 在应用进程             |
