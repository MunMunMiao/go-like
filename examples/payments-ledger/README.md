...
> 当前状态：已实现并验证。`src/main.ts` 是唯一程序入口，单元测试和固定 digest 的
> PostgreSQL + NATS Docker E2E 均可运行；
> 2026-07-23 的 fresh gate 结果见“验证结果”。
...
| NATS JetStream | `docker.io/library/nats:2.14.3-alpine@sha256:c11af972c99ae542de8925e6a7d9c533aa1eb039660420d2074beed6089b3bf0`，启动参数至少包含 `-js -sd /data` 并挂载持久 volume。 | 该仓当前真实门禁把此 digest 回读为 NATS Server `2.14.3`；NATS JS packages 固定为 `3.4.0`。升级时同时更新 tag、digest、版本断言和重连/JetStream 集成测试。 |
...
2026-07-23 fresh 执行以下门禁，均退出 `0`；本示例不生成 `dist`：

```bash
bun run --cwd examples/payments-ledger typecheck
bun run --cwd examples/payments-ledger test
bun run --cwd examples/payments-ledger test:coverage
bun run --cwd examples/payments-ledger test:docker
```

Docker E2E 已证实：并发同键请求收敛为 `1` 个幂等结果、`1` 个 transaction、`2` 条 posting 和 `1` 条
...
本门禁没有模拟 `PubAck` 后、数据库标记前的进程崩溃，也没有做多 publisher 压测；这两项仍是扩展部署测试，
不能从当前结果外推为已经验证。
...
- 只有确认 LikeGo 本身缺少完成该业务所需的能力时才创建本目录 `TODO.md`，并写明证据、影响、最小完成条件与
  验证命令，交由后续独立团队判伪或实施；应用自身未决事项进入 issue。
- 不在代码中留下没有负责人和退出条件的裸 `TODO`。
- 未决事项先创建可追踪 issue；代码中的标记统一写成 `TODO(<issue-id>): <原因>; remove when <可验证条件>`，
  issue 必须记录 owner、风险和验收条件。
- 版本选择、数据库约束、故障注入和 Docker readback 是首版实施门禁，不得用 TODO 延后。
- 完成事项在同一变更中删除标记；若没有有效 issue，则在合并前解决或删除该标记。
