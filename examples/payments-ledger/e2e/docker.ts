...
export async function main(): Promise<void> {
  ...
  try {
    ...
    const natsImageId = (
      await docker(ownedDocker, ["inspect", "--format", "{{.Image}}", NatsContainer])
    ).stdout
    const expectedPostgresImageId = (
      await docker(ownedDocker, ["image", "inspect", "--format", "{{.Id}}", PostgresImage])
    ).stdout
    const expectedNatsImageId = (
      await docker(ownedDocker, ["image", "inspect", "--format", "{{.Id}}", NatsImage])
    ).stdout
    assert(
    ...
    assert(
      postgresImageId === expectedPostgresImageId,
      `unexpected PostgreSQL image ID: ${postgresImageId}`
    )
    assert(natsImageId === expectedNatsImageId, `unexpected NATS image ID: ${natsImageId}`)
    ...
    assert(outageRejected, "outbox publish unexpectedly succeeded during NATS outage")
    const unpublished = (
