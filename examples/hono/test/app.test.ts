import { readFile } from "node:fs/promises"
import { join } from "node:path"

import { expect, test } from "bun:test"
import { newNodeServer } from "@likego/web/node"

import { newHandler } from "../src/app"

test("binds Hono through the published LikeGo framework bridge", async () => {
  const response = await newHandler()(new Request("https://example.test/users/42"))

  expect(response.status).toBe(200)
  expect(await response.json()).toEqual({ framework: "hono", id: "42" })
})

test("declares and documents the Hono bridge as the example boundary", async () => {
  const source = await readFile(join(import.meta.dir, "../src/app.ts"), "utf8")
  const packageJson = await readFile(join(import.meta.dir, "../package.json"), "utf8")
  const readme = await readFile(join(import.meta.dir, "../README.md"), "utf8")

  expect(source).toContain('import { newHonoHandler } from "@likego/hono"')
  expect(source).not.toContain("function fetch(")
  expect(packageJson).toContain('"@likego/hono": "workspace:*"')
  expect(readme).toContain("`@likego/hono`")
  expect(readme).not.toContain("不需要 Hono 专用 LikeGo 包")
})

test("composes Hono into the managed Node host", () => {
  expect(typeof newNodeServer(newHandler()).start).toBe("function")
})
