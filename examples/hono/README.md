...
Hono 继续拥有路由并暴露原生 Fetch 入口。应用通过 `@likego/hono` 的 `newHonoHandler(app)` 稳定绑定
单参数 Handler，再把它交给 `newNodeServer`；框架适配包不复制 router 或 middleware，Node 宿主只拥有
listener 生命周期。Node HTTP 翻译复用 `@hono/node-server`，但 LikeGo ABI 不暴露其 Node
`HttpBindings`、WebSocket 或静态文件扩展。

源码按职责拆分：

- `src/routes.ts`：Hono 的实际用户路由。
- `src/app.ts`：组合 Hono router 与 `@likego/hono` Fetch bridge，创建可复用 Handler。
