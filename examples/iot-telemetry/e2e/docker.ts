...
const Owner = `iot-telemetry-${RunId}`
...
async function main(): Promise<void> {
  ...
  console.log(
    `LIKEGO_EXAMPLE_IOT_TELEMETRY_E2E_RESULT=${JSON.stringify({
      image: { reference: NatsImage, imageId },
      versions: { nats: serverVersion, natsSdk: ExpectedSdkVersion },
      streams: {
        raw: { name: RawStream, storage: "file" },
        validated: { name: ValidatedStream, storage: "file" },
        deadLetter: { name: DeadLetterStream, storage: "file" }
      },
      durable: {
        name: ConsumerName,
        explicitAck: true,
      ...
      scenarios: {
        rawPubAckSequence: validPubAckSequence,
        validatedEventId,
        ackAckConfirmedByConsumerFloor: true,
        dlqPubAckBeforeTermErrorCode: dlqErrorCode,
        transientRedeliveries,
        reconnectAndContinue: true,
        boundedDrainMs: drainDurationMs,
        persistentNamedVolume: persistent,
