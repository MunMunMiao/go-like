...
async function main(): Promise<void> {
  ...
  try {
    ...
    assert(imageId === expectedImageId, `unexpected NATS image ID: ${imageId}`)
    ...
    const validPubAckSequence = await publishValid(client, "sensor_42", "msg_valid_1", "1")
    assert(validPubAckSequence === 1, `unexpected first raw PubAck: ${validPubAckSequence}`)
    try {
