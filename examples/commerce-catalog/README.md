...
bun run --filter @likego/example-commerce-catalog test
bun run --filter @likego/example-commerce-catalog test:docker
```

Docker E2E 使用固定 digest 的 Consul 2.0.2 与 Redis 8.8.1，验证注册、发现、真实内部 HTTP 调用、
