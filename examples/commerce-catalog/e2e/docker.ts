...
const Owner = `commerce-catalog-${RunId}`
...
async function main(): Promise<void> {
  ...
  console.log(
    `LIKEGO_EXAMPLE_COMMERCE_CATALOG_E2E_RESULT=${JSON.stringify({
      valid: true,
      images: { consul: ConsulImage, redis: RedisImage },
      versions: { consul: consulVersion, redis: redisVersion },
      scenarios: [
        "consul-register-discover-deregister",
        "pricing-http-transport",
        "hono-catalog-handler",
        "redis-cache-hit",
        "core-app-stop",
