...
根入口与 `./jetstream` 把官方 NATS Core `Subscription` 和 JetStream `ConsumerMessages` 接入 LikeGo 结构化
`Server` 生命周期；Broker 子路径从应用借用的 connection/client 创建一次订阅，并投影为 `@likego/broker`。
本包不创建或关闭连接、stream、durable consumer，也不接管确认、重试、死信和编解码策略。

## 导出布局

- `@likego/nats`：管理官方 `@nats-io/transport-node` 的原生 `Subscription`；
- `@likego/nats/broker`：从借用的 `NatsConnection` 创建并拥有一次 Core Subscription；
- `@likego/nats/jetstream`：管理官方 `@nats-io/jetstream` 的原生 `ConsumerMessages`；
- `@likego/nats/jetstream/broker`：从借用的 JetStream client 与应用 factory 取得并拥有一次
  ConsumerMessages。

四个入口都保留官方 SDK 类型和 Error identity。应用拥有数据面与连接；LikeGo 只在资源成功接纳后拥有对应的
...
Broker provider 会通过 `@likego/broker/provider` 关联已有 completion；因此被动原生终止或 handler
失败会拒绝 `newBrokerServer(...).start()`，但应用侧 `Subscriber` 仍然只有 `topic` 与
`unsubscribe(ctx)`。

## NATS Core：`@likego/nats`
...
工厂不接收 LikeGo `Context`，因此不会把 Context-first handler ABI 注入官方 NATS 数据面。应用仍负责
...
## JetStream：`@likego/nats/jetstream`
...
也可提供返回 `ConsumerMessages | PromiseLike<ConsumerMessages>` 的无参工厂。工厂同样不接收 LikeGo `Context`。
...
- `Subscription` 与 `ConsumerMessages` 都是 application-owned/native-borrowed、LikeGo-owned stop。
- 直接对象只在成功接纳后向 LikeGo 转移终止操作权；工厂创建但尚未接纳的临时对象允许 LikeGo 回滚。
...
固定官方 `@nats-io/transport-node@3.4.0` 与 `@nats-io/jetstream@3.4.0`，并在 Bun 1.3.14、Node.js 24.18.0
和 Node.js 26.5.0 门禁运行。
...
`NatsConnection.info` 两个已知 `TS2420`。中央 published 类型门禁会精确匹配这些诊断，再使用官方原生类型执行
consumer 检查；不得用 LikeGo facade 掩盖上游声明问题。
...
bun run build
bun run e2e:docker:core
bun run e2e:docker:jetstream
```

两条 Docker 门禁都从实际安装的官方 SDK `package.json` 读取版本，并使用固定 digest 的 NATS Server 2.14.3。
...
ConsumerMessages 保留。最终是否通过以同一工作区最后一次完整 `bun run verify` 的终态为准。
