...
function allowedRestParameter(node: Node, sourceFile: SourceFile): boolean {
  return (
    node.getText(sourceFile) === "...options: readonly NatsCoreOption[]" &&
    node.parent.getText(sourceFile).startsWith("export function newNatsCoreServer(") &&
...
async function syntheticViolations(source: string): Promise<readonly string[]> {
  const root = await mkdtemp(join(tmpdir(), "likego-nats-core-source-policy-"))
...
test("the functional-option rest exception requires a same-line explanation", async () => {
  const violations = await syntheticViolations(
    [
      "/** Applies one option. */",
      "type NatsCoreOption = (value: unknown) => void",
      "/** Unjustified option overload. */",
      "export function newNatsCoreServer(...options: readonly NatsCoreOption[]): void",
      "/** Justified option overload. */",
      "export function newNatsCoreServer(...options: readonly NatsCoreOption[] /* likego-typed-rest: preserves the functional-option ABI. */): void"
    ].join("\n")
  )
