...
async function installedSdkVersion(): Promise<string> {
  const manifest: PackageManifest = await Bun.file(
    `${import.meta.dir}/../../node_modules/@nats-io/transport-node/package.json`
  ).json()
  assert(typeof manifest.version === "string", "official NATS transport package has no version")
...
async function main(): Promise<void> {
  ...
  try {
    ...
    await Bun.sleep(50)
    scenarioEvidence["startup-cancel-unconsumed-subscription-cleanup"] = Object.freeze({
      exactCause: true,
      handlerCalls: 0,
      deliveryDelta: lateSubscription.getReceived() - deliveryBefore,
      borrowedConnectionOpen: !connection.isClosed() && !connection.isDraining()
    })
    scenarios.push("startup-cancel-unconsumed-subscription-cleanup")
    ...
    scenarioEvidence["startup-cancel-direct-subscription-preserved"] = Object.freeze({
      exactCause: true,
      openAfterRejectedStart,
      applicationDeliveries: directDeliveries,
      applicationCleanupTerminal: directSubscription.isClosed(),
      borrowedConnectionOpen: !connection.isClosed() && !connection.isDraining()
    })
    scenarios.push("startup-cancel-direct-subscription-preserved")
    ...
    scenarioEvidence["raw-pub-sub"] = Object.freeze({
      subject: rawSubjects[0],
      subjectMatches: rawSubjects[0] === rawSubject,
      delivered: rawBodies.length,
      bodyMatch: rawBodies[0] === "one",
      nativeSubscriptionTerminal: true,
      borrowedConnectionOpen: !connection.isClosed() && !connection.isDraining()
    })
    scenarios.push("raw-pub-sub")
    ...
    scenarioEvidence["typed-event-queue-group-round-trip"] = Object.freeze({
      published: 12,
      delivered: typedIds.length,
      uniqueIds: new Set(typedIds).size,
      decodedNames: typedNames.length,
      nativeSubjectsMatch: typedNativeSubjects.every((subject) => subject === typedSubject),
      nativeSubscriptionsTerminal: true,
      borrowedConnectionOpen: !connection.isClosed() && !connection.isDraining()
    })
    scenarios.push("typed-event-queue-group-round-trip")
    ...
    scenarioEvidence["queue-group-distribution"] = Object.freeze({
      published: 20,
      totalDelivered: firstIds.length + secondIds.length,
      uniqueDelivered: uniqueIds.size,
      firstWorker: firstIds.length,
      secondWorker: secondIds.length,
      nativeSubscriptionsTerminal: true,
      borrowedConnectionOpen: !connection.isClosed() && !connection.isDraining()
    })
    scenarios.push("queue-group-distribution")
    ...
    scenarioEvidence["at-most-once-handler-failure-isolation"] = Object.freeze({
      observedFailures,
      redeliveries: Math.max(0, failedDeliveries - 1),
      postFailureDeliveries: postFailure.length,
      postFailureBody: postFailure[0],
      nativeSubscriptionTerminal: true,
      borrowedConnectionOpen: !connection.isClosed() && !connection.isDraining()
    })
    scenarios.push("at-most-once-handler-failure-isolation")
    ...
    scenarioEvidence["transient-outage-reconnect"] = Object.freeze({
      ...
      postRestartBody: recovered[0],
      postRestartSubjectMatches: recoveredSubjects[0] === reconnectSubject,
      nativeSubscriptionTerminal: true,
      borrowedConnectionOpen: !connection.isClosed() && !connection.isDraining()
    })
  ...
  console.log(
    `LIKEGO_NATS_CORE_E2E_RESULT=${JSON.stringify({
      valid: true,
      package: "@likego/nats",
      image: NatsImage,
      imageId,
...
try {
  ...
  console.error(`LIKEGO_NATS_CORE_E2E_RESULT=${JSON.stringify({ valid: false, error: message })}`)
  process.exitCode = 1
}
