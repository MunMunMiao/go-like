...
/** Reads the actual installed official SDK version instead of duplicating a constant. */
async function installedSdkVersion(): Promise<string> {
  const manifest: PackageManifest = await Bun.file(
    `${import.meta.dir}/../../node_modules/@nats-io/jetstream/package.json`
  ).json()
  assert(typeof manifest.version === "string", "official JetStream package has no version")
...
async function main(): Promise<void> {
  ...
  let borrowedConnectionOpenAfterForce = false
  ...
  try {
    ...
    scenarioEvidence["startup-cancel-official-unconsumed-iterator-rollback"] = Object.freeze({
      exactCause: true,
      closedPendingBeforeConsumption,
      closedAfterIteratorConsumption,
      durableConsumerPreserved: true,
      borrowedConnectionOpen: !connection.isClosed() && !connection.isDraining()
    })
    scenarios.push("startup-cancel-official-unconsumed-iterator-rollback")
    ...
    scenarioEvidence["startup-cancel-direct-consumer-messages-preserved"] = Object.freeze({
      exactCause: true,
      closedPendingAfterRejectedStart,
      applicationDeliveries: directDeliveries,
      ackConfirmed: directAckConfirmed,
      closedAfterApplicationCleanup,
      durableConsumerPreserved: true,
      borrowedConnectionOpen: !connection.isClosed() && !connection.isDraining()
    })
    scenarios.push("startup-cancel-direct-consumer-messages-preserved")
    ...
    scenarioEvidence["durable-explicit-raw-jsmsg-ackack"] = Object.freeze({
      subjectMatches: rawSubjects[0] === rawSubject,
      bodyMatch: rawBodies[0] === "one",
      ackConfirmed: rawAcks[0] === true,
      ackPendingAfterAck: rawInfo.num_ack_pending,
      durableConsumerPreserved: true,
      nativeIteratorTerminal: true,
      borrowedConnectionOpen: !connection.isClosed() && !connection.isDraining()
    })
    scenarios.push("durable-explicit-raw-jsmsg-ackack")
    ...
    scenarioEvidence["typed-event-puback-ackack"] = Object.freeze({
      pubAckStream: typedPubAck.stream,
      pubAckSequence: typedPubAck.seq,
      decodedId: typedIds[0],
      ackConfirmed: typedAckResults[0] === true,
      nativeIteratorTerminal: true,
      durableConsumerPreserved: true,
      borrowedConnectionOpen: !connection.isClosed() && !connection.isDraining()
    })
    scenarios.push("typed-event-puback-ackack")
    ...
    scenarioEvidence["typed-invalid-schema-native-nak-term"] = Object.freeze({
    ...
    scenarios.push("typed-invalid-schema-native-nak-term")
    ...
    scenarioEvidence["explicit-ack-max-deliver"] = Object.freeze({
      deliveryCounts,
      maxDeliver: 2,
      totalDeliveries: deliveryCounts.length,
      nativeIteratorTerminal: true,
      borrowedConnectionOpen: !connection.isClosed() && !connection.isDraining()
    })
    scenarios.push("explicit-ack-max-deliver")
    ...
    scenarioEvidence["dlq-real-publish-failure-redelivery-puback-term-exactly-once"] =
      Object.freeze({
        publishFailureAtThreshold,
        sourceAckPending: sourceInfo.num_ack_pending,
        deadLetterMessages: deadStreamInfo.state.messages,
        deadLetterBodyMatch,
        deadLetterAckConfirmed,
        nativeIteratorTerminal: true,
        durableConsumerPreserved: true,
        borrowedConnectionOpen: !connection.isClosed() && !connection.isDraining()
      })
    scenarios.push("dlq-real-publish-failure-redelivery-puback-term-exactly-once")
    ...
    borrowedConnectionOpenAfterForce = !connection.isClosed() && !connection.isDraining()
    ...
    scenarioEvidence["transient-outage-reconnect"] = Object.freeze({
      disconnectObserved: statuses.includes("disconnect") || statuses.includes("reconnecting"),
      reconnectObserved: statuses.includes("reconnect"),
      postRestartDeliveries: recovered.length,
      postRestartAckConfirmed: recoveredAcks[0] === true,
      durableConsumerRestored: true,
      nativeIteratorTerminal: true,
      borrowedConnectionOpen: !connection.isClosed() && !connection.isDraining()
    })
    scenarios.push("transient-outage-reconnect")
  ...
  console.log(
    `LIKEGO_NATS_JETSTREAM_E2E_RESULT=${JSON.stringify({
      valid: true,
      package: "@likego/nats",
      image: NatsImage,
      imageId,
      ...
      cleanup: {
        ...
        remainingContainers: 0,
        activeHandlers: 0,
        borrowedConnectionOpenAfterForce
      }
    })}`
  )
}
...
try {
  ...
  console.error(
    `LIKEGO_NATS_JETSTREAM_E2E_RESULT=${JSON.stringify({ valid: false, error: message })}`
  )
  process.exitCode = 1
}
