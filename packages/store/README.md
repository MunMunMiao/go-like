...
go-like 的可移植 Store 公共契约。它定义 Context-first CRUD、TTL、CAS、稳定分页、不可变快照和
...
Provider 作者可从 `@go-like/store/provider` 导入 option、snapshot 与稳定冲突错误 helper；应用只使用根入口。
...
- 两者属于同一个 write condition，按 functional option 的既有 ordered/last-wins 规则生效；自定义 reducer
  同时提交两者会 fail closed。
- 条件不成立统一返回 `GO_LIKE_STORE_CONFLICT`。`expectedRevision: null` 表示调用方期望 key 不存在，
