...
function sourceViolations(sourceFile: SourceFile): string[] {
  ...
  if (/from ["']node:|import\(["']node:/.test(sourceFile.text)) {
    violations.push("node-runtime-import")
  }
  return violations
}

test("portable source contains no classes assertions any suppression or Node imports", async () => {
  ...
  try {
    ...
    if (project === undefined) throw new Error("Memory Store source-policy project is missing")
    const violations: string[] = []
    for (const name of await readdir(sourceRoot)) {
      if (!name.endsWith(".ts")) continue
      const sourceFile = await project.program.getSourceFile(join(sourceRoot, name))
      if (sourceFile === undefined) throw new Error(`Memory Store source file ${name} is missing`)
