...
test("publishes one portable non-resident Memory Store package", async () => {
  ...
  expect(capability).toMatchObject({
    ...
    exports: {
      ".": {
        ...
        ownerResources: [],
        capabilities: ["store", "store-memory"]
      }
    }
  })
})
...
test("package shell source and test inventories are exact", async () => {
  ...
  expect(sources).toEqual(["index.ts", "store.ts"])
