...
console.log(
  `LIKEGO_STORE_ETCD_DOCKER_V1=${JSON.stringify({
    valid: true,
    image: Image,
    etcd: etcdVersion,
    checks: [
    ...
    scenarios: ["etcd-store-crud-cas-pagination", "etcd-store-lease-restart"],
    scenarioEvidence: {
      ...
      "etcd-store-lease-restart": {
        leaseExpired: true,
        leaseRevoked: true,
        clientIndependence: true,
        restartPreserved: true
      }
    },
    cleanup: { remoteKeys: 0, remoteLeases: 0, containerRemoved: removed }
  })}`
)
