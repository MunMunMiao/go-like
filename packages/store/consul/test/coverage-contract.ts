...
for (const record of records) {
  ...
  if (
    ...
    throw new Error(`SF:${source} coverage is below 100% or counters are invalid`)
  }
...
await validateBunPackageCoverage(process.cwd(), lcov)
const missing = Array.from(expected).filter(function absent(source) {
...
// src/types.ts is intentionally type-only; typecheck and test/public-types.ts own its contract.
