...
const ResultMarker = "LIKEGO_STORE_CONSUL_E2E_RESULT"
...
async function main(): Promise<void> {
  ...
  const result = Object.freeze({
    ...
    scenarioEvidence: {
      "consul-store-root-isolation": {
        externalKvIgnored: Object.getOwnPropertyDescriptor(primaryEvidence, "externalKvIgnored")
          ?.value,
        differentRootsIsolated: Object.getOwnPropertyDescriptor(
          primaryEvidence,
          "differentRootsIsolated"
        )?.value,
        crossRootCursorRejected: Object.getOwnPropertyDescriptor(
          primaryEvidence,
          "crossRootCursorRejected"
        )?.value,
        corruptOwnedDataFailedClosed: Object.getOwnPropertyDescriptor(
          primaryEvidence,
          "corruptOwnedDataFailedClosed"
        )?.value
      },
      "consul-store-crud-cas-pagination-ttl": primaryEvidence,
      "consul-store-acl-redaction": aclEvidence
    },
    cleanup: {
      remoteKv: primaryKvResidual + aclKvResidual,
      remoteSessions: primarySessionResidual + aclSessionResidual,
      containers: containerResidual
    },
    residue: {
      primaryKv: primaryKvResidual,
      primarySessions: primarySessionResidual,
      aclKv: aclKvResidual,
      aclSessions: aclSessionResidual,
      containers: containerResidual
    }
  })
  const serialized = JSON.stringify(result)
  if (serialized.includes(AclToken)) throw new Error("ACL token leaked into integration evidence")
  console.log(`${ResultMarker}=${serialized}`)
}
