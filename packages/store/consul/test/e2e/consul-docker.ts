...
async function aclScenarios(address: string): Promise<
  Readonly<{
    deniedStatus: number
    authorizedRead: boolean
    ttlDeleted: boolean
    tokenOnlyInHeader: boolean
  }>
> {
...
async function main(): Promise<void> {
  ...
  try {
    ...
    const expectedImageId = (await docker(["image", "inspect", "--format", "{{.Id}}", Image]))
      .stdout
    if (
      !binaryVersion.includes("Consul v2.0.2") ||
    ...
    const aclEvidence = await aclScenarios(aclAddress)
    if (
      aclEvidence.deniedStatus !== 403 ||
      !aclEvidence.authorizedRead ||
      !aclEvidence.ttlDeleted ||
      !aclEvidence.tokenOnlyInHeader
    ) {
      throw new Error("Consul ACL isolation evidence was incomplete")
    }
