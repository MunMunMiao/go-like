...
bun run --filter @likego/store-vault test:coverage
bun run --filter @likego/store-vault build
bun run --filter @likego/store-vault test:docker
```
...
远端 root 清理和容器零残留；证据见 `test/integration/vault-2.0.3-report.md`。
