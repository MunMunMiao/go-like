...
const ExpectedRedisVersion = "8.8.1"
...
interface RawWorkerCrashEvidence {
  readonly exitCode: number
  readonly lockedJobId: string
}
...
async function crashRawWorker(
...
): Promise<RawWorkerCrashEvidence> {
  ...
  return Object.freeze({ exitCode, lockedJobId: expectedJobId })
}
...
/** Executes the fixed-digest Redis 8.8.1 release gate. */
async function main(): Promise<void> {
  ...
  assert(bullmqVersion === "5.81.2", `unexpected BullMQ version: ${bullmqVersion}`)
  ...
  try {
    ...
    scenarios.push("retry-attempts-fixed-backoff-and-borrowed-queue")
    scenarioEvidence["retry-attempts-fixed-backoff-and-borrowed-queue"] = {
      processorAttempts: attempts.length,
      nativeAttemptsMade: completedRetry.attemptsMade,
      nativeTokensNonEmpty: nativeTokens.every((token) => token.length > 0),
      nativeSignalObserved,
      fixedBackoffElapsedMs: attemptTimes[2]! - attemptTimes[0]!,
      firstFixedBackoffElapsedMs,
      secondFixedBackoffElapsedMs,
      connectionsBeforeWorkerStart: baselineConnections,
      connectionsAfterWorkerStop: retryConnectionsAfterStop,
      privateConnectionsReturnedToBaseline: retryConnectionsAfterStop === baselineConnections,
      applicationQueueUsableAfterWorkerStop: queriedPostStop?.data.usable === true
    }
    ...
    const acceptedOutageWorker = await outageWorkerCreated.promise
    assert(
      outageFactoryCalls === 1,
      `expected one application Worker factory call, observed ${outageFactoryCalls}`
    )
    await waitUntil("outage Worker connections", async () => {
    ...
    let outageWorkerReady = false
    acceptedOutageWorker.once("ready", () => {
      outageWorkerReady = true
    })
    ...
    await waitUntil("Worker Redis reconnect", () => outageWorkerReady, 60_000)
    await waitUntil(
      "outage Worker connections",
      async () => {
        return queue !== null && (await queueConnectionCount(queue)) === baselineConnections + 2
      },
      60_000
    ...
    const outageDidNotPublishTerminal = !outageTerminal
    await stopManaged(outageManaged, managedWorkers)
    await waitUntil("recovered Worker connections to return to baseline", async () => {
      return queue !== null && (await queueConnectionCount(queue)) === baselineConnections
    })
    scenarios.push("redis-stop-start-recovery-with-observational-errors")
    scenarioEvidence["redis-stop-start-recovery-with-observational-errors"] = {
      outageErrorsObservedWhileStopped,
      applicationFactoryCalls: outageFactoryCalls,
      recoveredJobProcessed: recoveredJobs.includes("after-outage"),
      outageDidNotPublishTerminal,
      privateConnectionsReturnedToBaseline: true
    }

    const stalledJob = await queue.add("stalled", { recover: true }, { jobId: "real-stalled-lock" })
    const rawWorkerCrash = await crashRawWorker(
      "127.0.0.1",
      port,
      queueName,
      prefix,
    ...
    scenarios.push("independent-raw-worker-crash-and-stalled-recovery")
    scenarioEvidence["independent-raw-worker-crash-and-stalled-recovery"] = {
      rawWorkerLockedJobId: rawWorkerCrash.lockedJobId,
      rawWorkerExitCode: rawWorkerCrash.exitCode,
      recoveredJobId: recoveredStalled[0],
      attemptsStarted: completedStalled.attemptsStarted,
      stalledCounter: completedStalled.stalledCounter,
      privateConnectionsReturnedToBaseline: true
    }
    ...
    const nativeCancellationCause = requiredError(
      forcedSignalCause,
    ...
    scenarios.push("noncooperative-provider-timeout-until-handler-terminal")
    scenarioEvidence["noncooperative-provider-timeout-until-handler-terminal"] = {
      contextCanceled: forcedSignalCanceled,
      cancellationCauseName: nativeCancellationCause.name,
      nativeSignalCanceled: forcedSignalCanceled,
      runtimePendingBeforeHandlerRelease: true,
      nativeTerminalObservedAfterRelease: true,
      errorIdentityStable: terminalFailure === adapterTimeout,
      privateConnectionsReturnedToBaseline: true
    }
    ...
    scenarios.push("application-closes-queue-last-and-zero-connections")
    scenarioEvidence["application-closes-queue-last-and-zero-connections"] = {
      applicationQueueUsableBeforeClose: finalQuery?.data.queryable === true,
      privateConnectionsBeforeQueueClose: baselineConnections,
      persistentConnectionsAfterQueueClose: 0
    }
  ...
  console.log(
    `LIKEGO_BULLMQ_E2E_RESULT=${JSON.stringify({
      valid: true,
...
try {
  ...
  console.error(`LIKEGO_BULLMQ_E2E_RESULT=${JSON.stringify({ valid: false, error: message })}`)
  process.exitCode = 1
}
