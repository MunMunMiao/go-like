...
if (JSON.stringify(inventory) !== JSON.stringify(expected)) {
  throw new Error(`unexpected Winston adapter production inventory: ${inventory.join(",")}`)
}
...
const required = new Set(inventory.filter((source) => source !== "src/types.ts"))
...
if (seen.size !== required.size) {
  throw new Error("LCOV does not contain the exact executable Winston adapter production inventory")
}
console.log(
  `LIKEGO_BUN_COVERAGE_CONTRACT=${JSON.stringify({
