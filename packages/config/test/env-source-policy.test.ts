...
const AllowedModules = new Set(["@likego/config"])
...
test("AST policy rejects explicit any, spread, and rest syntax", async () => {
  const root = await mkdtemp(join(tmpdir(), "likego-config-env-source-policy-"))
  ...
  try {
    ...
    const violations = await withProject(root, async function inspect(project) {
...
test("development trees contain only TypeScript and extensionless relative module specifiers", async () => {
  ...
  const files = [
    "src/env.ts",
    "test/env.test.ts",
    "test/env-public-api.test.ts",
    "test/env-public-types.ts"
  ]
  for (const file of files) {
    const text = await Bun.file(`${packageRoot}/${file}`).text()
    for (const match of text.matchAll(/(?:from\s+|import\s*\()(["'])(\.\.?\/[^"']+)\1/g)) {
      const specifier = match[2]
      if (specifier !== undefined && extname(specifier) !== "")
        violations.push(`relative-extension:${file}:${specifier}`)
    }
  }
  expect(violations).toEqual([])
})
