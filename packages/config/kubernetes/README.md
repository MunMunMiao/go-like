...
`@likego/config-kubernetes` 使用调用方注入的标准 Web `fetch`，从一个命名空间内的
...
中断时先按相同 selector LIST 对账，再继续。删除会触发重载，LikeGo Config 保留
...
每个 source 只代表一个资源的一个 key；多个 source 会由 LikeGo Config 依次合并，但
