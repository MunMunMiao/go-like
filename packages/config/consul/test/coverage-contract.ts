...
if (JSON.stringify(inventory) !== JSON.stringify(expected)) {
  throw new Error(`unexpected Consul config production inventory: ${inventory.join(",")}`)
}
...
if (seen.size !== required.size)
  throw new Error("LCOV does not contain the exact Consul config production inventory")
console.log(
  `LIKEGO_BUN_COVERAGE_CONTRACT=${JSON.stringify({ package: "@likego/config-consul", files: seen.size, functions, lines })}`
)
