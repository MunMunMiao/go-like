...
async function main(): Promise<void> {
  ...
  let outageValueIdentityPreserved = false
  let outageReleasePreserved: unknown = null
  let outageFeatureEnabledPreserved: unknown = null
  ...
  try {
    ...
    consulVersion = versionResult.stdout.split("\n")[0] ?? "missing"
    ...
    scenarioEvidence["consul-kv-initial-load"] = Object.freeze({
      release: release.load(),
      revisionObserved: typeof initialSource.revision === "string"
    })
    ...
    outageValueIdentityPreserved = feature.load() === initialFeature
    outageReleasePreserved = release.load()
    const outageFeature = feature.load()
    outageFeatureEnabledPreserved = configObject(outageFeature) ? outageFeature.enabled : null
    ...
    const reconciledSource = await source.load(background())
    ...
    scenarioEvidence["restart-reconciles-new-index"] = Object.freeze({
      release: 2,
      replacementValuePublished: reconciledFeature !== initialFeature,
      indexRegressionObserved: indexObservations.some(function regressed(observation) {
        const match = /^([0-9]+)->[0-9]+:([0-9]+)$/.exec(observation)
        return (
          match?.[1] !== undefined && match[2] !== undefined && BigInt(match[1]) > BigInt(match[2])
        )
      }),
      revisionAdvanced: updatedSource.revision !== initialSource.revision
    })
    scenarioEvidence["blocking-query-publishes-change"] = Object.freeze({
      release: release.load(),
      blockingRevisionAdvanced: updatedSource.revision !== reconciledSource.revision,
      nestedValuePreserved: updatedFeature.enabled === true
    })
    ...
    const cleanup = await command(["rm", "--force", Name], true)
  ...
  scenarioEvidence["outage-preserves-last-good"] = Object.freeze({
    valueIdentityPreserved: outageValueIdentityPreserved,
    release: outageReleasePreserved,
    featureEnabled: outageFeatureEnabledPreserved,
    fetchFailures,
    retryStatuses,
    outageFailureCount
  })
  console.log(
    `${ResultMarker}=${JSON.stringify({
      ...
      image: Image,
      version: Version,
      dockerServerVersion: version.stdout,
      consulVersion,
      observedVersion: consulVersion,
      events,
      fetchAttempts,
      fetchFailures,
      retryStatuses,
      ...
      cleanup: {
        ...
        pendingTimers,
        activeFetches,
        activeBlockingFetches,
        watcherTerminal: events.includes("config-closed")
      }
    })}`
  )
}
