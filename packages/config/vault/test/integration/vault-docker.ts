...
const ResultMarker = "LIKEGO_CONFIG_VAULT_E2E_RESULT"
...
async function main(): Promise<void> {
  ...
  try {
    ...
    console.log(
      `${ResultMarker}=${JSON.stringify({ image: Image, version: Version, authentication: rejectedCode, first: first.revision, second: second.revision, requests: requests.length })}`
    )
  } catch (error) {
    primary = error
  } finally {
