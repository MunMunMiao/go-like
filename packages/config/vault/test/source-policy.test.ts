...
/** Reports whether one callable declaration has non-empty JSDoc. */
...
/** Finds prohibited production syntax, modules, globals, and undocumented callables. */
function sourceViolations(sourceFile: SourceFile): string[] {
  ...
  if (sourceFile.text.includes("Object.assign")) violations.push("object-assign")
  return violations
}

/** Opens one compiler project and always disposes its session. */
...
test("production source satisfies the complete portable syntax policy", async () => {
...
test("development trees contain no handwritten JavaScript or relative extensions", async () => {
  const packageRoot = `${import.meta.dir}/..`
  const violations: string[] = []
