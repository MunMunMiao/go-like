...
`watch` 是 Config 拥有的真实轮询 watcher。它默认每 5 秒读取一次完整 KV v2 响应，只有 generation revision
变化时 `next(ctx)` 才完成。revision 不能只使用递增 `version`：永久删除 metadata 后重建相同 path 会重新从
version 1 开始，而新版本的 `created_time` 不同。当前 token 因此同时绑定两者；旧版纯十进制 revision 会被接受，
并在第一次成功轮询时触发一次安全 resync。传输错误及 HTTP 404、408、425、429、5xx 使用可取消的指数退避并保留 last-good；
...
它已真实验证错误 token、metadata 永久删除后同为 version 1 的重建、普通版本递增、轮询发布、生命周期排空和容器零残留；普通单元测试不会自动启动
Docker，需在仓库根目录显式执行 `bun run test:e2e:suites -- --suite config-vault-docker`。

官方参考：

- https://developer.hashicorp.com/vault/api-docs/secret/kv/kv-v2
- https://developer.hashicorp.com/vault/api-docs
- https://github.com/hashicorp/vault-secrets-operator/blob/main/helpers/hmac.go
