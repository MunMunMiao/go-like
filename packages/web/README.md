...
转换、流式响应、Header 和未消费请求体清理由 `@hono/node-server` 2.0.11 提供；LikeGo 只拥有原生监听器
...
Hono、H3 与 Elysia 应用分别通过 `@likego/hono`、`@likego/h3`、`@likego/elysia` 转成 Handler，再显式传给
`newNodeServer`。框架继续拥有路由、中间件、流和错误策略，LikeGo 只稳定绑定原生 handler 并管理 listener
生命周期。
