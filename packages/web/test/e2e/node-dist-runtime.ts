...
if (probeAddress === null || typeof probeAddress === "string")
  throw new Error("runtime port probe did not bind TCP")
...
while (response === null && Date.now() < deadline) {
  try {
    response = await fetch(`http://127.0.0.1:${probeAddress.port}/runtime`)
  } catch {
    await new Promise<void>((resolveDelay) => setTimeout(resolveDelay, 5))
  }
}
if (response === null) throw new Error("runtime listener did not become ready")
if (response.status !== 200) throw new Error(`unexpected runtime status: ${response.status}`)
if ((await response.text()) !== "ok") throw new Error("unexpected runtime body")
...
console.log(`dist-runtime ok ${process.version}`)
