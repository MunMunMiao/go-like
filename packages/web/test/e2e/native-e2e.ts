...
verify(hostVersion === "2.0.11", `unexpected @hono/node-server version: ${hostVersion}`)
...
const scenarioEvidence = Object.freeze({
  "request-response-method-body-headers": Object.freeze({
    status: basic.status,
    method: firstSeen.method,
    body: firstSeen.body,
    responseBody,
    responseHeader,
    responseCookie
  }),
  "incremental-readable-stream-response": Object.freeze({
    firstChunk: new TextDecoder().decode(first.value),
    secondChunk: new TextDecoder().decode(second.value),
    streamTerminal: terminal.done,
    readerClosed,
    readerLockReleased,
    incrementalLatencyMs: elapsed
  }),
  "exact-one-argument-fetch-abi": Object.freeze({ argumentCount: handlerArgumentCount }),
  "graceful-drain-refuses-new-connections": Object.freeze({
    runningPendingBeforeHandlerRelease: gracefulPendingBeforeRelease,
    acceptedRequestDrained: true,
    newConnectionRefused: refused,
    listenerTerminal: true
  }),
  "client-abort-cancels-response-body": Object.freeze({
    canceledBeforeStop,
    cancelCalls: clientAbortCancelCalls,
    listenerTerminal: true
  }),
  "hard-force-noncooperative-body": Object.freeze({
    errorCode: hasErrorCode(forced) ? forced.code : null,
    streamTerminal: forceStreamTerminal,
    listenerTerminal: true,
    errorIdentityStable: stableForced === forced
  }),
  "force-port-rebind": Object.freeze({ sourcePort: forcePort, portReleased: forcePortReleased }),
  "released-port-rebind": Object.freeze({ sourcePort: mainPort, portReleased })
})

process.stdout.write(
  `LIKEGO_WEB_NODE_E2E_RESULT=${JSON.stringify({
    valid: true,
    package: "@likego/web/node",
    hostVersion,
    ...
    cleanup: {
      ...
      forcePortReleased,
      streamTerminal: forceStreamTerminal,
      listenerTerminal: acceptedServers === terminalServers,
      activeTimersBefore: baselineTimeouts,
      activeTimersAfter: finalTimeouts,
      pendingTimers,
      unhandledListenerDelta
    }
  })}\n`
)
