...
if (handler.length !== 1 || response !== expectedResponse) {
  throw new Error(`${runtime} Web Handler identity smoke failed`)
}
...
if (observed.context === undefined || cause(observed.context) !== abortReason) {
  throw new Error(`${runtime} request Context cause identity smoke failed`)
}
...
if (
  ...
  throw new Error(`${runtime} Web health handler smoke failed`)
}
