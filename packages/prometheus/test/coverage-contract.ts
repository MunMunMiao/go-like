...
if (JSON.stringify(inventory) !== JSON.stringify(expected)) {
  throw new Error(`unexpected Prometheus adapter production inventory: ${inventory.join(",")}`)
}
...
if (seen.size !== required.size)
  throw new Error("LCOV does not contain the exact Prometheus adapter production inventory")
console.log(
  `LIKEGO_BUN_COVERAGE_CONTRACT=${JSON.stringify({ package: "@likego/prometheus", files: seen.size, functions, lines })}`
)
