...
const ResultMarker = "LIKEGO_OTEL_E2E_RESULT"
...
const Scenarios = Object.freeze([
  ...
  "collector-restart-recovers-export",
  "shutdown-flushes-both-signals",
  "no-duplicate-shutdown-span"
])
...
let observedCollectorVersion = "unobserved"
let evidenceReady = false
let providersTerminal = false
  ...
  `${ResultMarker}=${JSON.stringify({
    ...
    scenarioEvidence: {
      "otlp-traces-and-metrics-export": {
        ...
        applicationResourceReceived: true
      },
      "collector-outage-does-not-block-business": {
        businessProgress: 2,
        traceFailureObserved: traceExportFailures.length > 0,
        metricFailureObserved: metricExportFailures.length > 0
      },
      "collector-restart-recovers-export": {
        collectorRestarted: true,
        recoveredTrace: true,
        recoveredMetric: true
      },
      "shutdown-flushes-both-signals": {
        traceFlushed: true,
        metricFlushed: true
      ...
      "no-duplicate-shutdown-span": {
        shutdownSpanCount,
        duplicateShutdownSpans: shutdownSpanCount - 1
      }
    },
    image: CollectorImage,
    otelVersion,
    version: CollectorVersion,
    collectorVersion: observedCollectorVersion,
    cleanupExitCode,
    cleanupFailures: 0,
    residualContainers,
    cleanup: {
      remainingContainers: residualContainers,
      duplicateShutdownSpans: shutdownSpanCount - 1,
      providersTerminal
    }
  })}`
