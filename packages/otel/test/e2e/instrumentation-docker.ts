...
const ResultMarker = "LIKEGO_OTEL_INSTRUMENTATION_E2E_RESULT"
...
interface InstrumentationEvidence {
  readonly traceId: string
  readonly webTraceId: string
  readonly collectorSpans: readonly string[]
...
let evidence: InstrumentationEvidence | null = null
...
try {
  ...
  evidence = {
    traceId,
    webTraceId,
    collectorSpans: names,
    response: Decoder.decode(captured.response.body),
    scenarioEvidence: {
      "client-http-server-parent-child": {
        traceId,
        chainSpanCount: 3,
        sameTrace: true,
        clientParentRoot: true,
        serverParentClient: true,
        responseBody: Decoder.decode(captured.response.body),
        responseHeader: captured.response.header["x-likego-e2e-response"] ?? ""
      },
      "standard-web-handler-parent-child": {
        traceId: webTraceId,
        chainSpanCount: 2,
        sameTrace: true,
        handlerParentRoot: true,
        requestBodyUsedAtHandlerEntry: webRequestState.bodyUsedAtHandlerEntry,
        requestBodyLockedAtHandlerEntry: webRequestState.bodyLockedAtHandlerEntry,
        responseBodyUsedBeforeOwnerRead: webResponseBodyUsedBeforeOwnerRead,
        responseBodyLockedBeforeOwnerRead: webResponseBodyLockedBeforeOwnerRead,
        requestBody: webRequestState.body,
        responseBody: webResponse,
        collectorReceived: true
      },
      "otel-collector-export": {
        collectorReceived: true,
        spanCount: names.length
      }
    }
  }
} catch (value) {
...
ensure(evidence !== null, "instrumentation E2E produced no evidence")
...
console.log(
  `${ResultMarker}=${JSON.stringify({
    ...
    package: "@likego/otel",
    otelVersion,
    collector: { image: CollectorImage, version: "0.157.0" },
    scenarios: [
      "client-http-server-parent-child",
      "standard-web-handler-parent-child",
      "otel-collector-export"
    ...
    webTraceId: evidence.webTraceId,
    collectorSpans: evidence.collectorSpans,
    collectorSpanCount: evidence.collectorSpans.length,
