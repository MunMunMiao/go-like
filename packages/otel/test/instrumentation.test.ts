...
describe("explicit OpenTelemetry instrumentation", () => {
  ...
  test("delegates stacked typed Client calls once and reports terminal codec failures", async () => {
    context.disable()
    propagation.disable()
    const manager = new AsyncLocalStorageContextManager().enable()
    expect(context.setGlobalContextManager(manager)).toBe(true)
    ...
    const contract = typedEndpoint("catalog", "TypedRead", codec, codec)
    ...
    const rejectedCodec: BodyCodec<{ readonly id: number }> = {
      contentType: "application/json",
      encode: codec.encode,
      decode() {
        throw decodeFailure
    ...
    await expect(
      client.call(
        background(),
        typedEndpoint("catalog", "TypedFail", codec, rejectedCodec),
        { id: 8 },
