...
type ClientDecorator = (
  ctx: Context,
  endpoint: CallRequest | Endpoint<unknown, unknown>,
  invoke: ClientInvocation
) => Promise<unknown>

/** Creates one stable client span name from the declared service operation. */
function clientSpanName(request: CallRequest | Endpoint<unknown, unknown>): string {
...
function isEndpoint(value: unknown): value is Endpoint<unknown, unknown> {
  return (
    ...
    "endpoint" in value &&
    typeof value.endpoint === "string" &&
    "requestCodec" in value &&
    typeof value.requestCodec === "object" &&
    value.requestCodec !== null &&
    "responseCodec" in value &&
    typeof value.responseCodec === "object" &&
    value.responseCodec !== null
  )
}
...
function wrapClient(client: Client, decorate: ClientDecorator): Client {
  ...
  async function call(ctx: Context, subject: unknown, _first?: unknown): Promise<unknown> {
    ...
    async function invokeTyped(callContext: Context): Promise<unknown> {
      const callArguments: [Context, Endpoint<unknown, unknown>, unknown, ...CallOption[]] = [
        callContext,
        endpoint,
        request
      ]
