...
if (JSON.stringify(inventory) !== JSON.stringify(expected)) {
  throw new Error(`unexpected Pino adapter production inventory: ${inventory.join(",")}`)
}
...
const required = new Set(
  inventory.filter((source) => source !== "src/thread-stream-node26-compat.ts")
)
...
for (const record of readFileSync(coveragePath, "utf8").split("end_of_record\n")) {
  ...
  if (source.startsWith("src/") && !allowed.has(source))
...
if (seen.size !== required.size)
  throw new Error("LCOV does not contain the exact executable Pino adapter production inventory")
console.log(
  `LIKEGO_BUN_COVERAGE_CONTRACT=${JSON.stringify({ package: "@likego/pino", files: seen.size, functions, lines })}`
)
