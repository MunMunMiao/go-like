...
/** Reads Pino or one dependency from the exact dependency tree resolved for Pino. */
async function packageVersion(packageName: string): Promise<string> {
  const pinoPackage = await realpath(resolve(process.cwd(), "node_modules", "pino", "package.json"))
  const path =
    packageName === "pino"
      ? pinoPackage
      : createRequire(pinoPackage).resolve(`${packageName}/package.json`)
  const value = JSON.parse(await readFile(path, "utf8")) as PackageJson
...
let startCaptureCloseRejected = false
...
let transportPreterminalRejected = false
...
try {
  ...
  structuralFileDestinationAccepted = true
  ...
  try {
    ...
    startPrototypeMutationRejected = true
  ...
  try {
    ...
    startOwnMethodMutationRejected = true
  ...
  try {
    ...
    startLoggerBindingDriftRejected = true
  ...
  try {
    ...
    startRegistrationReentryRejected = true
  ...
  try {
    ...
    startCaptureCloseRejected = true
  ...
  destinationEndingWindowRejected = true
...
process.stdout.write(
  `LIKEGO_PINO_E2E_RESULT=${JSON.stringify({
    valid: true,
    runtime: `Node.js ${process.versions.node}`,
    pinoVersion: await packageVersion("pino"),
    pinoOwnedSonicBoomVersion: await packageVersion("sonic-boom"),
    threadStreamVersion: await packageVersion("thread-stream"),
    scenarios: ["pino-native-destination-lifecycle", "pino-native-transport-lifecycle"],
    scenarioEvidence: {
      "pino-native-destination-lifecycle": {
        component: destinationComponent,
        redacted: destinationRedacted,
        nativeCloseObserved: destinationCloseObserved,
        structuralFileDestinationAccepted,
        fileLanded: destinationFileLanded,
        preterminalRejected: destinationPreterminalRejected,
        preterminalOwnershipCalls: destinationPreterminalOwnershipCalls,
        preterminalListenersRestored: destinationPreterminalListenersRestored,
        endingWindowRejected: destinationEndingWindowRejected,
        endingWindowOwnershipCalls: destinationEndingWindowOwnershipCalls,
        endingWindowListenersRestored: destinationEndingWindowListenersRestored,
        startPrototypeMutationRejected,
        startPrototypeMutationOwnershipCalls,
        startPrototypeMutationListenersRestored,
        startOwnMethodMutationRejected,
        startOwnMethodMutationOwnershipCalls,
        startOwnMethodMutationListenersRestored,
        startLoggerBindingDriftRejected,
        startLoggerBindingOwnershipUnchanged,
        startLoggerBindingListenersRestored,
        startRegistrationReentryRejected,
        startRegistrationReentryOwnershipCalls,
        startRegistrationReentryListenersRestored,
        startCaptureDestroyRejected,
        startCaptureDestroyOwnershipCalls,
        startCaptureDestroyListenersRestored,
        startCaptureErrorRejected,
        startCaptureErrorIdentityPreserved,
        startCaptureErrorOwnershipCalls,
        startCaptureErrorListenersRestored,
        startCaptureCloseRejected,
        startCaptureCloseOwnershipCalls,
        startCaptureCloseListenersRestored,
        startCaptureCloseDestinationOpen,
        ownerPrototypeMethodCaptured,
        ownerPrototypeReplacementCalls,
        ownerOwnMethodsCaptured,
        ownerOwnEndCalls,
        ownerOwnDestroyCalls,
        ownerLoggerMethodCaptured,
        ownerAdmittedFlushCalls,
        ownerReplacementFlushCalls,
        ownerStreamDriftRejected,
        ownerStreamDriftErrorStable,
        ownerStreamOriginalClosed,
        ownerStreamReplacementOpen
      ...
      "pino-native-transport-lifecycle": {
        ...
        nativeCloseObserved: transportCloseObserved,
        fileLanded: transportFileLanded,
        preterminalRejected: transportPreterminalRejected,
        preterminalOwnershipCalls: transportPreterminalOwnershipCalls,
        preterminalListenersRestored: transportPreterminalListenersRestored
      }
    },
    cleanup: { terminalRuns, directoryRemoved }
  })}\n`
)
