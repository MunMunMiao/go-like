...
import { endpoint, type BodyCodec, type Message } from "@likego/transport"
...
describe("Pino Client and Server request logging", () => {
  ...
  test("preserves typed Client calls and validates their runtime arguments", async () => {
    const captured = capturedLogger()
    const codec: BodyCodec<string> = {
      contentType: "application/test",
      encode: (value) => new TextEncoder().encode(value),
      decode: (body) => new TextDecoder().decode(body)
    }
    const contract = endpoint("catalog", "TypedGet", codec, codec)
    const option: CallOption = (options) => options
    const native = {
      async call(
        this: unknown,
