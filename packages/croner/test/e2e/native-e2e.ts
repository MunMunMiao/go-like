...
try {
  ...
  const finiteEvidence = Object.freeze({
    contextIdentityStable: true,
    tickCalls,
    observedNativeFailures: observed.length,
    schedulingExhausted: finite.value?.nextRun() === null && finite.value?.isRunning() === false,
    passiveDonePending: passive === "pending"
  })
  ...
  const busyBeforeStop = active.value?.isBusy() === true
  stopAttempts += 1
  ...
  const busyAfterStop = active.value?.isBusy() === true
  const contextCanceled = activeContext.value?.err() === canceled
  scenarios.push("explicit-stop-does-not-fabricate-native-callback-drain")
  release.resolve(undefined)
  await nativeRun
  assert.equal(active.value?.isBusy(), false)
  const activeEvidence = Object.freeze({
    busyBeforeStop,
    stopAndDoneSettledBeforeRelease: true,
    busyAfterStop,
    contextCanceled,
    callbackSettledAfterRelease: active.value?.isBusy() === false
  })
  ...
  process.stdout.write(
    `LIKEGO_CRONER_E2E_RESULT=${JSON.stringify({
      valid: true,
      package: "@likego/croner",
      cronerVersion: cronerPackage.version,
      engine: typeof Bun === "undefined" ? "node" : `bun-${Bun.version}`,
      runtime: `Node.js ${process.versions.node}`,
      runtimeVersion: process.versions.node,
      scenarios,
      scenarioEvidence: {
        "native-factory-resume-and-exhaustion-unobservable": finiteEvidence,
        "explicit-stop-does-not-fabricate-native-callback-drain": activeEvidence
      },
      tickCalls,
      observedNativeFailures: observed.length,
      acceptedServers,
