...
`@likego/croner` 把应用创建的原生 Croner `Cron` 实例接入 LikeGo 结构化 `Server` 生命周期。Cron
...
LikeGo 按返回顺序恢复任务。启动成功后，每个返回任务的生命周期所有权都归 Server；应用代码不得再对这些实例
...
- `start(ctx)` 返回的 Promise 覆盖完整运行期，并且只会由显式 LikeGo 停止操作结算。
...
Croner 也没有暴露被动终态 Promise 或事件。通过 `maxRuns`、`stopAt`、一次性调度自然耗尽，或在 LikeGo 外部调用
原生 `stop()`，都无法可靠结算 LikeGo 运行期。因此，本包虽然是 v1 发布阻断适配器，但会如实把常驻终态
