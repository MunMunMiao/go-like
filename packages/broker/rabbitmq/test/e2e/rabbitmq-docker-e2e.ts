...
/** Reads the installed SDK version from the package-local resolved dependency. */
async function installedSdkVersion(): Promise<string> {
  const manifest: PackageManifest = await Bun.file(
    `${import.meta.dir}/../../node_modules/amqplib/package.json`
  ).json()
  assert(typeof manifest.version === "string", "amqplib package has no version")
  return manifest.version
}

/** Executes the fixed-digest RabbitMQ provider gate and prints machine-readable evidence. */
    ...
    JSON.stringify(
      {
        ...
        sdk: { name: "amqplib", version: await installedSdkVersion() },
        image: RabbitMqImage,
        imageId,
        serverVersion,
        publisherConfirm: {
          initialAck: initialConfirmAck,
          recoveredGenerationAck,
          generations: confirmGenerations,
          pendingCloseRejected,
          concurrentRequested: ConcurrentPublisherConfirms,
          concurrentConfirmed,
          flowControlAccepted,
          flowControlBackpressured,
          forcedConnectionErrors: connectionErrors.length,
          unhandledRejections: lateRejections.length
        },
        scenarios: {
          publishSubscribe: deliveries,
          canonicalManualAck: true,
          consumerCountAfterBasicCancel,
          basicCancelRecovery: consumerCountAfterBasicCancel === 1,
          terminalFailureObserved,
          terminalConsumerCountAfterFailure,
          consumerCountAfterRecovery,
          consumerCountAfterUnsubscribe,
          consumerCountAfterUnsubscribeRecovery,
          stableConsumerNoDuplicate: true,
          applicationOwnedConnection: true
        },
        cleanupVerified
      },
      null,
