...
import { withConnClose } from "@likego/transport"
...
process.stdout.write(
  `LIKEGO_TRANSPORT_HTTP_NODE_E2E_RESULT=${JSON.stringify({
    valid: true,
    package: "@likego/transport-http/node",
    version: packageJson.version,
    runtime: `Node.js ${process.versions.node}`,
    runtimeVersion: process.versions.node,
    scenarios: ["unary-loopback", "graceful-drain", "hard-force-cleanup", "passive-host-failure"],
    scenarioEvidence: {
      "unary-loopback": {
        status: 200,
        acceptedBeforeRequest,
        requestBody: unaryRequestBody,
        responseBody: unaryResponseBody,
        requestHeader: unaryRequestHeader,
        responseHeader: unaryResponseHeader,
        requestBytes: encoder.encode(unaryRequestBody).byteLength,
        responseBytes: unaryResponse.body.byteLength,
        actualAddress: unaryAddress,
        actualPort: addressParts(unaryAddress).port,
        terminal: true,
        portReleased: true
      },
      "graceful-drain": {
        stopPendingBeforeRelease,
        requestDrained: gracefulResponse.status === 200,
        callerCancellationScoped,
        secondStopJoined: true,
        newConnectionRefused,
        responseBody: gracefulResponseBody,
        terminal: true,
        portReleased: true
      },
      "hard-force-cleanup": {
        closePendingBeforeForce,
        closeBeforeForce,
        clientStreamTerminal,
        hostTerminal: true,
      ...
      "passive-host-failure": {
        originalErrorIdentity,
        stableDoneRejection,
        serveErrorIdentity,
        listenerTerminal: true,
        activeHandlers: passiveInvocations,
        activeSockets: passiveSockets.size,
        portReleased: true
      }
    },
    connectionMetadata: { ipv4: ipv4Metadata, ipv6: ipv6Metadata },
    cleanup: {
      acceptedServers,
      terminalServers,
      portReleased: true,
    ...
    pooling: {
      ...
      canceledResponseDiscarded: true,
      truncatedResponseDiscarded: true,
      connectionCloseBypassedPool: true,
      activeSockets: poolSockets.size
    }
  })}\n`
)
