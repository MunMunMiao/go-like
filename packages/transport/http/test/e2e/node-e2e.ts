...
verify(callerCancellationScoped, "caller cancellation escaped its close attempt")
const gracefulClose = gracefulHandle.close(background())
const stopPendingBeforeRelease = await pending(gracefulClose)
verify(stopPendingBeforeRelease, "graceful close settled before the accepted request")
const newConnectionRefused = await connectionRefused(gracefulHandle.address())
verify(newConnectionRefused, "graceful close accepted a new connection")
...
verify(gracefulResponseBody === "drained:accepted", "graceful response body changed")
await gracefulClose
...
try {
  ...
  // Forced transport shutdown may complete or reject the already accepted response.
}
...
verify(stableDoneRejection, "host done Promise identity changed")
