...
import type { HTTPHostHandle, HTTPHostListenOptions } from "../src/types"
...
describe("Node HTTP host", () => {
  ...
  test("binds a real port, rejects pre-serve traffic, and exchanges standard Fetch values", async () => {
    ...
    try {
      ...
      expect(handle.done()).toBe(terminal)
      ...
      const callerFailure = new Error("close caller canceled")
      cancelCaller(callerFailure)
      await expect(handle.close(callerCtx)).rejects.toBe(callerFailure)
