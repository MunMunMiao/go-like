...
describe("unary Client", () => {
  ...
  test("calls one typed endpoint without replacing the raw Message API", async () => {
    ...
    const operation = endpoint("orders", "Create", codec, codec)
    const subject = harness({
      response: {
        header: { "content-type": "Application/Example+Number; charset=binary" },
    ...
    expect(raw.body).toEqual(new Uint8Array([42]))
    await client.close(background())
  })

  test("maps malformed typed responses to TransportProtocolError", async () => {
    const requestCodec: BodyCodec<number> = {
      contentType: "application/request",
      encode: (value) => new Uint8Array([value]),
      decode: (body) => body[0] ?? 0
    }
    const decodeFailure = new Error("response validation failed")
    const responseCodec: BodyCodec<number> = {
      contentType: "application/response",
      encode: (value) => new Uint8Array([value]),
      decode() {
        throw decodeFailure
      }
    }
    const operation = endpoint("orders", "Create", requestCodec, responseCodec)
    const responses = [
      { header: {}, body: new Uint8Array([1]) },
      {
        header: {
      ...
      { header: { "Content-Type": "application/response" }, body: new Uint8Array([1]) }
    ]
  ...
  test("rejects malformed typed calls before service I/O", async () => {
    const subject = harness()
    const invalidCodec: BodyCodec<number> = {
      contentType: "application/test",
      encode: () => null as never,
      decode: () => 0
    }
    const operation = endpoint("orders", "Create", invalidCodec, invalidCodec)
    const client = newClient(withDiscovery(subject.discovery), withTransport(subject.transport))

    await expect(client.call(background(), operation, 1)).rejects.toThrow(
      "Client request codec must return Uint8Array"
    )
