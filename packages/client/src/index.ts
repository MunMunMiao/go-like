...
function newTypedResponseBoundary<Response>(
  codec: BodyCodec<Response>
): readonly [
  validator: TypedResponseValidator,
  result: () => readonly [Response] | null,
  decode: (message: Message) => Promise<Response>
] {
  let result: readonly [Response] | null = null

  /** Decodes and captures the latest successful response attempt. */
  async function decode(response: Message): Promise<Response> {
    try {
      const value = messageContentType(response.header)
      if (value === null || mediaType(value) !== mediaType(codec.contentType)) {
        throw new TypeError("unexpected response Content-Type")
      }
      const decoded = await codec.decode(response.body)
      const captured: readonly [Response] = Object.freeze([decoded])
      result = captured
      return decoded
    } catch (value) {
  ...
  function capturedResult(): readonly [Response] | null {
    return result
  }
...
function isEndpoint(value: unknown): value is Endpoint<unknown, unknown> {
  return (
    typeof value === "object" &&
    value !== null &&
    typeof Reflect.get(value, "service") === "string" &&
    typeof Reflect.get(value, "endpoint") === "string" &&
    typeof Reflect.get(value, "requestCodec") === "object" &&
    typeof Reflect.get(value, "responseCodec") === "object"
  )
}
...
function createClient(
  ...
  async function call(
    ...
    const contract = endpointContract(
      subject.service,
      subject.endpoint,
      subject.requestCodec,
      subject.responseCodec
    )
    const options = runtimeCallOptions(values, 1)
    const resolved = callOptions(options)
    const body = await contract.requestCodec.encode(values[0])
    if (!(body instanceof Uint8Array)) {
      throw new TypeError("Client request codec must return Uint8Array")
    }
    const request: CallRequest = {
      service: contract.service,
      endpoint: contract.endpoint,
      message: {
        header: { [contentTypeHeader]: contract.requestCodec.contentType },
        body
      }
    }
    const boundary = newTypedResponseBoundary(contract.responseCodec)
