...
/** Carries one stable externally observable terminal promise. */
...
export function completion(): Completion {
  ...
  void result.promise.catch(ignoreFailure)
  return Object.freeze({ promise: result.promise, resolve: result.resolve, reject: result.reject })
}
...
/** Links caller cancellation, owner cancellation, and one operation timeout. */
export function operationLease(
  ...
  function callerAborted(): void {
    if (caller !== null && !controller.signal.aborted) {
      controller.abort(cause(ctx) ?? caller.reason)
    }
  }
...
/** Converts one aborted signal into its exact Error identity. */
...
export function waitForSignal(signal: AbortSignal, timeoutMs: number): Promise<void> {
  if (signal.aborted) return Promise.reject(signalFailure(signal, "ZooKeeper wait was aborted"))
  return new Promise<void>(function wait(resolve, reject): void {
    ...
    function aborted(): void {
      clearTimeout(timer)
      reject(signalFailure(signal, "ZooKeeper wait was aborted"))
    }
