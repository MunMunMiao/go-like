...
for (const record of records) {
  ...
  const sourceLine = lines.find(function source(line): boolean {
  ...
  seen.add(source)
  const fnf = lines.find(function found(line): boolean {
    return line.startsWith("FNF:")
  })
  const fnh = lines.find(function hit(line): boolean {
    return line.startsWith("FNH:")
  })
  const lf = lines.find(function found(line): boolean {
    return line.startsWith("LF:")
  })
  const lh = lines.find(function hit(line): boolean {
  ...
  if (
    ...
    throw new Error(`SF:${source} source coverage is below 100% or counters are invalid`)
  }
...
const missing = [...expected].filter(function absent(source): boolean {
