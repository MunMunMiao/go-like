...
bun run --filter @likego/registry-etcd test
bun run --filter @likego/registry-etcd build
LIKEGO_E2E_OWNER=local bun run --filter @likego/registry-etcd test:docker
```
...
门禁覆盖 register/get/list、replacement watch 的新增/更新/删除、丢失 transaction 响应的精确
