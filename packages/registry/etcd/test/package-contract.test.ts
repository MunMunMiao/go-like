import { readFile } from "node:fs/promises"
import { join } from "node:path"

import { expect, test } from "bun:test"

const Root = join(import.meta.dir, "..")

/** Reads one package JSON artifact without executing it. */
async function json(name: string): Promise<unknown> {
  return JSON.parse(await readFile(join(Root, name), "utf8"))
}

/** Reads one exact Markdown table row without formatter padding. */
function markdownRow(source: string, key: string): readonly string[] {
  const line = source.split("\n").find(function matching(candidate): boolean {
    return candidate.includes(key)
  })
  if (line === undefined || !line.startsWith("|") || !line.endsWith("|")) {
    throw new TypeError(`README table row is missing: ${key}`)
  }
  return Object.freeze(
    line
      .slice(1, -1)
      .split("|")
      .map(function cell(value): string {
        return value.trim()
      })
  )
}

test("package and manifests publish the resident Fetch-only etcd contract", async () => {
  expect(await json("package.json")).toMatchObject({
    name: "@likego/registry-etcd",
    version: "0.0.1",
    type: "module",
    module: "src/index.ts",
    typings: "src/index.ts",
    sideEffects: false,
    files: ["dist"],
    exports: { ".": "./src/index.ts" },
    scripts: {
      build: "bun x --bun tsdown --config-loader native",
      "test:docker": "bun test/integration/etcd-docker.ts",
      "test:runtime": "bun run build && bun test/integration/published-runtime.ts"
    },
    dependencies: {
      "@likego/context": "0.0.1",
      "@likego/core": "0.0.1",
      "@likego/registry": "0.0.1"
    }
  })
  expect(await json("capability.json")).toMatchObject({
    schemaVersion: 2,
    package: "@likego/registry-etcd",
    packageKind: "portable",
    exports: {
      ".": {
        kind: "portable",
        residency: "resident",
        ownerResources: ["etcd-registration", "etcd-watcher"],
        capabilities: ["registry", "registry-etcd", "service-discovery"]
      }
    }
  })
  expect(await json("owner.json")).toEqual({
    schemaVersion: 1,
    package: "@likego/registry-etcd",
    resources: [
      {
        id: "etcd-registration",
        owner: "likego-owned",
        exposure: "managed-private",
        stopContract: "likego-owned"
      },
      {
        id: "etcd-watcher",
        owner: "likego-owned",
        exposure: "managed-private",
        stopContract: "likego-owned"
      }
    ]
  })
  const readme = await readFile(join(Root, "README.md"), "utf8")
  expect(markdownRow(readme, "`etcd-fetch`")).toEqual([
    "`etcd-fetch`",
    "应用",
    "仅借用；本包不调用 `close`、`destroy` 或同类能力。"
  ])
  expect(markdownRow(readme, "`etcd-process`")).toEqual([
    "`etcd-process`",
    "应用/运维",
    "仅通过 HTTP 使用；本包不启动、停止或配置 etcd 进程。"
  ])
  expect(readme).toContain("newEtcdRegistry")
  expect(readme).toContain("replacement snapshot")
  expect(readme).not.toContain("newEtcdDiscovery")
  expect(readme).not.toContain("newEtcdRegistrar")
  expect(readme).not.toContain("hardDrainTimeoutMs")
  expect(readme).not.toContain("EtcdDrainTimeoutError")
  expect(readme).toContain(
    "gcr.io/etcd-development/etcd:v3.7.1@sha256:a9983dd6d9283138ab926daa307c6c25623636703ecf5645d5df4d666ce9eba2"
  )
})

test("package source inventory is explicit", async () => {
  const sources: string[] = []
  for await (const file of new Bun.Glob("src/**/*.ts").scan({ cwd: Root, onlyFiles: true })) {
    sources.push(file)
  }
  expect(sources.sort()).toEqual([
    "src/codec.ts",
    "src/discovery.ts",
    "src/errors.ts",
    "src/http.ts",
    "src/index.ts",
    "src/options.ts",
    "src/protocol.ts",
    "src/records.ts",
    "src/registration.ts",
    "src/runtime.ts",
    "src/types.ts"
  ])
})
