...
try {
  ...
  ensure(registerResult === undefined && updateResult === undefined, "register returned a value")
  ensure(JSON.stringify(discovered) === JSON.stringify([initial]), "discovery changed the service")
  ensure(
    /^li-[a-z2-7]{52}$/.test(remoteId) &&
      Object.keys(remoteAfter).length === 1 &&
      Object.keys(remoteAfter)[0] === remoteId,
    "Consul changed the deterministic remote ID"
  )
  ensure(deregisterResult === undefined, "deregister returned a value")
  ensure(initialSnapshot.length === 1, "watcher did not return the initial service")
  ensure(
    updatedSnapshot.length === 1 && JSON.stringify(updatedSnapshot[0]) === JSON.stringify(updated),
    "watcher did not replace the service snapshot"
  )
  ensure(emptySnapshot.length === 0, "watcher did not return an empty snapshot")
  ensure(watcherSurfaceExact, "watcher exposed an unexpected API")
  ensure(heartbeatPasses - heartbeatBaseline >= 2, "TTL heartbeat did not renew twice")
} catch (value) {
