...
async function auditPublisher(container: string, env: CommandEnvironment): Promise<JSONObject> {
  const script = [
    ...
    "let socketFDs=0;",
    "for(const name of fs.readdirSync('/proc/1/fd')){try{if(fs.readlinkSync('/proc/1/fd/'+name).startsWith('socket:['))socketFDs+=1}catch{}}",
    "function rows(file){let count=0;for(const line of fs.readFileSync(file,'utf8').split('\\n').slice(1)){const local=line.trim().split(/\\s+/,2)[1];if(local?.split(':').at(-1)?.toUpperCase()==='14E9')count+=1}return count}",
    "process.stdout.write(JSON.stringify({socketFDs,udp4Rows:rows('/proc/net/udp'),udp6Rows:rows('/proc/net/udp6')}));"
  ].join("")
  const result = await command(["docker", "exec", container, "node", "-e", script], env)
...
/** Executes all release-blocking real mDNS phases. */
async function main(): Promise<void> {
  ...
  const result = Object.freeze({
    valid: true,
    nodeRuntime: "Node.js 24.18.0",
    images: Object.freeze({ node: NodeImage, netshoot: NetshootImage }),
    ...
    scenarioEvidence: Object.freeze({
      "register-discover": Object.freeze({
        ipv4Created: ipv4.observer.created,
        ipv6Created: ipv6.observer.created,
        ipv4CompletePayload: ipv4.observer.completePayload,
        ipv6CompletePayload: ipv6.observer.completePayload,
        ipv4DomainIsolated: ipv4.observer.domainIsolated,
        ipv6DomainIsolated: ipv6.observer.domainIsolated,
        ipv6AdvertisedEndpoints,
        ipv6PacketSourceAddresses: ipv6.packets.sourceAddresses,
        ipv6IdentityLifecycle,
        ipv6AdvertisedULAObserved: ipv6Alias.advertisedULAObserved,
        ipv6PacketLinkLocalObserved: ipv6Alias.packetLinkLocalObserved,
        ipv6SingleIdentityLifecycleObserved: ipv6Alias.singleIdentityLifecycleObserved,
        ipv6ULAtoLinkLocalAliasObserved: ipv6Alias.aliasObserved
      }),
      "watch-update-delete": Object.freeze({
        ipv4Updated: ipv4.observer.updated,
        ipv4Restored: ipv4.observer.restored,
        ipv4Deleted: ipv4.observer.deleted,
        ipv6Updated: ipv6.observer.updated,
        ipv6Restored: ipv6.observer.restored,
        ipv6Deleted: ipv6.observer.deleted
      }),
      "crash-expiry": Object.freeze({
        createObserved: crash.observer.createObserved,
        expiryDeleteObserved: crash.observer.expiryDeleteObserved,
        publisherExitCode: crash.publisherExitCode,
        killWithoutGoodbye: crash.killWithoutGoodbye,
        recordTTLValues: crash.packets.recordTTLValues,
        recordTTL2: crash.packets.recordTTLValues.includes(2)
      }),
      "collision-rescue": Object.freeze({
        ipv4CollisionCode: ipv4.collision.code,
        ipv6CollisionCode: ipv6.collision.code,
        ipv4Rescued: ipv4.observer.rescued,
        ipv6Rescued: ipv6.observer.rescued
      }),
      "wire-cleanup": Object.freeze({
        ipv4PacketValid: ipv4.packets.valid,
        ipv6PacketValid: ipv6.packets.valid,
        ipv4IPTTL255: ipv4.packets.ipTTLValues.length === 1 && ipv4.packets.ipTTLValues[0] === 255,
        ipv6IPTTL255: ipv6.packets.ipTTLValues.length === 1 && ipv6.packets.ipTTLValues[0] === 255,
        ipv4RecordTTL120And0:
          ipv4.packets.recordTTLValues.includes(120) && ipv4.packets.recordTTLValues.includes(0),
        ipv6RecordTTL120And0:
          ipv6.packets.recordTTLValues.includes(120) && ipv6.packets.recordTTLValues.includes(0),
        ipv4CompleteRRGraph:
          ipv4.packets.completeGraphs.ipv4.positiveTTL120 &&
          ipv4.packets.completeGraphs.ipv4.goodbyeTTL0,
        ipv6CompleteRRGraph:
          ipv6.packets.completeGraphs.ipv6.positiveTTL120 &&
          ipv6.packets.completeGraphs.ipv6.goodbyeTTL0,
        ipv4CacheFlushClassificationValid: ipv4.packets.cacheFlushCounts.ipv4.invalid === 0,
        ipv6CacheFlushClassificationValid: ipv6.packets.cacheFlushCounts.ipv6.invalid === 0,
        ipv4StoppedObserverNoReceive: ipv4.observer.stoppedObserverNoReceive,
        ipv6StoppedObserverNoReceive: ipv6.observer.stoppedObserverNoReceive
      })
    }),
    ipv4,
    crash,
    ipv6: Object.freeze({ supported: true, evidence: ipv6 }),
    cleanup: Object.freeze({
      projectsRemoved: true,
      containersRemoved: true,
      networksRemoved: true,
      processTreesRemoved: true,
      protectedContainersUnchanged: true
    }),
    protectedContainers: protectedAfter
  })
  process.stdout.write(`LIKEGO_REGISTRY_MDNS_E2E_RESULT=${JSON.stringify(result)}\n`)
}
