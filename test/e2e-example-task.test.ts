...
test("keeps a scenario failure primary when resource publication remains incomplete", async () => {
  if (!Posix) return
  const selected = await fixture()
  const spawned = spawnHarness()
  const capability = Object.freeze({
  ...
  const scenarioFailure = new Error("scenario settlement failed")
  const firstSettlement = Promise.withResolvers<ExampleTaskChildResult>()
  let settlementReads = 0
  try {
    const task = runExampleTask(selected.frame, selected.cwd, {
      ...baseOptions(selected, spawned),
      assertRootIdentity: async () => rootIdentity(capability),
      spawn(argv, options): ExampleTaskChild {
        spawned.records.push(Object.freeze({ argv: Object.freeze(argv.slice()), options }))
        return Object.freeze({
          stdout: spawned.child.stdout,
          stderr: spawned.child.stderr,
          get settled(): Promise<ExampleTaskChildResult> {
            settlementReads += 1
            return settlementReads === 1 ? firstSettlement.promise : Promise.resolve(childResult())
          },
          kill: spawned.child.kill
        })
      }
    })
    const requestId = await registrationRequest(selected)
    ...
    await waitForFixture("scenario spawn", () => spawned.records.length > 0)
    firstSettlement.reject(scenarioFailure)
    ...
    expect(result.cleanupFailures.map((failure) => failure.code)).toEqual([
      "example-worker-failed",
      "resource-event-invalid"
    ])
    expect(result.cleanupFailures[0]?.summary).toBe("Error: scenario settlement failed")
  } finally {
    await unlink(temporaryPath).catch(() => {})
    spawned.settle()
    await cleanup(selected)
