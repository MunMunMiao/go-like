...
LikeGo 的公共 API、首次成功路径、包导航和 examples 应尽量复用 go-micro、go-kratos 与
...
| Scaffold | `bunx @likego/create <directory>` 生成职责拆分、带独立 package manifest、安装后可测试并运行的内部 unary 微服务；只使用既有 Core/Server/Transport 公共模型，不引入第二套 runner、远程模板、交互 prompt、Proto 或 Gateway。                                                                                                                                                                                                         |
