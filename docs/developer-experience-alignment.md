...
- Hono、Elysia 与 H3 2.x 直接把 `app.fetch` 交给 `@likego/web`，H3 1.x 使用官方 `toWebHandler(app)`；LikeGo 不发布只做转发的框架 wrapper。
...
| Web/框架 | 外部接入保持标准单参数 `Handler`；Hono、Elysia 与 H3 2.x 直接提供 `app.fetch`，H3 1.x 使用官方 `toWebHandler(app)`，不需要 LikeGo 框架桥接包。                                                                                                                                                                                                                                                                                    |
