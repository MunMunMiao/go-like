...
本轮不继续扩充 LikeGo 的能力面，而是检查新增实现是否存在运行时死锁、未处理 Promise、验证假绿、发布边界和公开文档失真。所有修改必须由当前代码或真实命令复现支撑，并维持既定产品边界：标准 Web API、结构式 Server、外部 Web 与内部 Transport 分离、无 gRPC/Proto、无 Event Store/replay、Registry provider 不再扩张。

## 2. 团队调查与主线复核

三条独立调查线分别覆盖核心语义、发布与运行门禁、Examples 与开发者体验。主线重新执行最小复现后确认：

1. `@likego/core` 在 Server Promise 创建后、安装失败监听前存在窗口。Server 立即拒绝且 `afterStart` 延迟时，会产生一次 `unhandledRejection`。
2. `@likego/web/node` 在 `start()` 后同一事件循环调用 `stop()` 时，stop 可结束，但 start Promise 永久 pending。
3. `bun audit` 报告 2 个 high、3 个 moderate 开发链漏洞；`bun audit --production` 通过。受影响链路为 Ajv 的 `fast-uri` 和 VitePress 的 Vite/esbuild，不进入 LikeGo 发布包运行时。
4. `@likego/core`、`@likego/web`、`@likego/create` 尚未发布到 npm，而根 README 与站点仍把 registry 安装命令写成当前可执行路径。
...
7. 多语言 package reference 缺少 `@likego/create`，部分 locale 还漏列已经发布规划内的 provider；example portfolio 有一处 tier 与 `catalog.json` 不一致。
...
- 根 README、默认与简体中文 Getting Started、`@likego/create` README 明确当前 npm 未发布，并提供仓库内验证命令；发布完成后才能移除提示。
...
完整 verify 已包含真实 Docker suite。最终还要按 LikeGo owner label 回读 container、network、volume，确认本轮 owner 资源零残留。任何命令未运行、失败或仍在运行时，不宣称完成。
