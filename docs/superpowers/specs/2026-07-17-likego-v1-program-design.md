...
5. 第三方业务 API native-first。LikeGo 不逐方法镜像 Croner、Pino、Winston、Web 框架、NATS、BullMQ 或
   OpenTelemetry，也不复制它们的配置、调度、路由、日志和诊断语义。
6. adapter 只把第三方对象的原生启动、停止和终态映射为 `Server`/`ServerHandle`。应用继续通过第三方
   原生对象使用全部业务能力；需要完整自定义时，用户只需实现 structural Server，无需继承 LikeGo 类型。
...
此外，标准 Web API 无法让被冻结或挂起的宿主在暂停期间执行 timer；LikeGo 只对发布门禁覆盖的持续运行
...
`30_000ms` 局部预算，实际使用较早 deadline；具备安全公开 force 的 LikeGo-owned resident adapter 在
...
LikeGo 不提供 router。Vanilla 传函数，Hono、Elysia、H3 都传各自的 `app.fetch`。
...
首个 managed Node host 使用 `@hono/node-server` 承接标准 Fetch ABI。LikeGo 不实现
...
`@likego/resilience` 因此是 v1 release-blocking portable package，而不是未来 provider adapter。
...
- NATS JetStream：应用创建原生 `ConsumerMessages`，并负责迭代、ack、MaxDeliver、重投与 DLQ；adapter 只
  承接 close/stop/closed。PubAck、ack pending 和 reconnect 可作为官方原生集成证据，但不冒充 LikeGo
  实现的数据面能力。
...
- Pino：应用使用官方 API 创建并持有 raw Logger 与 destination，并保留完整 Pino 配置/transport 能力；
  `start(ctx)` 接纳后把 destination 生命周期显式移交给 LikeGo。adapter 只映射 logger flush 与 destination
...
- OpenTelemetry：业务通过官方 API 创建并配置 provider、processor、reader 与 exporter，并继续使用原生
  Tracer/Meter；LikeGo 只承接 provider shutdown。Collector down 不阻塞业务，恢复与 flush 语义由官方
...
提升为 v1 portable package；同日用户进一步批准“第三方库拥有能力、LikeGo 只适配 Server 生命周期”的
