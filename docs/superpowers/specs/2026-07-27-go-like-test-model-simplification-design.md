...
本轮把 go-like 收敛为两种测试：单元测试和 E2E。格式检查、类型检查、构建、安全审计与文档构建继续作为工程
命令存在，但不伪装成第三种测试。
...
- `@go-like/create` 通过 TypeScript 模板生成新的 TypeScript 项目，属于当前产品不需要的 scaffold 能力；
...
两个仓库中均没有 go-like 当前的 `source-policy`、`package-contract`、`coverage-contract`、`capability.json`、
`owner.json` 文件体系。上游存在的 Proto 或 CLI generator 不构成 go-like 必须实现 generator 的理由；go-like 已明确不支持
...
删除 private `@go-like/testing` workspace。它只有一个真实消费者，listener 生命周期断言直接收回对应的
`@go-like/transport-http` 单元测试；不再用一个独立 package 测试测试工具本身。
...
删除整个 `@go-like/create` package，以及 README、站点、release notes、workspace references、Changesets 和 published
E2E 中对它的引用。go-like v1 不提供 scaffold、模板引擎或源代码生成能力。
...
1. 删除自证体系和 `@go-like/create`，清理 workspace、依赖和文档引用；
...
4. 当前 package/workspace 和用户文档不再存在 `@go-like/create` 或任何生成项目/测试源码的实现；
...
完成本设计只能证明 go-like 回到清晰、可维护的验证模型。生产级结论仍取决于公共 API 正确性、真实 provider E2E、
