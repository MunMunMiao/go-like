...
本文是 LikeGo E2E 实施与验收的规范性来源。调研事实、上游证据和曾评估的宿主机 containment 方案保留在 [`2026-07-29-likego-e2e-evidence-architecture.md`](./2026-07-29-likego-e2e-evidence-architecture.md)，仅作为历史研究。

## 1. 支持边界

LikeGo 是 runtime-neutral TypeScript toolkit：
...
LikeGo 不承诺或要求操作系统级进程沙箱、fork-complete containment 或跨平台相同的宿主机隔离语义。这些能力不是 PR、release、tag 或平台兼容性的门禁。
...
- 不把 scope、目录或脚本名称包装成新的测试等级；LikeGo 只有 unit 与 E2E。
...
Docker 使用 exact `io.likego.e2e.owner` 与 invocation ownership。Scenario 正常清理自己的 container、network 和 volume；root backstop 只处理当前 invocation 与已注册 child owner 的交集。Foreign、unknown-owner 或 label collision 资源保持 untouched，并使测试失败。
...
每条命令必须等待结束并记录 exit code；只报告实际环境、动态数量和本次观察结果。未运行宿主机 containment 实验不影响 LikeGo 平台支持与 release readiness，因为它们不属于产品合同。
...
上述结果不构成 60 分钟 soak 或宿主机 containment 声明。LikeGo 的实现状态与 release 不受已撤销的宿主机门禁阻塞。
