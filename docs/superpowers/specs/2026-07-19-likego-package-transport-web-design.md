# LikeGo 包拓扑、Transport 与 Web 边界设计

日期：2026-07-19

状态：部分被后续决策取代；保留为历史实施依据

> 更新（2026-08-03）：本文关于 `@likego/hono`、`@likego/h3`、`@likego/elysia` 的设计已被原生 Fetch 边界取代。Hono、Elysia 与 H3 2.x 直接提供 `app.fetch`，H3 1.x 使用官方 `toWebHandler(app)`；当前边界见 [ADR 0003](../../adr/0003-resident-adapter-ownership.md)。

## 1. 文档效力

本文档收敛 LikeGo v1 的包拓扑，以及内部微服务传输、注册发现与外部 Web 服务的边界。它覆盖并替代现有
设计中关于 `adapters/`、`@likego/fetch`、Fetch 作为 transport、框架适配器位置、transport 缺失和 Consul
...
2. 单一能力域的供应商或框架包使用短包名，例如 `@likego/hono`、`@likego/pino`；当实现必须归属公共
   能力域并与父契约形成对称结构时，使用 `@likego/<domain>-<implementation>`，例如
   `@likego/transport-http`、`@likego/registry-mdns` 和 `@likego/registry-consul`。
3. `@likego/transport` 只定义内部微服务同步通信的公共契约，不依赖任何具体传输实现。
4. HTTP 传输实现名为 `@likego/transport-http`，物理路径固定为 `packages/transport/http`。它同时提供
   client 与 server：`dial` 是 client 入口，`listen/accept` 是底层 server 入口，`newHTTPServer` 是可交给
   Core 编排的生命周期入口。不存在 `@likego/http`。
5. 对外 HTTP/Web 服务属于 `@likego/web`，物理路径为 `packages/web`。Web 框架不进入 transport 域。
6. 删除 `@likego/fetch`。标准 `Request -> Response` handler 与 request-to-Context 桥接进入
   `@likego/web`；Fetch 在 `@likego/transport-http` 中仅作为内部 HTTP wire 的可移植执行能力。
7. Hono、H3、Elysia 等产品包在 `packages/` 下平铺，不放入 `packages/web/*`。
8. 第三方能力继续遵守 native-first：LikeGo 只提供公共契约、必要的机械适配和 Server 生命周期，不复制
...
10. Registry 与 transport 使用相同的“公共契约父包 + 实现子 workspace”结构：
    ...
    `packages/config/consul`，不再用一个 `@likego/consul` 聚合两个能力域。
11. mDNS 是第一版 Registry 实现，并使用 LikeGo 自有 wire namespace；标准 Web API 没有 UDP multicast，
...
未上 wire 等行为；LikeGo 对齐公共角色，不把这些缺陷升级成自身契约。
...
其 Server 实现公共 transport lifecycle。LikeGo 因此同时保留 go-micro 的 `dial/listen/accept` 低层形状与
...
公共能力由 `@likego/transport`、`@likego/web`、`@likego/registry` 等包定义。框架包保持 `packages/` 平铺；
...
### 4.3 不采用：单一 `@likego/core` 或 `@likego/micro` 总管门面
...
| --- | --- | --- |
| `@likego/context` | `.` | 保留 |
| `@likego/core` | `.`, `./lifecycle` | 保留 App、Server、ServerHandle 与现有 Context wait helper；不聚合 transport 或 Web |
| `@likego/config` | `.`, `./env`, `./file` | 合并 `@likego/config-env` 与 `@likego/config-file` |
| `@likego/config-consul` | `.` | 从 `@likego/config-consul` 迁入 `packages/config/consul`；继续使用标准 Fetch |
| `@likego/health` | `.` | 只保留 probe 域；HTTP handler 迁到 `@likego/web/health` |
| `@likego/registry` | `.`, `./testing` | 对齐 Registry/Service/Node/Endpoint/Value/Watcher，并保留便利层与 provider conformance |
| `@likego/registry-consul` | `.` | 从 `@likego/registry-consul` 迁入 `packages/registry/consul`；继续使用标准 Fetch |
| `@likego/registry-mdns` | `.`, `./node`, `./testing` | 新增 portable mDNS provider、host SPI、首个真实 Node UDP multicast host |
| `@likego/resilience` | `.` | 保留 retry、backoff、breaker、limiter |
| `@likego/testing` | `.`, `./server`, `./listener` | 保留 Server 契约并新增仅供测试使用的 listener conformance |
| `@likego/transport` | `.`, `./headers`, `./testing` | 新增内部通信 SPI、Likego headers 与 transport conformance suite |
| `@likego/transport-http` | `.`, `./node`, `./testing` | 新增 Fetch unary client、底层 listener server、Core lifecycle server 与首个 Node host |
| `@likego/web` | `.`, `./health`, `./node`, `./node/testing` | 吸收 `@likego/fetch` 与 `@likego/fetch-node` 的正确职责 |
| `@likego/hono` | `.` | Hono native app 到 `@likego/web` Handler 的薄接缝 |
| `@likego/h3` | `.` | H3 native app 到 `@likego/web` Handler 的薄接缝 |
| `@likego/elysia` | `.` | Elysia native app 到 `@likego/web` Handler 的薄接缝 |
| `@likego/croner` | `.` | 重命名 `@likego/cron-croner`，继续只适配 Cron 生命周期 |
| `@likego/bullmq` | `.`, `./testing` | 重命名 `@likego/job-bullmq-node` |
| `@likego/nats` | `.`, `./jetstream` | 合并两个 NATS 包；根入口是 core transport，子路径保留 JetStream |
| `@likego/pino` | `.` | 重命名 `@likego/log-pino-node` |
| `@likego/winston` | `.` | 重命名 `@likego/log-winston-node` |
| `@likego/otel` | `.`, `./testing` | 重命名 `@likego/otel-node` |
| `@likego/prometheus` | `.` | 重命名 `@likego/metrics-prom-client-node`，handler 类型改用 `@likego/web` |

`@likego/consule` 是拼写错误，不创建该包；`@likego/consul` 聚合包也不创建。所有旧 adapter 包名、
`@likego/fetch`、`@likego/fetch-node`、候选名 `@likego/http` 和 `adapters/` 目录在迁移完成后均不存在。

四个 private workspace 的最终 identity 同样固定，不使用过渡名：

| 目录 | package name |
| --- | --- |
| `examples/vanilla-web` | `@likego/example-vanilla-web` |
| `examples/hono` | `@likego/example-hono` |
| `examples/h3` | `@likego/example-h3` |
| `examples/elysia` | `@likego/example-elysia` |
...
| --- | --- | --- |
| `@likego/context` | 无 | 无 |
| `@likego/core` | context | 无 |
| `@likego/config` | context、core | `@standard-schema/spec` |
| `@likego/config-consul` | config、context、core | 无；Fetch 由应用注入 |
| `@likego/health` | context、core | 无 |
| `@likego/registry` | context、core | 无 |
| `@likego/registry-consul` | registry、context、core | 无；Fetch 由应用注入 |
| `@likego/registry-mdns` | registry、context、core | portable codec 不依赖 Node SDK |
| `@likego/resilience` | context | 无 |
| `@likego/testing` | context、core | 无；只能作为 devDependency 使用 |
| `@likego/transport` | context | 无 |
| `@likego/transport-http` | transport、context、core | Node host 依赖在同包条件子路径隔离 |
| `@likego/web` | context、core、health | `@hono/node-server` 只由 `./node` 使用 |
| `@likego/hono` | web | `hono` peer |
| `@likego/h3` | web | `h3` peer |
| `@likego/elysia` | web | `elysia` peer |
| `@likego/croner` | context、core | `croner` |
| `@likego/bullmq` | context、core | `bullmq` |
| `@likego/nats` | context、core | `@nats-io/transport-node`、`@nats-io/jetstream` |
| `@likego/pino` | context、core | `pino` production；Pino 自行拥有其 `sonic-boom` implementation dependency |
| `@likego/winston` | context、core | `winston` |
| `@likego/otel` | context、core | `@opentelemetry/api`、`@opentelemetry/sdk-metrics`、`@opentelemetry/sdk-trace` |
| `@likego/prometheus` | web | `prom-client` |

`@likego/testing/listener` 只通过 devDependency 进入 transport-http 与 Web 的测试图，绝不成为两者的 production
依赖。`@likego/web/node`、`@likego/web/health` 等子路径已经计入所属 workspace 的依赖行，不是额外包。

`@likego/pino` 不得把 `sonic-boom` 声明为 production、dev 或 peer direct dependency，也不得 import 其实现。
Pino `10.3.1` 自己声明 `sonic-boom ^4.0.1`，该范围及实际解析版本由 Pino 的 dependency graph 管理；LikeGo
...
`@likego/pino` 没有隐藏的 direct/peer coupling。
...
1. `@likego/transport` 不依赖 `@likego/transport-http`、Web、core、registry 或供应商包。
2. `@likego/transport-http` 不依赖 `@likego/web`，也不承接 Web router、middleware、健康页或对外静态资源。
3. `@likego/web` 不依赖 transport；它可以独立托管普通 Web 应用。
4. `@likego/core` 继续只管理结构式 Server 生命周期，不知道 transport、Web 或供应商类型。
5. Framework package 只依赖 `@likego/web` 和对应 peer framework，不重导出框架、不代理路由 API。
6. `@likego/health` 不依赖 Web；`@likego/web/health` 依赖 `@likego/health`。
7. `@likego/web` 与 `@likego/transport-http` 的根入口保持 portable；其 Node-only 代码只能从明确的
   `./node` 子路径进入依赖图。其他 runtime/vendor 包的根入口按真实能力声明，不能伪装 portable。
8. `@likego/registry` 不依赖任何 provider；`@likego/registry-mdns` 与 `@likego/registry-consul` 都只依赖
   公共 Registry 契约，不互相依赖。
9. `@likego/registry-mdns` 根入口不静态 import `node:`；UDP socket、multicast membership 与网卡枚举只从
   明确的 runtime host 进入。

## 7. `@likego/transport` 公共契约

### 7.1 公共类型

公共角色与 go-micro 一一对应：阻塞调用变为 Promise，`error` 变为 throw/rejection，`[]byte` 变为
`Uint8Array`，输出参数变为返回值。语言映射之外，LikeGo 还明确强化资源所有权、终态、defensive copy 与
...
`insecureSkipVerify`。LikeGo Context 不放入 Options，因为它已经是独立首参；实现私有配置由具体包的 typed
...
`defaultTransport`：否则 `@likego/transport` 必须反向依赖 `@likego/transport-http` 并形成包环。应用在
...
`@likego/transport/headers` 保留 go-micro 当前的常量角色，但使用 LikeGo 自有 namespace，导出
lowerCamelCase 值。所有项目自有 header 固定使用用户确认的 `Likego-` 前缀，不发布旧前缀 alias；标准
...
| 错误 | code | 用途 |
| --- | --- | --- |
| `TransportClosedError` | `LIKEGO_TRANSPORT_CLOSED` | 已关闭的 socket/listener 上继续操作 |
| `TransportStateError` | `LIKEGO_TRANSPORT_STATE` | recv-before-send、重复 accept 等非法状态 |
| `UnsupportedTransportCapabilityError` | `LIKEGO_TRANSPORT_UNSUPPORTED_CAPABILITY` | implementation 无法兑现已请求能力 |
| `TransportProtocolError` | `LIKEGO_TRANSPORT_PROTOCOL` | wire 格式、响应或协议约束无效 |

Context 取消继续使用 `@likego/context` 的 `canceled` 或 `deadlineExceeded`，不重写成 transport 错误。

## 8. `@likego/transport-http`
...
- 通过 `newHTTPServer` 把 listener/accept 组合为可由 `@likego/core` 编排的结构式 Server；
...
```ts
import type { Context } from "@likego/context"
import type { Server, ServerHandle } from "@likego/core"
import type {
  AcceptHandler,
  Listener,
  TLSConfig,
...
export interface HTTPServerDrainTimeoutError extends Error {
  ...
  readonly code: "LIKEGO_HTTP_SERVER_DRAIN_TIMEOUT"
...
```ts
import type { ListenOptions } from "@likego/transport"
...
| `newHTTPServer(...).start(...)` | 应用可组合 lifecycle Server | go-kratos HTTP Server、LikeGo Core Server |
...
`HTTPHandler` 接收带连接 metadata 的 host envelope，`@likego/web` Handler 只接收标准 Request；两者既不
...
`http2BufferSizeBytes` 为 `null`，不得把 transport 默认常量误判为调用方能力请求。`@likego/transport-http/node`
...
`@likego/transport-http/node` 与 `@likego/web/node` 独立实现各自的 owner state machine，不跨包共享源码或
互相依赖：前者返回 transport `HTTPHostHandle`，后者返回 Core `ServerHandle`，终态和所有权并不相同。两者
只在测试图通过 `@likego/testing/listener` 共享 bind、address、close、terminal、force 和端口释放 conformance。
...
未来若增加 raw Node/Bun/Deno host，它可以在同一个 `@likego/transport-http` 包内声明对应 capability 并通过
相同公共 conformance suite；在真实全双工 E2E 通过前，v1 不发布 stream 支持声明。

## 9. `@likego/registry`、mDNS 与 Consul
...
`@likego/registry` 对齐 go-micro 当前 Registry 的公共角色，同时采用 LikeGo 的显式资源所有权：Context
...
`["likego.service-content.v1", name, version, sortedServiceMetadata, canonicalEndpoints]`，其中 metadata 转成按
...
`["likego.identity-content.v1", name, version, sortedServiceMetadata, canonicalEndpoints,
[node.id, addresses, sortedNodeMetadata]]`；addresses 保持声明顺序，其余 canonical 子结构完全复用上一段定义。
两个 content hash 都在压缩前计算，分别写入 `Likego-Service-Content-Hash` 与
`Likego-Identity-Content-Hash`。`Likego-Identity-Hash` 的值精确等于
`li-` + lowercase/no-padding Base32(SHA-256(UTF8(JSON([name,version,node.id]))))；Consul
`Likego-Registration-Token` 的值包含完整 `lr-` 前缀与 256-bit token Base32，不存裸 token 或另作编码。
...
`LIKEGO_*`。未找到服务返回空数组，不把正常空结果重写成异常。

### 9.2 Provider 拓扑与一致性

```text
packages/registry/          @likego/registry
packages/registry/mdns/     @likego/registry-mdns
packages/registry/consul/   @likego/registry-consul
```
...
`@likego/registry-consul` 迁移现有标准 Fetch 实现，不引入 Node-only Consul SDK。其 TTL health、watch、
崩溃过期和 deregister 必须继续使用真实 Consul Docker 容器验证。`@likego/config-consul` 独立归属配置域；
两者可以共享协议事实和测试容器，但不能通过一个 `@likego/consul` 根包重新耦合。

### 9.3 `@likego/registry-mdns` 公共面与 host SPI
...
clock/cache 或 watcher diff，这些协议行为只在 portable provider 实现一次。`@likego/registry-mdns/node` 使用
...
| helper | 默认 | 校验与语义 |
| --- | --- | --- |
| `domain(value)` | `likego` | 一个或多个 DNS-safe label；规范化为小写 FQDN，并对所有最终 owner/target 执行 DNS wire 长度校验 |
...
LikeGo 参考 go-micro 的节点级 PTR/SRV/TXT/A/AAAA 角色，但不宣称与 go-micro 默认 `.micro` wire 兼容：

1. 默认 domain 为 `likego.`，所有自有 TXT key 使用 `Likego-` 前缀；wire 包含
   `Likego-Wire-Version=1`，不读写旧 namespace alias。
2. canonical identity 是 UTF-8 JSON tuple `[service.name, service.version, node.id]`。`serviceLabel` 为
   ...
   `["likego.host.v1", [service.name, service.version, node.id], sortedAddresses]`，addresses 按 ASCII code point
...
5. 每节点 TXT payload 是 canonical UTF-8 JSON：对象字段顺序固定，metadata key 以 Unicode code point 排序，
   ...
   `Likego-Wire-Version`、`Likego-Encoding`、`Likego-Service-Content-Hash`、
   `Likego-Identity-Content-Hash`、`Likego-Chunk-Count` 与连续零填充
   `Likego-Chunk-NNN`；缺段、重复、unknown encoding、hash 不符或解压超限都报 protocol error。
...
9. get/list 的 query socket 在 timeout/Context/成功后关闭；watchers 可引用计数共享 listener，最后 watcher
   ...
   与查询无关的 DNS packet、未知 Likego wire version、malformed/foreign packet 只做 rate-limited secret-safe log
...
`@likego/registry-consul` 导出 `newConsulRegistry(options)`；provider options 精确沿用已验证实现：必填 borrowed
...
2. Agent Service 添加精确 marker tag `Likego-Wire-Version=1`，供 catalog list 排除非 LikeGo service；tag 不承载
   payload。Meta 按 key 字典序写入 `Likego-Wire-Version=1`、`Likego-Encoding=deflate+base64url`、
   `Likego-Identity-Hash`、`Likego-Registration-Token`、`Likego-Service-Content-Hash`、
   `Likego-Identity-Content-Hash`、`Likego-Chunk-Count` 和连续零填充 `Likego-Chunk-NNN`。payload 使用与
...
- 真实 mDNS 容器必须抓取 packet，断言 IP multicast TTL 255、RR owner/target、Likego TXT、DNS RR
...
`@likego/web/node` 吸收原 `@likego/fetch-node`：
...
它继续结构式实现 `@likego/core` 的 `Server<NodeServerHandle>`，并只负责 listen accepted、实际地址、稳定
`done()`、graceful drain、hard force 与真实 terminal；协议转换由 `@hono/node-server` 提供。其他 runtime
可以自行实现接收 Handler 的结构式 Server，LikeGo 不要求继承或 decorator。
...
`@likego/health` 仅保留 probe registry 和检查语义。`HealthFetchOptions` 与 `createHealthFetch` 不发布 alias。
`@likego/prometheus` 返回 `@likego/web` 的 Handler，不依赖 Node 子路径。

### 10.3 框架接缝

`@likego/hono`、`@likego/h3`、`@likego/elysia` 各自只把 native app 变成稳定绑定的 Handler：
...
而不是只检查数量；还需证明父包 `@likego/transport`、`@likego/config`、`@likego/registry` 的 tarball 均不包含
...
7. Runtime lane 按包精确固定：`@likego/transport-http` 的 `.`、`./testing` 在 Bun、Node、Deno lanes
   验证，`./node` 只进入 Node lanes；`@likego/web` 的 `.`、`./health` 在 Bun、Node、Deno lanes 验证，
   `./node`、`./node/testing` 只进入 Node lanes。portable 子路径不得因同包存在 Node 子路径而退化为
   Node-only，Node 子路径也不得进入 Bun/Deno lane。`@likego/registry-mdns` 的 `.`、`./testing` 在 Bun、
   Node、Deno lanes 验证 portable graph，`./node` 只进入 Node lanes；这只证明 portable API/codec 可加载，
   不代表 Bun/Deno 已具备 mDNS host。`@likego/registry-consul` 与 `@likego/config-consul` 根入口使用标准
...
| --- | --- | --- | --- | --- |
| `@likego/registry` `.` | portable | resident | `service-registration`、`discovery-watcher` | 返回的 RegistrationHandle/DiscoveryWatcher `done()` |
| `@likego/registry` `./testing` | portable | non-resident | 空 | not-applicable |
| `@likego/registry-consul` `.` | portable | resident | `consul-registration`、`consul-watcher` | RegistrationHandle/Watcher `done()` |
| `@likego/registry-mdns` `.` | portable | resident | `mdns-registration`、`mdns-watcher` | RegistrationHandle/Watcher `done()` |
| `@likego/registry-mdns` `./node` | integration | resident | `node-mdns-datagram` | MDNSDatagramSocket `done()` |
| `@likego/registry-mdns` `./testing` | portable | non-resident | 空 | not-applicable |
...
真实 terminal 测试。borrowed Fetch、HTTP Consul process、MDNSHost factory 不计入 LikeGo-owned resource。

### 11.3 Transport 与 Web residency 清单

| package/export | kind | residency | ownerResources | terminal evidence |
| --- | --- | --- | --- | --- |
| `@likego/transport` `.` | portable | non-resident | 空 | not-applicable |
| `@likego/transport` `./headers` | portable | non-resident | 空 | not-applicable |
| `@likego/transport` `./testing` | portable | non-resident | 空 | not-applicable |
| `@likego/transport-http` `.` | portable | resident | `http-client`、`http-listener`、`http-server` | Client `close()`、Listener `accept()/close()`、ServerHandle `done()` |
| `@likego/transport-http` `./node` | integration | resident | `http-server`、`node-http-host` | ServerHandle 与 HTTPHostHandle `done()` |
| `@likego/transport-http` `./testing` | portable | non-resident | 空 | not-applicable |
| `@likego/web` `.` | portable | non-resident | 空 | not-applicable |
| `@likego/web` `./health` | portable | non-resident | 空 | not-applicable |
| `@likego/web` `./node` | integration | resident | `node-server` | NodeServerHandle `done()` |
| `@likego/web` `./node/testing` | integration | non-resident | 空 | not-applicable |

`@likego/transport-http/owner.json` 精确登记 `http-client`、`http-listener`、`http-server`、
`node-http-host` 四项；每项都使用 `owner:"likego-owned"`、`exposure:"managed-private"` 和
`stopContract:"likego-owned"`。`@likego/web` 只登记 `node-server`，使用相同三个 ownership 值。
borrowed executor、HTTPHost factory、注入的 HTTPTransport、handler 与 `@hono/node-server` factory 不登记为
LikeGo-owned resource。
...
4. 以测试先行新增 `@likego/transport` 契约、headers、错误与 conformance suite，先锁定 transport address 与
   metadata 角色，供 Registry resolver 显式消费。
5. 以测试先行实现 `@likego/transport-http` unary Fetch client、低层 listener server、Core lifecycle server、
   host SPI 和真实 Node host；不提前声明 stream。
6. 以测试先行扩展 `@likego/registry` 公共契约、owner handles、显式 ServiceInstanceResolver 与 conformance。
7. 迁移 Consul，随后实现 mDNS portable codec/provider、Node host、packet capture 与真实双容器测试。
8. 把 Fetch handler/Context bridge 移入 `@likego/web`，把 Node host 移入 `@likego/web/node`，迁移 health 和
   Prometheus handler，删除 `@likego/fetch` 与 `@likego/fetch-node`。
...
- 最终扫描不得存在可执行配置或源码对旧包名、`adapters/`、`@likego/fetch`、`@likego/http` 的引用。
...
2. `@likego/transport` 公共角色与 go-micro 的 Transport、Message、Socket、Client、Listener 和三组 options
   一一对应，并通过结构式 conformance suite。
3. `@likego/transport-http` 真实完成 unary Fetch client、低层 listener server 和 Core lifecycle server E2E；
   未支持的 raw/stream 能力显式失败。
4. 对外入站 Web application hosting 只通过 `@likego/web`；Consul/transport 的标准 Fetch client 不属于入站
   Web hosting。transport-http 不包含 Web 框架职责，两者无循环依赖。
5. 任意用户自实现 Server 或 runtime Web Server 可以结构式接入 Core；任意 HTTPHost 可以结构式注入
   `@likego/transport-http`。HTTPHost 本身不是 Core Server；同包 `newHTTPServer` 负责把
   `listen/accept/close` 正确组合成 Core Server，但应用仍可自行实现等价结构式 Server。
6. `@likego/registry` 完整覆盖 Registry/Service/Node/Endpoint/Value/Watcher 与 owning handles；mDNS 与
   Consul 均通过 capability-parameterized 公共 conformance，mDNS 通过真实双容器 multicast、Consul 通过
   真实 Consul 容器。
7. mDNS wire 只使用 LikeGo namespace，TTL、crash expiry、update、watch overflow 和 socket cleanup 均有
