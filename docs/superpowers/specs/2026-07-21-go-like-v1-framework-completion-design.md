...
本文定义 go-like v1 从“已验证的生命周期与适配器集合”补齐为可组合微服务工具包的实施边界。用户已经批准
...
- `docs/adr/0004-service-registry-and-selection.md` 中对 etcd、Kubernetes、ZooKeeper、Store、Broker、
  selector 策略和内部路由的延期；
- `docs/superpowers/specs/2026-07-20-go-like-service-call-closure-design.md` 中对自动注册、内部 Server
  路由与 streaming 的排除；
- `docs/capability-comparison.md` 中“go-like 不提供 Store”的旧结论。
...
- `@go-like/web` 面向外部 HTTP 应用和框架，公共 ABI 是标准 `Request -> Response`；
- `@go-like/transport` 定义内部服务通信的公共 SPI；
- `@go-like/transport-http` 使用 HTTP/Fetch 实现内部 Transport client 与 server；
- `@go-like/server` 只提供内部服务声明、分派和自动注册组合，不成为外部 Web 框架；
- Hono、Elysia、H3 等框架继续由各自的 `@go-like/*` 包适配 `@go-like/web` 生命周期。
...
`@go-like/struct` 变为 29。这是实施前既有计数漂移，不是本设计引入的回归。

### 3.2 已经成立的能力

下列能力已有足够实现，后续只做明确的小幅增强或组合：

- `@go-like/context`：Go `context` 等价的取消、deadline、cause、value 与派生语义；
- `@go-like/core`：顺序启动、失败回滚、逆序 drain、被动退出、hard deadline、orphan diagnostics；
- `@go-like/web`：标准 Fetch handler、Context 适配、Node host、Hono/Elysia/H3 适配；
- unary `@go-like/transport` 与 `@go-like/transport-http`；
- `@go-like/client` 的单次 discover-select-dial-send-recv 调用；
- `@go-like/config` 的 immutable snapshot、ordered merge、schema、watch、last-good；
- `@go-like/registry` 的完整 SPI、canonical identity、discovery、round robin 与 conformance；
...
- `@go-like/struct` 的 Go 风格 schema 与 object-level Web codec。
...
另一个跨 runtime 实验确认：直接在 request factory 新建的 Request 上设置 go-like header 后，Node、Bun、Deno
...
packages/
  ...
  transport/http/               @go-like/transport-http
...
`@go-like/config/yaml` 子路径，不单独形成 workspace。
...
这里的 `lifecycleServer` 指从 `@go-like/core` 导入并重命名的 `server`。两个 registered 入口都返回
...
显式传入可被对应 `ServiceInstanceResolver` 理解的 advertise override。go-like 不猜 transport scheme、
NAT、Ingress、容器网卡或端口映射。

注册失败必须回滚已绑定 server。撤注册失败不能跳过 server drain。多个失败保持观察顺序。

用户可以传入任意 `AddressServer` factory；go-like 不要求使用 `@go-like/transport-http`，也不要求继承。
...
重复 endpoint、空 identity、非法 UTF-16、不完整 handler，以及 metadata 中以 `go-like.` 开头但不属于
`go-like.endpoint.kind` 或 `go-like.codec.media-type` 的 key，在任何 I/O 前拒绝。Registry 发布的 endpoint
metadata 固定增加 `go-like.endpoint.kind = unary|fetch`；业务 metadata 不得覆盖该 key。
...
`@go-like/transport` 增加 provider-neutral `ServiceError` 与 wire helper：
...
- 新增 `Go-Like-Service-Error: v1`、`Go-Like-Service-Error-Code` 与
  `Go-Like-Service-Error-Status`；既有 `Go-Like-Error` 保持 Transport protocol diagnostic，不复用；
- JSON body 固定为
...
- `@go-like/client` 识别 wire error 并抛出不可变 `ServiceError`；
...
`@go-like/client` 保留现有 unary API，并增加 functional options：
...
幂等性负责。推荐 retry 继续由 `@go-like/resilience` 在调用外显式组合。
...
typed helper 接受 `@go-like/struct` 的 `Codec<T>`，只负责 body encode/decode，不注册全局 codec。

## 7. 标准 Fetch streaming

### 7.1 Transport SPI

`@go-like/transport` 增加独立接口，不修改 unary `Message.body: Uint8Array`：
...
`@go-like/transport-http` 实现：

- `newHTTPFetchTransport(...)`；
- `newHTTPFetchServer(httpHost, handler, ...options)`；
- `@go-like/transport-http/node#newNodeHTTPFetchServer(...)`。
...
`TransportFetchHandler` 是 Context-first 的内部 Transport ABI，不是 `@go-like/web` 的单参数标准 handler。
...
`@go-like/struct` 在现有 object-level `encodeJson/decodeJson` 上增加唯一的 bytes codec：
...
公共 `@go-like/store/testing` 提供 provider-neutral conformance。
...
`@go-like/store-file` 根入口接受结构式 filesystem host；`./node` 提供 Node host。
...
`@go-like/store-consul` 使用注入的标准 Fetch 调用 Consul KV/Session HTTP API：
...
`@go-like/store-etcd` 使用 etcd v3 JSON gRPC gateway：
...
`@go-like/config-etcd` 实现现有 `ConfigSource`：
...
`@go-like/registry-etcd` 使用 JSON gateway：
...
`@go-like/registry-kubernetes` 使用标准 Fetch 调用
`discovery.k8s.io/v1 EndpointSlice`：

- 每个 go-like Node 对应一个受管 EndpointSlice；
- `kubernetes.io/service-name` 和 go-like managed labels 用 canonical hash；
...
`@go-like/registry-zookeeper` 使用纯 JavaScript `node-zookeeper-client@1.1.3`，明确支持 Node/Bun 后端入口：
...
`@go-like/registry` 保留现有 `newRoundRobinSelector()`，增加：
...
`@go-like/nats/broker` 与 `@go-like/nats/jetstream/broker` 实现该 SPI；现有 lifecycle adapter 保持兼容。
...
Broker connection；它与 `@go-like/broker` 的 `subscription(...)` 具有相同 admission、rollback、drain
与 stable `done()` 契约。
...
应用继续显式注册 NATS、Redis、Registry、Store 等 dependency probe。go-like 不为无法可靠观测健康状态的
第三方 SDK 伪造 probe。

## 15. OpenTelemetry

现有 provider 生命周期继续由应用配置和拥有。`@go-like/otel` 增加显式 wrapper，而不是自动插桩：
...
1. 修复工作区基线计数，补 `@go-like/struct` bytes codec；
2. 补 Core App identity/hooks 与 Health readiness；
3. 补 `@go-like/server`、ServiceError、Client middleware、自动注册；
4. 补 Fetch Transport、Stream Client 与真实 streaming；
5. 补 Selector；
6. 建 `@go-like/store` 与 file provider；
