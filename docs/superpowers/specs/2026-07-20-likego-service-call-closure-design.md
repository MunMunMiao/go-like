...
LikeGo 已经具备独立的 Core 生命周期、Registry/Discovery/Selector、Transport/HTTP client+server、Web、
...
`@likego/client` 不依赖 `@likego/core`、`@likego/resilience`、`@likego/transport-http`、`@likego/web` 或任何
供应商包。应用 composition root 负责选择 Consul/mDNS、resolver 和具体 Transport。

现有包的依赖扩展分阶段处理：

- `@likego/otel` 只有在真实传播实验通过后，才依赖 `@likego/client` 与 `@likego/transport`；
- `@likego/prometheus` 只有在真实指标行为测试通过后，才依赖 `@likego/client` 与 `@likego/transport`；
- `@likego/pino`、`@likego/winston` 可以用各一个纯机械函数适配现有 `TransportLogger`，不定义 LikeGo 日志门面。

## 4. HTTP Server 动态地址

`@likego/transport-http` 扩展现有接口：
...
- client 独占大小写不敏感的 `Likego-Service` 与 `Likego-Endpoint`；调用方预置任一保留头时，在 discovery
  之前以 `TypeError` 拒绝；
- client 不写 `Likego-Method`、trace、ID、namespace、target 或 error header；
...
- 调用方若使用 `@likego/resilience.retry`，必须在每次 attempt 内重新调用 `Client.call`；
...
- `@likego/client` 自身不创建 timer 或默认 timeout，并把调用 Context 原样交给依赖；注入的 Transport 可以拥有
...
```text
recv Message
→ 读取 Likego-Service / Likego-Endpoint
→ 应用自行分派
→ send Message
```

LikeGo 不新增通用 RPC Router 或 handler registry。应用可以直接 switch、调用自己的 router，或自行实现任意
`Server` 并交给 Core。
...
1. 服务调用闭环：HTTP Server address、SelectionOutcome 收敛、联合 Docker E2E、`@likego/client`；
