...
在不扩大 LikeGo v1 能力边界的前提下，修复已由代码或真实运行复现的问题，补齐回归测试，并把运行时、Docker 依赖、CI、首发 Changesets 与文档收敛到可验证状态。

明确不加入：gRPC/proto/IDL、Event Store/历史查询/replay、更多 Registry provider、`@likego/struct`、统一 logger facade、自动发布流水线。
