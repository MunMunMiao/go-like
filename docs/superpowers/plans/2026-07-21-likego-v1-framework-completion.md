# LikeGo v1 微服务工具包完整落地实施计划

状态：历史计划。`@likego/struct`、Handle、ResidentClient、Fetch Transport、ServiceDeclaration 与自动注册
组合器已经在首发前删除；当前公共契约以 `docs/developer-experience-alignment.md` 为准。

> **执行要求：** 按任务使用 TDD 红—绿—重构；每个生产切片完成后分别做规范符合性与代码质量审查。

**目标：** 按
`docs/superpowers/specs/2026-07-21-likego-v1-framework-completion-design.md`
...
`@likego/core` 生命周期；endpoint 声明同时驱动分派和注册；外部 Web 与内部 Transport 严格分层。
...
- 直接在 `/Users/munmunmiao/Documents/web/likego` 的 dirty `main` 工作树实施。
...
- 每个任务开始前在 `/tmp/likego-v1-task-N.before/` 保存 scope 文件与 SHA-256；完成后保存
...
bun run --filter @likego/struct typecheck
```
...
bun run --filter @likego/struct typecheck
bun run --filter @likego/struct test:coverage
...
bun run --filter @likego/health typecheck
bun run --filter @likego/core test:coverage
bun run --filter @likego/health test:coverage
```
...
- [ ] 既有 `Likego-Error` 继续只表示 Transport protocol diagnostic。
...
- [ ] `@likego/server` 只组合 `server()`、`registration()`、`AddressServer` 和 declaration。
...
- [ ] 类型门禁要求 `TransportFetchHandler`、`FetchTransport`、`StreamClient`，不与 `@likego/web` 的
  单参数 Handler 混名。
...
- [ ] 使用现有 HTTPHost fixture 证明 direct Fetch server：
  ...
  - request abort 进入 LikeGo Context；
...
- [ ] 跑 `@likego/store/testing` 全 conformance。
...
- [ ] 公共 Registry 类型继续来自 `@likego/registry`。
...
- [ ] 最终 `docker ps -a`、network、volume 与项目进程均无 LikeGo 残留。
