...
**Goal:** 将 Zen Kit 已验证的标准 Web API struct 实现迁入独立的 `@go-like/struct@0.0.1`，并保留 Go struct 的零值、未知字段忽略、alias 和显式业务校验语义。

**Architecture:** `@go-like/struct` 是无生产依赖、非驻留、可移植的叶子包，不依赖 Core、Context、Transport 或 Zen Kit。第一阶段只迁移 struct 声明、运行时解析/编码及现有 codec，不接入尚未实现的 Contract/Server。
...
- 保留 MIT 许可证与来源说明。
- 包版本固定为 `0.0.1`，包名固定为 `@go-like/struct`。
...
**Interfaces:**
- Consumes: go-like package shell conventions。
...
Expected: 因 `@go-like/struct` 包与源码尚不存在而失败。
...
Expected: 全部退出 0，line/function coverage 满足 go-like 100% 契约。
