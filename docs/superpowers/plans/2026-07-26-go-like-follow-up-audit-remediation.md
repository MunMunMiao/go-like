...
Run: `bun run --filter @go-like/core test:coverage`
...
Run: `bun run --filter @go-like/web test:coverage`
...
在 `test/doc-site.test.ts` 复用 `discoverWorkspaces()`，取得 `private === false` 的 46 个 name；对八个 package reference 提取精确 `@go-like/<name>` token 并断言集合相等。

Run: `bun test test/doc-site.test.ts --test-name-pattern "package"`
Expected: FAIL，至少报告缺少 `@go-like/create`。
...
```json
"test:examples": "bun run --filter '@go-like/example-*' --parallel test"
```
...
按 `io.go-like.e2e.owner`、`com.go-like.published.owner` 过滤 container、network、volume；本轮 owner 资源必须为零，不清理 foreign owner。
