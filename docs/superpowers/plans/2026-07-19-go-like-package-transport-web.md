...
**目标：** 在不保留旧 adapter 包壳的前提下，把 go-like 收敛为 23 个发布包、4 个 private example workspace，
...
`@hono/node-server` 2.0.12、Hono 4.12.31、H3 2.0.1-rc.25、Elysia 1.4.29、Croner 10.0.1、
BullMQ 5.81.1、NATS JS 3.4.0、Pino 10.3.1、Winston 3.19.0、Prom Client 15.1.3、
OpenTelemetry 1.9.1/2.10.0、Docker 29.6.1。

## 全局约束

- 规范源是 `docs/superpowers/specs/2026-07-19-go-like-package-transport-web-design.md`；若计划与规范冲突，
  以规范为准并先修订计划。
...
- 大量现有实现是 untracked，`git diff` 不是恢复基线。Task 1 修改任何仓库文件前必须完成下面的全仓
  ...
  `/tmp/go-like-task-XX.{before,after}-tree/`，生成 SHA-256 与 `diff -ruN`。不得用 HEAD 覆盖这些文件。
...
- 所有 go-like 自有 HTTP/TXT header 使用 `Go-Like-` 前缀；不得出现 `Micro-` alias。
...
- [ ] 从仓库根使用 `rg --files --hidden --no-ignore .` 枚举全部 tracked/untracked reviewable 文件，包括
  ...
  `/tmp/go-like-preflight.before.files`。
- [ ] 严格按该清单把每个文件的完整相对路径和内容复制到 `/tmp/go-like-preflight.before-tree/`，保存
  `/tmp/go-like-preflight.before.sha256`，并用 fresh SHA-256 回读证明副本与工作树一致。只保存路径或 hash
  不算通过。
- [ ] 另存 `git status --short --untracked-files=all` 到 `/tmp/go-like-preflight.before.status`，保存完整
  `git diff --binary HEAD` 到 `/tmp/go-like-preflight.before.patch`。前者保护 untracked identity，后者保护
...
同时把 published runner 的 NATS exact-optional exception 从 `@go-like/nats-core-node`/
`@go-like/nats-jetstream-node` package-name 分支改为 case 声明的 `(export, directDependency)` policy；runner 必须反查
manifest direct dependency、TypeScript/SDK 版本、exit code 与逐行 diagnostic，未知 policy 继续 fail closed。测试使用
中性 fixture package name，并证明旧布局的两个 package 仍通过；这样 Task 13 的单一 `@go-like/nats` 根与
`./jetstream` 可以分别声明 policy，而 shared runner 不再携带旧 go-like identity。
...
负向扫描锁定不存在 `adapters/`、`@go-like/fetch`、`@go-like/fetch-node`、`@go-like/http` 和所有旧 adapter
包名。
...
`/tmp/go-like-layout.before.{files,sha256}`，包括 hidden `.omo/evidence`。严格按清单把完整文件内容复制到
`/tmp/go-like-layout.before-tree/`，不能只保存路径或 hash。

迁移后按最终 review scope 建立 `/tmp/go-like-layout.after-tree/` 与 after SHA-256，并保存
`diff -ruN /tmp/go-like-layout.before-tree /tmp/go-like-layout.after-tree` 为
`/tmp/go-like-layout.review.diff`。只有 hidden evidence、tracked/untracked 源码与根配置都可在该 diff 中审阅，
...
为该映射生成仓库外 `/tmp/go-like-nats-merge-map.before.tsv`，记录每个旧文件 SHA-256、最终落点及处置方式。
...
### Task 4：实现 `@go-like/transport` 公共契约与 conformance
...
同一步先为 `@go-like/testing/listener` 写 conformance harness 的失败测试，再创建 runner-neutral 实现；它只通过
...
### Task 5：实现 `@go-like/transport-http` portable unary client/server
...
standard Fetch/Node HTTP/go-like Transport 服务版本，以及 terminal、socket、port、process-tree cleanup。
...
Node host 只实现 HTTP transport envelope 与 socket ownership，不依赖 `@go-like/web`。Server 从 start 同一 turn
...
### Task 7：扩展 `@go-like/registry` 公共 Registry 与 conformance

> 本任务记录早期实现步骤，Registry 公共体验已由
> [`../specs/2026-07-19-go-like-package-transport-web-design.md`](../specs/2026-07-19-go-like-package-transport-web-design.md)
...
### Task 8：迁移并收敛 `@go-like/registry-consul`
...
### Task 9：实现 `@go-like/registry-mdns` portable codec/provider
...
host label、owner/target、DNS 63/255 byte 边界、RR shared/unique cache-flush 和全套 `Go-Like-*` TXT。
...
TTL/cache-flush/TTL0、`Go-Like-Wire-Version=1` 且不存在旧 namespace。
...
docker ps -a --filter label=go-like.suite=registry-mdns
docker network ls --filter label=go-like.suite=registry-mdns
```
...
### Task 11：完成 `@go-like/web`、health、Prometheus 与框架接缝
...
`@go-like/health` 不再导出 HTTP；`@go-like/web/health` 保留 GET/HEAD、200/503、404、405/Allow、no-store、
脱敏和取消；`@go-like/prometheus` 返回 Web `Handler`，不依赖 Node 子路径。
...
原 `fetch-node-native` suite 原子改名为 `web-node-native`：迁入的 native E2E 使用 `@go-like/web/node`、
`newNodeServer` 与 `GO_LIKE_WEB_NODE_E2E_RESULT`；同步更新 suite parser、proof contract 和既有 sourced case 的
...
把 `e2e/scripts/web-framework-native.ts` 改为从本任务已构建的最终 `@go-like/web`、`@go-like/hono`、
`@go-like/h3`、`@go-like/elysia` package export 加载，并在 helper 内创建最小 native app 后启动真实 listener；
...
逐项比对：`@go-like/config` 的 `.`/`./env`/`./file`、`@go-like/config-consul` borrowed Fetch 与 runtime matrix、
...
dependency 又允许消费方合法形成两份实现，使 Pino destination 与 go-like 私有 ABI 校验来自不同副本。因此
...
- 发布门禁必须打包真实 tarball，在不创建/继承 lockfile 的 npm 消费方中证明正常图只解析一份 SonicBoom
  `4.2.1`，且 Pino 与 go-like 使用同一原型；另建显式请求 `sonic-boom` `4.0.1` 的冲突消费方，要求安装以
...
逐项比对 `@go-like/nats` 根 `Subscription`/factory、`./jetstream` `ConsumerMessages`/factory、两条 published
...
- 可执行配置/源码不存在旧包名、`adapters/`、`@go-like/fetch`、`@go-like/http`、`Micro-`。
...
README 以 `@go-like/web` 对外、`@go-like/transport-http` 内部 client/server、`@go-like/registry` +
...
先按 Preflight 的同一 hidden-aware reviewable 规则建立 `/tmp/go-like-final.after-tree/`、排序 inventory 与
SHA-256，并生成从 `/tmp/go-like-preflight.before-tree/` 到 final tree 的累计 `diff -ruN`。然后把：
...
统一登记到仓库外 `/tmp/go-like-final-review-package/manifest.txt`。manifest 必须能定位每件证据并记录 SHA-256；
...
git diff --check
git status --short --untracked-files=all
! rg -n --hidden -g '!**/test/**' -g '!**/*.test.ts' -g '!**/fixtures/**' -g '!**/.omo/evidence/**' -g '!**/dist/**' -g '!**/node_modules/**' 'Micro-|@go-like/(fetch|fetch-node|http|config-env|config-file|cron-croner|job-bullmq-node|nats-core-node|nats-jetstream-node|log-pino-node|log-winston-node|metrics-prom-client-node|otel-node)|adapters/' package.json tsconfig*.json packages examples e2e scripts README.md docs/adr docs/capability-comparison.md
```
