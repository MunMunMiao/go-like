...
- 规范源是 `docs/superpowers/specs/2026-07-20-likego-service-call-closure-design.md`；若计划与规范冲突，以规范
  为准并先修订计划。
- 直接在 `/Users/munmunmiao/Documents/web/likego` 的 dirty `main` 工作树实施；不创建 worktree、feature
...
- 未经用户另行授权，不执行 `git add`、`git commit`、push、PR、publish、release 或 deploy。该约束替代
  ...
  测试日志。
- 每个任务首次编辑前把精确 scope 复制到 `/tmp/likego-service-call-task-N.before/`；完成后复制到对应
...
`/tmp/likego-service-call-task-1.*`，回读必须逐字节一致。
...
bun run --filter @likego/transport-http typecheck
```
...
保存到 `/tmp/likego-service-call-task-2.*`。同时 fresh 记录 Docker 的 `likego-*` container/network/volume
...
export const sourcedCase = newSourcedCase({
  ...
  runtimes: ["Bun 1.3.14"],
  services: ["Consul 2.0.2 Docker", "LikeGo HTTP Transport", "standard Fetch"],
  assertions: [
...
它只作为提取 `@likego/client` 前的真实 recipe，Task 4 必须删除。
...
### Task 3：收敛 SelectionOutcome 并实现 `@likego/client`
...
新文件以不存在记录；现有文件完整复制。保存到 `/tmp/likego-service-call-task-3.*`。
...
```sh
bun run --filter @likego/registry typecheck
```
...
```sh
bun run --filter @likego/registry typecheck
bun run --filter @likego/registry test:coverage
```
...
`Likego-Service/Likego-Endpoint` 合并、单次 get/select/dial/send/recv、`done(withoutCancel(ctx), outcome)`、
...
保存到 `/tmp/likego-service-call-task-4.*`。先运行现有 workspace/release/repository/published tests 记录基线，
不得把已有失败归因于本任务。

- [ ] **Step 2：把联合 suite 的局部 helper 替换为 `@likego/client`**
...
增加 `@likego/client` path/reference/identity/dependencies/capability/export；数量精确变为：
...
```sh
bun scripts/published/cli.ts --gate runtime --package @likego/client
bun scripts/published/cli.ts --gate types --package @likego/client
bun scripts/published/cli.ts --gate runtime --package @likego/transport-http
bun scripts/published/cli.ts --gate types --package @likego/transport-http
bun run test:e2e:prepared -- --suite registry-transport-consul-docker
```
...
验证没有新增 `likego-*` container/network/volume、没有监听端口或子进程残留；保存完整 `git status --short`，确认
无计划外文件被改动。
