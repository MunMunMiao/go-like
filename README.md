...
- `test:unit`：不依赖外部服务的日常确定性单元测试；`test:unit:coverage` 对该范围采集并强制校验覆盖率，但
  `examples/payments-ledger` 会额外运行需要 Docker 的 PostgreSQL/NATS 集成场景。
  `test:stability` 会把 root 与各 workspace 单元测试随机排序并重复两次，逐段打印可复现的 seed，不使用 retry，
  `test:parallel` 则用两个隔离 worker 执行一次相同范围，专门检查文件级并行安全；两者都不进入 canonical gate。
