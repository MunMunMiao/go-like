...
function CheckSpecifier(
  ...
  if (IsRelativeSpecifier(specifier)) {
    if (
      specifier.includes("\\") ||
      ForbiddenTypeScriptSpecifierExtensions.has(posix.extname(specifier))
    ) {
      issues.push(
        Issue(
          "MODULE_RELATIVE_EXTENSION_FORBIDDEN",
          Path,
          `relative TypeScript module specifier must omit its extension: ${specifier}`
        )
      )
      return
    }
    const target = resolve(dirname(sourceFile.fileName), `${specifier}.ts`)
  ...
  for (const sourceFile of sourceFiles) {
    ...
    function Visit(node: Node): void {
      if (isImportEqualsDeclaration(node)) {
        issues.push(
          Issue("MODULE_IMPORT_EQUALS_FORBIDDEN", Path, "import-equals declarations are forbidden")
        )
        return
      }
      if (isImportDeclaration(node)) {
        ...
        if (node.expression.kind === SyntaxKind.ImportKeyword) {
          ...
          if (isIdentifier(expression) && expression.text === "require") {
            issues.push(Issue("MODULE_REQUIRE_FORBIDDEN", Path, "require calls are forbidden"))
          } else if (
            ...
            issues.push(
              Issue("MODULE_MODULE_REQUIRE_FORBIDDEN", Path, "module.require calls are forbidden")
            )
          }
        }
      }
