...
import type { ManifestIssue } from "../manifests/validate.ts"
...
export function EvaluateFixtureCorpus(
  ...
  if (!IsSafeRelativePath(familyRoot)) {
    return InventoryFailure(0, 0, "fixture family root must be a safe non-empty relative path")
  }
  ...
  for (const file of snapshot.Files) {
    if (files.has(file.Path)) {
      return InventoryFailure(0, snapshot.Files.length, `duplicate snapshot path: ${file.Path}`)
    }
  ...
  if (casesFile === undefined) {
    return InventoryFailure(0, 0, `missing fixture case inventory: ${casesPath}`)
  }
  const document = ParseCases(casesFile)
  if (document === null) {
    return InventoryFailure(0, 0, "fixture case inventory must use the fixed common shape")
  }
  const expected = document.cases.length
  if (expected === 0) {
    return InventoryFailure(0, 0, "fixture case inventory must not be empty")
  }
  ...
  for (const fixtureCase of document.cases) {
    if (ids.has(fixtureCase.id) || paths.has(fixtureCase.path)) {
      return InventoryFailure(expected, 0, "fixture case ids and paths must be unique")
    }
  ...
  for (const file of snapshot.Files) {
    ...
    if (matches.length !== 1) {
      return InventoryFailure(expected, 0, `fixture payload must belong to exactly one listed case: ${file.Path}`)
    }
    payloadsByCase.get(matches[0]!.id)?.push(file)
  }
  for (const fixtureCase of document.cases) {
    if ((payloadsByCase.get(fixtureCase.id)?.length ?? 0) === 0) {
      return InventoryFailure(expected, 0, `listed fixture path has no snapshotted payload: ${fixtureCase.path}`)
    }
  }
  ...
  for (const fixtureCase of document.cases) {
    ...
    const expectedCodes = [...fixtureCase.expectedCodes].sort(CompareCodeUnits)
    const validation = SafelyValidate(validationFiles, validate)
    const matches = !validation.Fatal && ExactCodes(expectedCodes, validation.Codes)
    checks.push({
  ...
  return { SubjectsExpected: expected, SubjectsChecked: document.cases.length, Checks: checks }
}
