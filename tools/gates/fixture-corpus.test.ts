...
import type { InputSnapshot, SnapshotFile } from "./result"
...
async function LoadCorpus() {
  return import("./fixture-corpus")
}

async function LoadManifestValidator() {
  return import("../manifests/validate")
}
...
describe("committed fixture assets", () => {
  test("loads the common case list and accounts for every committed manifest payload", async () => {
    ...
    const payloads = (await FilesBelow(join(RepositoryRoot, FamilyRoot))).filter(
  ...
  test("runs the application-owned ServerHandle lifecycle through its Context-first methods", async () => {
    const module = await import(
      pathToFileURL(join(RepositoryRoot, StructuralServerRoot, "server.ts")).href
    )
    const context = {
...
describe("evaluateFixtureCorpus", () => {
  test("fails closed for an empty case list or empty family root", async () => {
    ...
    const emptyList = evaluateFixtureCorpus(
      Snapshot({ [CasesPath]: Cases([]) }),
      FamilyRoot,
      validate
    )
  ...
  test("compares sorted negative codes as an exact multiset and preserves duplicates", async () => {
    ...
    const exact = evaluateFixtureCorpus(snapshot, FamilyRoot, () => [
      issue("A_CODE"),
      issue("Z_CODE"),
      issue("A_CODE")
    ])
    const wrongCode = evaluateFixtureCorpus(snapshot, FamilyRoot, () => [
      issue("A_CODE"),
      issue("B_CODE"),
      issue("A_CODE")
    ])
    const wrongCount = evaluateFixtureCorpus(snapshot, FamilyRoot, () => [
      issue("A_CODE"),
      issue("Z_CODE")
    ])
    ...
    expect(exact.Checks).toEqual([
      expect.objectContaining({ id: "FIXTURE_CASE_MATCH", status: "pass" })
    ])
    ...
    const expectedThrowSentinel = evaluateFixtureCorpus(
      Snapshot({
  ...
  test("supports a listed file root and rejects shared-path collisions after rebasing", async () => {
    ...
    const fileRoot = evaluateFixtureCorpus(
      Snapshot({
    ...
    const collision = evaluateFixtureCorpus(
      Snapshot({
    ...
    expect(fileRoot.Checks).toEqual([
      expect.objectContaining({ id: "FIXTURE_CASE_MATCH", status: "pass" })
    ])
...
`
  ...
  test("keeps hostile thenables invalid while observing getter and call failures", async () => {
    ...
    const evaluations = [hostileGetter, hostileCall].map((value) =>
  ...
  test("traverses the exact committed positive and negative corpus from listed snapshot bytes only", async () => {
    ...
    const paths = [
      ...
      ...(await FilesBelow(join(RepositoryRoot, FamilyRoot)).then((items) =>
        items.map((path) => `${FamilyRoot}/${path}`)
      ))
    ]
    ...
    const expected = (
      JSON.parse(await readFile(join(RepositoryRoot, CasesPath), "utf8")) as { cases: unknown[] }
    ).cases.length
    ...
    expect(
      evaluation.Checks.every(
        (check) => check.id === "FIXTURE_CASE_MATCH" && check.status === "pass"
      )
    ).toBe(true)
  })
})
...
describe("evaluateAsyncFixtureCorpus", () => {
  test("awaits delayed validators strictly in listed case order", async () => {
    ...
    const events: string[] = []
    const evaluation = await evaluateAsyncFixtureCorpus(
      Snapshot({
        [CasesPath]: Cases([Case("first", "z-first"), Case("second", "a-second")]),
  ...
  test("continues after non-array and malformed results and cannot whitelist the invalid sentinel", async () => {
    ...
    ): Promise<readonly TestIssue[] | null | readonly unknown[]> => {
      const label = PayloadText(files)
      calls.push(label)
    ...
    const evaluation = await evaluateAsyncFixtureCorpus(
      ...
      FamilyRoot,
      validate as unknown as (files: readonly SnapshotFile[]) => Promise<readonly TestIssue[]>
    )
  ...
  test("classifies a throwing issue getter from a resolved async validator as invalid", async () => {
    ...
    const evaluation = await evaluateAsyncFixtureCorpus(
      ...
      FamilyRoot,
      async () => [malformed] as unknown as readonly TestIssue[]
    )
  ...
  test("keeps rebased collisions case-local and continues both sync and async evaluation", async () => {
    ...
    const snapshot = Snapshot({
      [CasesPath]: Cases([Case("collision", "z-collision"), Case("later", "a-later")]),
    ...
    const syncEvaluation = evaluateFixtureCorpus(
      snapshot,
      FamilyRoot,
      (files: readonly SnapshotFile[]) => {
    ...
    expect(
      syncEvaluation.Checks.map((check) => ({
        id: check.id,
        status: check.status,
        path: check.path
      }))
