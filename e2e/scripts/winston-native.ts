...
let joinedStops = false
let startPendingBeforeStop = false
let finalListenerDelta = Number.NaN
let nativeFinishObserved = false
const lifecycleOrder: string[] = []
let recordComponent: unknown = null
let recordFinal: unknown = null
let recordMessage: unknown = null
let recordLevel: unknown = null
...
try {
  ...
  terminalCompleted = nativeFinishObserved
  if (!terminalCompleted) throw new Error("Winston Logger did not reach its native finish terminal")

  const finalListeners = listenerCounts(logger)
  finalListenerDelta = listenerDelta(baseline, finalListeners)
  ...
  if (typeof record === "object" && record !== null) {
    recordComponent = "component" in record ? record.component : null
    recordFinal = "final" in record ? record.final : null
    recordMessage = "message" in record ? record.message : null
    recordLevel = "level" in record ? record.level : null
  }
  nativeLoggerRecord =
...
process.stdout.write(
  `LIKEGO_WINSTON_E2E_RESULT=${JSON.stringify({
    valid: true,
    runtime: `Node.js ${process.versions.node}`,
    winstonVersion: packageJson.version,
    ...
    scenarioEvidence: {
      "winston-native-file-lifecycle": {
        ...
        level: recordLevel,
        joinedStops,
        startPendingBeforeStop,
        nativeFinishObserved,
        fileLanded,
        lifecycleOrder: lifecycleOrder.join(">"),
        finishBeforeStopResolution:
          lifecycleOrder.indexOf("native-finish") < lifecycleOrder.indexOf("stop-resolved"),
        finishBeforeStartResolution:
          lifecycleOrder.indexOf("native-finish") < lifecycleOrder.indexOf("start-resolved"),
        finalRecordReadAfterFinish:
          lifecycleOrder.indexOf("native-finish") < lifecycleOrder.indexOf("file-read")
      }
    },
    assertions: {
      nativeLoggerRecord,
    ...
    cleanup: {
      terminalCompleted,
      listenerDelta: finalListenerDelta,
      directoryRemoved
    }
  })}\n`
)
