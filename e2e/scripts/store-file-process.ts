...
async function main(): Promise<void> {
  ...
  try {
    ...
    directoryRemoved = !(await exists(directory))
    console.log(
      `LIKEGO_STORE_FILE_PROCESS_E2E_RESULT=${JSON.stringify({
        valid: true,
        scenarios: ["file-store-process-crash-recovery", "file-store-checksum-fail-closed"],
        scenarioEvidence: {
          "file-store-process-crash-recovery": {
            childExitCode,
            staleLockRejected: true,
            staleTempObserved: true,
            lastSnapshotRecovered: true,
            staleTempRemoved: true
          },
          "file-store-checksum-fail-closed": {
            corruptionCode: "LIKEGO_FILE_STORE_CORRUPTION",
            validSnapshotRestored: true
          }
        },
        cleanup: {
          directoryRemoved,
