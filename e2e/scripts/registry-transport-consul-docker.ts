...
const Scenario = "consul-discovery-http-call-lifecycle"
...
let scenarioEvidence: Readonly<Record<string, unknown>> | null = null
...
try {
  ...
  scenarioEvidence = Object.freeze({
    bindBeforeRegister,
    deregisterBeforeStop,
    reachableDuringDeregister,
    deregisterProbeNodes: deregisterProbeNodes.join(","),
    lifecycleOrder,
    dynamicAddressesReady: endpointA !== endpointB,
    boundEndpointsMatchRegistry,
    rawRegistrations,
    discoveredNodes: discoveredNodes.join(","),
    watcherInitialNodes,
    roundRobinSequence,
    selectedNodes: selections.slice(0, 4).join(","),
    registrationsAfterFirstDeregister,
    watcherAfterDeregister,
    postDeregisterNode,
    registrationsAfterSecondDeregister,
    registryOperationsReturnedVoid,
    selectionFeedbackExactlyOnce,
    selectionFeedbackCalls: selectionTracker.feedbackCalls,
    feedbackOutcomesHealthy,
    transportClientsClosedExactlyOnce,
    transportBinds: transportTracker.binds,
    transportClientDials: transportTracker.dialedClients,
    transportClientCloseCalls: transportTracker.closeCalls,
    consulFetchCalls,
    transportFetchCalls,
    unaryCallsA: nodeState.calls.a,
    unaryCallsB: nodeState.calls.b
  })
} catch (value) {
...
ensure(scenarioEvidence !== null, "joint Docker suite omitted scenario evidence")
ensure(
...
console.log(
  `LIKEGO_REGISTRY_TRANSPORT_CONSUL_E2E_RESULT=${JSON.stringify({
    schemaVersion: 1,
    valid: true,
