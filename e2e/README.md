...
Web、Hono、H3 与 Elysia 的 package-owned built-dist synthetic bridge suites 已进入默认 suites/all；旧 framework compatibility IDs 保持显式可选，但不再进入默认计划。Framework dist 与 vendor dependency staging 使用 invocation-scoped secure temp，不写入仓库内 `e2e/node_modules`。
