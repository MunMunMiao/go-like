...
#include "likego_e2e_posix_protocol.h"
...
#else
#error "likego_e2e_posix_controller supports only macOS and Linux"
#endif
...
#define LIKEGO_E2E_CONTROL_FD STDIN_FILENO
#define LIKEGO_E2E_RESPONSE_FD 3
#define LIKEGO_E2E_TARGET_STDOUT_FD 4
#define LIKEGO_E2E_TARGET_STDERR_FD 5
#define LIKEGO_E2E_INTERNAL_POLL_MS 25
#define LIKEGO_E2E_MAX_TRACKED_PROCESSES 65536u
#define LIKEGO_E2E_ERROR_MESSAGE_MAX 256u
#define LIKEGO_E2E_CGROUP_NAME_MAX 96u
#define LIKEGO_E2E_CGROUP_POPULATED_NA UINT32_MAX
...
struct cgroup_context {
  ...
  char invocation_name[LIKEGO_E2E_CGROUP_NAME_MAX];
  char command_name[LIKEGO_E2E_CGROUP_NAME_MAX];
  char expected_identity[LIKEGO_E2E_MAX_CGROUP_IDENTITY_BYTES];
...
struct controller {
  uint8_t nonce[LIKEGO_E2E_NONCE_SIZE];
  uint64_t last_request_id;
  uint64_t prepare_request_id;
  uint64_t start_request_id;
  enum likego_e2e_controller_state state;
  enum likego_e2e_process_mode mode;
  uint32_t platform;
  ...
  uint8_t input[LIKEGO_E2E_MAX_WIRE_FRAME];
  size_t input_length;
...
static int byte_buffer_reserve(struct byte_buffer *buffer, size_t additional) {
  if (additional > LIKEGO_E2E_MAX_FRAME_BODY ||
      buffer->length > LIKEGO_E2E_MAX_FRAME_BODY - additional) {
    errno = EOVERFLOW;
    return -1;
  }
  ...
  while (capacity < required) {
    if (capacity > LIKEGO_E2E_MAX_FRAME_BODY / 2u) {
      capacity = LIKEGO_E2E_MAX_FRAME_BODY;
      break;
    }
    capacity *= 2u;
  }
...
static int parse_prepare_payload(const uint8_t *payload, size_t payload_length,
  ...
  if ((raw_mode != LIKEGO_E2E_MODE_ANCHORED_MANAGED &&
       raw_mode != LIKEGO_E2E_MODE_LINUX_CGROUP_V2_STRICT) ||
      argc == 0u || argc > LIKEGO_E2E_MAX_ARGC || envc > LIKEGO_E2E_MAX_ENVC ||
      cwd_length == 0u || cwd_length > LIKEGO_E2E_MAX_CWD_BYTES ||
  ...
  for (size_t index = 0u; index < parsed.argc; index += 1u) {
    ...
    if (!reader_u32(&reader, &length) || length > LIKEGO_E2E_MAX_STRING_BYTES ||
  ...
  for (size_t index = 0u; index < parsed.envc; index += 1u) {
    ...
    if (!reader_u32(&reader, &key_length) || !reader_u32(&reader, &value_length) ||
        key_length > LIKEGO_E2E_MAX_STRING_BYTES ||
        value_length > LIKEGO_E2E_MAX_STRING_BYTES ||
  ...
  *mode = (enum likego_e2e_process_mode)raw_mode;
  *target = parsed;
  return 0;
...
static int send_frame(const struct controller *controller, uint16_t type, uint32_t flags,
  ...
  if (payload_length > LIKEGO_E2E_MAX_FRAME_BODY - LIKEGO_E2E_FRAME_HEADER_SIZE) {
    errno = EOVERFLOW;
    return -1;
  }
  uint8_t prefix[LIKEGO_E2E_FRAME_PREFIX_SIZE];
  uint8_t header[LIKEGO_E2E_FRAME_HEADER_SIZE];
  uint32_t body_length = (uint32_t)(LIKEGO_E2E_FRAME_HEADER_SIZE + payload_length);
  store_u32_be(prefix, body_length);
  memset(header, 0, sizeof(header));
  store_u32_be(header + LIKEGO_E2E_HEADER_MAGIC_OFFSET, LIKEGO_E2E_FRAME_MAGIC);
  store_u16_be(header + LIKEGO_E2E_HEADER_VERSION_OFFSET, LIKEGO_E2E_PROTOCOL_VERSION);
  store_u16_be(header + LIKEGO_E2E_HEADER_TYPE_OFFSET, type);
  store_u32_be(header + LIKEGO_E2E_HEADER_FLAGS_OFFSET, flags);
  store_u64_be(header + LIKEGO_E2E_HEADER_REQUEST_ID_OFFSET, request_id);
  memcpy(header + LIKEGO_E2E_HEADER_NONCE_OFFSET, controller->nonce,
         LIKEGO_E2E_NONCE_SIZE);
  if (write_all(LIKEGO_E2E_RESPONSE_FD, prefix, sizeof(prefix)) < 0 ||
      write_all(LIKEGO_E2E_RESPONSE_FD, header, sizeof(header)) < 0 ||
      (payload_length > 0u &&
       write_all(LIKEGO_E2E_RESPONSE_FD, payload, payload_length) < 0)) {
    return -1;
  }
  return 0;
}
...
static int send_error(const struct controller *controller, uint64_t request_id,
                      enum likego_e2e_error_code error_code, int system_errno, bool terminal,
                      const char *message) {
  struct byte_buffer payload = {0};
  size_t message_length = strlen(message);
  if (message_length > LIKEGO_E2E_ERROR_MESSAGE_MAX) {
    message_length = LIKEGO_E2E_ERROR_MESSAGE_MAX;
  }
  int result = -1;
  ...
  result = send_frame(controller, LIKEGO_E2E_RESPONSE_ERROR,
                      terminal ? LIKEGO_E2E_RESPONSE_FLAG_TERMINAL : 0u, request_id,
...
static int random_nonce(uint8_t nonce[LIKEGO_E2E_NONCE_SIZE]) {
#if defined(__APPLE__)
  arc4random_buf(nonce, LIKEGO_E2E_NONCE_SIZE);
  return 0;
#elif defined(__linux__)
  size_t offset = 0u;
  while (offset < LIKEGO_E2E_NONCE_SIZE) {
    ssize_t count = getrandom(nonce + offset, LIKEGO_E2E_NONCE_SIZE - offset, 0u);
    if (count < 0) {
      if (errno == EINTR) {
        continue;
      }
...
static bool nonce_equal(const uint8_t *left, const uint8_t *right) {
  ...
  for (size_t index = 0u; index < LIKEGO_E2E_NONCE_SIZE; index += 1u) {
...
static int parse_request_body(const uint8_t *body, size_t body_length,
  ...
  if (body_length < LIKEGO_E2E_FRAME_HEADER_SIZE) {
    errno = EPROTO;
    return -1;
  }
  if (load_u32_be(body + LIKEGO_E2E_HEADER_MAGIC_OFFSET) != LIKEGO_E2E_FRAME_MAGIC) {
    errno = EPROTO;
    return -1;
  }
  if (load_u16_be(body + LIKEGO_E2E_HEADER_VERSION_OFFSET) !=
      LIKEGO_E2E_PROTOCOL_VERSION) {
    errno = EPROTONOSUPPORT;
    return -1;
  }
  frame->type = load_u16_be(body + LIKEGO_E2E_HEADER_TYPE_OFFSET);
  frame->flags = load_u32_be(body + LIKEGO_E2E_HEADER_FLAGS_OFFSET);
  frame->request_id = load_u64_be(body + LIKEGO_E2E_HEADER_REQUEST_ID_OFFSET);
  frame->nonce = body + LIKEGO_E2E_HEADER_NONCE_OFFSET;
  frame->payload = body + LIKEGO_E2E_HEADER_PAYLOAD_OFFSET;
  frame->payload_length = body_length - LIKEGO_E2E_FRAME_HEADER_SIZE;
  return 0;
}
...
static void child_exec_target(const struct target_spec *target, int exec_error_fd) {
  ...
  if (null_fd < 0 || dup2(null_fd, STDIN_FILENO) < 0 ||
      dup2(LIKEGO_E2E_TARGET_STDOUT_FD, STDOUT_FILENO) < 0 ||
      dup2(LIKEGO_E2E_TARGET_STDERR_FD, STDERR_FILENO) < 0 ||
...
static void anchor_process(const struct target_spec *target,
                           enum likego_e2e_process_mode mode, int control_fd, int event_fd,
                           int cgroup_procs_fd) {
  close(LIKEGO_E2E_CONTROL_FD);
  close(LIKEGO_E2E_RESPONSE_FD);
...
#if defined(__linux__)
  if (mode == LIKEGO_E2E_MODE_LINUX_CGROUP_V2_STRICT) {
    char pid_text[32];
  ...
  while (true) {
    ...
    poll_fd.revents = 0;
    int polled = poll(&poll_fd, 1u, LIKEGO_E2E_INTERNAL_POLL_MS);
    if (polled < 0) {
      if (errno == EINTR) {
        continue;
      }
    ...
    if ((poll_fd.revents & POLLIN) != 0) {
      ...
      close(exec_pipe[1]);
      close(LIKEGO_E2E_TARGET_STDOUT_FD);
      close(LIKEGO_E2E_TARGET_STDERR_FD);
      int exec_errno = 0;
      ...
      if (exec_result == 0) {
        ...
        (void)anchor_send_event(event_fd, ANCHOR_EVENT_TARGET_EXIT, reported_errno,
                                (uint32_t)target_pid,
                                LIKEGO_E2E_EXIT_EXEC_FAILURE & 0xffffu);
        target_done = true;
      }
    }
...
static int tracker_add(struct controller *controller, pid_t pid, uint64_t start_identity) {
  ...
  if (controller->tracked_count >= LIKEGO_E2E_MAX_TRACKED_PROCESSES) {
    errno = EOVERFLOW;
    return -1;
  }
  if (controller->tracked_count == controller->tracked_capacity) {
    ...
    if (capacity > LIKEGO_E2E_MAX_TRACKED_PROCESSES) {
      capacity = LIKEGO_E2E_MAX_TRACKED_PROCESSES;
    }
...
static int take_process_snapshot(struct process_snapshot *snapshot) {
  ...
  while ((entry = readdir(directory)) != NULL) {
    ...
    if (count == capacity) {
      ...
      if (next_capacity > LIKEGO_E2E_MAX_TRACKED_PROCESSES) {
        next_capacity = LIKEGO_E2E_MAX_TRACKED_PROCESSES;
      }
...
static int wait_cgroup_empty(int events_fd, int64_t deadline_ms) {
  while (true) {
    ...
    if (timeout > LIKEGO_E2E_INTERNAL_POLL_MS) {
      timeout = LIKEGO_E2E_INTERNAL_POLL_MS;
    }
    struct pollfd poll_fd;
...
static int cgroup_probe(struct cgroup_context *context,
  ...
  char probe_name[LIKEGO_E2E_CGROUP_NAME_MAX];
  ...
  char expected[LIKEGO_E2E_MAX_CGROUP_IDENTITY_BYTES];
  char actual[LIKEGO_E2E_MAX_CGROUP_IDENTITY_BYTES];
  ...
  if (wait_cgroup_empty(events_fd, monotonic_ms() + LIKEGO_E2E_HARD_BUDGET_MS) < 0) {
    saved_errno = errno;
    goto probe_cleanup;
  }
  result = 0;
...
probe_cleanup:
  ...
  int wait_status = 0;
  int64_t reap_deadline = monotonic_ms() + LIKEGO_E2E_HARD_BUDGET_MS;
...
static int cgroup_setup(struct controller *controller) {
  ...
  int invocation_length =
      snprintf(context->invocation_name, sizeof(context->invocation_name),
               "likego-%s-%ld", nonce_hex, (long)getpid());
...
static uint32_t test_skip_kill_rounds(const struct controller *controller) {
  const char *value = target_environment_value(&controller->target,
                                                "LIKEGO_E2E_TEST_SKIP_KILL_ROUNDS");
  if (value == NULL) {
    return 0u;
  }
  if (strcmp(value, "1") == 0) {
...
static int test_signal_barrier(const struct controller *controller, const char *stage,
  ...
  const char *directory = target_environment_value(
      &controller->target, "LIKEGO_E2E_TEST_SIGNAL_BARRIER_DIR");
  ...
  while (access(release_path, F_OK) < 0) {
    ...
    sleep_until_or_for(deadline_ms, LIKEGO_E2E_INTERNAL_POLL_MS);
  }
  return 0;
}
#else
...
static int send_target_exit_event(struct controller *controller) {
  ...
  result = send_frame(controller, LIKEGO_E2E_RESPONSE_TARGET_EXIT,
                      LIKEGO_E2E_RESPONSE_FLAG_EVENT, controller->start_request_id,
                      payload.data, payload.length);
done:
  byte_buffer_free(&payload);
  return result;
...
static int handle_anchor_event(struct controller *controller,
  ...
  if (event->kind == ANCHOR_EVENT_READY) {
...
#if defined(__linux__)
    if (controller->mode == LIKEGO_E2E_MODE_LINUX_CGROUP_V2_STRICT &&
    ...
    result = send_frame(controller, LIKEGO_E2E_RESPONSE_ANCHOR_READY, 0u,
    ...
    controller->state = LIKEGO_E2E_STATE_ANCHOR_READY;
    return 0;
  }
  if (event->kind == ANCHOR_EVENT_TARGET_STARTED) {
    ...
    result = send_frame(controller, LIKEGO_E2E_RESPONSE_TARGET_STARTED, 0u,
    ...
    controller->state = LIKEGO_E2E_STATE_RUNNING;
  ...
  if (event->kind == ANCHOR_EVENT_TARGET_EXIT) {
    ...
    controller->start_pending = false;
    controller->state = LIKEGO_E2E_STATE_TARGET_EXITED;
    (void)refresh_tracker(controller);
...
static int start_anchor(struct controller *controller) {
  ...
  if (anchor_pid == 0) {
    ...
    close(event_pipe[0]);
#if defined(__linux__)
    int cgroup_procs_fd = controller->mode == LIKEGO_E2E_MODE_LINUX_CGROUP_V2_STRICT
...
static int finalize_anchored(struct controller *controller, bool include_term_phase,
                             struct observation *observation,
                             enum likego_e2e_cleanup_result *cleanup_result,
                             const char **detail) {
  int64_t started = monotonic_ms();
  if (started < 0) {
    return -1;
  }
  int64_t deadline = started + LIKEGO_E2E_HARD_BUDGET_MS;
  memset(observation, 0, sizeof(*observation));
  *cleanup_result = LIKEGO_E2E_CLEANUP_INCONCLUSIVE;
  *detail = "anchored cleanup did not establish a final observation";
  ...
  if (cleanup_required && include_term_phase) {
    ...
    sleep_until_or_for(deadline, LIKEGO_E2E_TERM_GRACE_MS);
    if (drain_anchor_events(controller) < 0 ||
        observe_managed_processes(controller, observation) < 0) {
  ...
  while (observation->same_group_live > 0u &&
         controller->kill_rounds < LIKEGO_E2E_MACOS_MAX_KILL_ROUNDS &&
    ...
    sleep_until_or_for(deadline, LIKEGO_E2E_KILL_ROUND_WAIT_MS);
    if (drain_anchor_events(controller) < 0 ||
        observe_managed_processes(controller, observation) < 0) {
  ...
  if (observation->same_group_live > 0u || observation->breakaway_live > 0u) {
    *cleanup_result = LIKEGO_E2E_CLEANUP_RESIDUAL_PRESENT;
  ...
  *cleanup_result = LIKEGO_E2E_CLEANUP_ZERO_OBSERVED;
...
static int finalize_strict_cgroup(struct controller *controller,
  ...
  int64_t deadline = started + LIKEGO_E2E_HARD_BUDGET_MS;
  *cleanup_result = LIKEGO_E2E_CLEANUP_PLATFORM_FAILURE;
  *cgroup_populated = 1u;
  *detail = "strict cgroup cleanup failed";
  ...
  while (true) {
    ...
    sleep_until_or_for(deadline, LIKEGO_E2E_INTERNAL_POLL_MS);
  }
  reap_linux_managed_children(controller);
  if (controller->anchor_pid > 0) {
    int wait_status = 0;
...
static int send_finalized(struct controller *controller, uint64_t request_id,
  ...
  if (detail_length > LIKEGO_E2E_ERROR_MESSAGE_MAX) {
    detail_length = LIKEGO_E2E_ERROR_MESSAGE_MAX;
  }
  int result = -1;
  ...
  result = send_frame(controller, LIKEGO_E2E_RESPONSE_FINALIZED, 0u, request_id,
                      payload.data, payload.length);
done:
  byte_buffer_free(&payload);
  return result;
...
static int finalize_controller(struct controller *controller, uint64_t request_id,
  ...
  struct observation observation;
  memset(&observation, 0, sizeof(observation));
  enum likego_e2e_cleanup_result cleanup_result = LIKEGO_E2E_CLEANUP_INCONCLUSIVE;
  uint32_t cgroup_populated = LIKEGO_E2E_CGROUP_POPULATED_NA;
  ...
  controller->state = result == 0 ? LIKEGO_E2E_STATE_FINALIZED : LIKEGO_E2E_STATE_FAILED;
  if (send_response) {
    if (result < 0) {
      (void)send_error(controller, request_id,
#if defined(__linux__)
                       controller->mode == LIKEGO_E2E_MODE_LINUX_CGROUP_V2_STRICT
                           ? LIKEGO_E2E_ERROR_CGROUP_OPERATION
                           : LIKEGO_E2E_ERROR_OBSERVATION,
#else
                       LIKEGO_E2E_ERROR_OBSERVATION,
#endif
...
static int send_query(struct controller *controller, uint64_t request_id) {
  ...
  uint32_t cgroup_populated = LIKEGO_E2E_CGROUP_POPULATED_NA;
  if (controller->anchor_pid > 0 &&
      controller->mode == LIKEGO_E2E_MODE_ANCHORED_MANAGED &&
      observe_managed_processes(controller, &observation) < 0) {
    return send_error(controller, request_id, LIKEGO_E2E_ERROR_OBSERVATION, errno, false,
  ...
  if (controller->mode == LIKEGO_E2E_MODE_LINUX_CGROUP_V2_STRICT &&
    ...
    return send_error(controller, request_id, LIKEGO_E2E_ERROR_CGROUP_OPERATION, errno,
  ...
  result = send_frame(controller, LIKEGO_E2E_RESPONSE_QUERY, 0u, request_id,
                      payload.data, payload.length);
done:
  byte_buffer_free(&payload);
  return result;
...
static int handle_prepare_request(struct controller *controller,
  ...
  if (controller->state != LIKEGO_E2E_STATE_READY || controller->prepare_pending ||
...
static int handle_start_request(struct controller *controller,
                                const struct request_frame *frame) {
  if (controller->state != LIKEGO_E2E_STATE_ANCHOR_READY || controller->start_pending ||
      frame->payload_length != 0u) {
    errno = EPROTO;
    return -1;
  }
...
static int handle_direct_signal_request(struct controller *controller,
                                        const struct request_frame *frame) {
  if (controller->state != LIKEGO_E2E_STATE_RUNNING || frame->payload_length != 4u) {
    errno = EPROTO;
    return -1;
  }
  ...
  if (controller->target_pidfd < 0) {
    return send_error(controller, frame->request_id, LIKEGO_E2E_ERROR_PIDFD_UNAVAILABLE,
                      ENOSYS, false,
  ...
  if (pidfd_send_signal_owned(controller->target_pidfd, (int)signal_number) < 0) {
    return send_error(controller, frame->request_id, LIKEGO_E2E_ERROR_DIRECT_SIGNAL,
                      errno, false, "pidfd_send_signal failed");
  }
  uint8_t payload[4];
  store_u32_be(payload, signal_number);
  return send_frame(controller, LIKEGO_E2E_RESPONSE_DIRECT_SIGNAL_SENT, 0u,
                    frame->request_id, payload, sizeof(payload));
#else
  return send_error(controller, frame->request_id, LIKEGO_E2E_ERROR_PLATFORM_UNSUPPORTED,
...
static bool known_request_type(uint16_t type) {
  switch (type) {
    case LIKEGO_E2E_REQUEST_PREPARE:
    case LIKEGO_E2E_REQUEST_START:
    case LIKEGO_E2E_REQUEST_FINALIZE:
    case LIKEGO_E2E_REQUEST_TERMINATE:
    case LIKEGO_E2E_REQUEST_QUERY:
    case LIKEGO_E2E_REQUEST_DIRECT_SIGNAL:
    case LIKEGO_E2E_REQUEST_CLOSE:
    case LIKEGO_E2E_REQUEST_HARD_TERMINATE:
      return true;
    default:
      return false;
  }
...
static int dispatch_request(struct controller *controller,
  ...
  switch (frame->type) {
    ...
    case LIKEGO_E2E_REQUEST_TERMINATE:
    case LIKEGO_E2E_REQUEST_HARD_TERMINATE:
      if ((controller->state != LIKEGO_E2E_STATE_ANCHOR_READY &&
           controller->state != LIKEGO_E2E_STATE_RUNNING &&
           controller->state != LIKEGO_E2E_STATE_TARGET_EXITED) ||
          frame->payload_length != 0u) {
        errno = EPROTO;
      ...
      return finalize_controller(controller, frame->request_id, true,
                                 frame->type != LIKEGO_E2E_REQUEST_HARD_TERMINATE);
    case LIKEGO_E2E_REQUEST_QUERY:
    ...
    case LIKEGO_E2E_REQUEST_CLOSE:
      if (frame->payload_length != 0u ||
          (controller->state != LIKEGO_E2E_STATE_READY &&
           controller->state != LIKEGO_E2E_STATE_FINALIZED)) {
        errno = EPROTO;
        return -1;
      }
      if (send_frame(controller, LIKEGO_E2E_RESPONSE_CLOSED, 0u, frame->request_id, NULL,
                     0u) < 0) {
        return -1;
      }
      controller->state = LIKEGO_E2E_STATE_CLOSED;
      return 0;
    default:
      errno = ENOTSUP;
      return -4;
...
static void emergency_cleanup(struct controller *controller) {
  if (controller->state == LIKEGO_E2E_STATE_ANCHOR_READY ||
      controller->state == LIKEGO_E2E_STATE_RUNNING ||
      controller->state == LIKEGO_E2E_STATE_TARGET_EXITED || controller->prepare_pending ||
...
static int process_one_wire_frame(struct controller *controller, const uint8_t *body,
  ...
  if (parse_request_body(body, body_length, &frame) < 0) {
    enum likego_e2e_error_code code =
        errno == EPROTONOSUPPORT ? LIKEGO_E2E_ERROR_VERSION : LIKEGO_E2E_ERROR_PROTOCOL;
  ...
  if (result == -2) {
    return terminal_protocol_error(controller, frame.request_id, LIKEGO_E2E_ERROR_NONCE,
                                   errno, "request nonce did not authenticate");
  }
  if (result == -3) {
    return terminal_protocol_error(controller, frame.request_id,
                                   LIKEGO_E2E_ERROR_REQUEST_ID, errno,
                                   "request ID was duplicate, stale, or out of order");
  }
  if (result == -4) {
    return terminal_protocol_error(controller, frame.request_id,
                                   LIKEGO_E2E_ERROR_UNKNOWN_FRAME, errno,
                                   "unknown request frame type");
  }
  enum likego_e2e_error_code code = LIKEGO_E2E_ERROR_INVALID_STATE;
  const char *message = "request was invalid for the current controller state";
#if defined(__linux__)
  char strict_message[192];
#endif
  if (frame.type == LIKEGO_E2E_REQUEST_PREPARE) {
#if defined(__linux__)
    if (controller->mode == LIKEGO_E2E_MODE_LINUX_CGROUP_V2_STRICT ||
        (frame.payload_length >= 4u &&
         load_u32_be(frame.payload) == LIKEGO_E2E_MODE_LINUX_CGROUP_V2_STRICT)) {
      int failure_errno = errno;
      code = LIKEGO_E2E_ERROR_CGROUP_PREREQUISITE;
      int message_length =
...
#endif
    {
      code = LIKEGO_E2E_ERROR_INVALID_PAYLOAD;
      message = "PREPARE payload or anchor setup was invalid";
    }
  } else if (frame.type == LIKEGO_E2E_REQUEST_START) {
    code = LIKEGO_E2E_ERROR_ANCHOR;
  ...
  *saw_eof = false;
  while (true) {
    if (controller->input_length == sizeof(controller->input)) {
      return terminal_protocol_error(controller, 0u, LIKEGO_E2E_ERROR_PROTOCOL,
                                     EOVERFLOW, "control input exceeded the bounded frame buffer");
    }
    ssize_t count = read(LIKEGO_E2E_CONTROL_FD,
    ...
    if (count > 0) {
      ...
      *saw_eof = true;
      ...
      return terminal_protocol_error(controller, 0u, LIKEGO_E2E_ERROR_PROTOCOL, errno,
                                     "control channel read failed");
    }
  }

  while (controller->input_length >= LIKEGO_E2E_FRAME_PREFIX_SIZE) {
    uint32_t body_length = load_u32_be(controller->input);
    if (body_length < LIKEGO_E2E_FRAME_HEADER_SIZE ||
        body_length > LIKEGO_E2E_MAX_FRAME_BODY) {
      return terminal_protocol_error(controller, 0u, LIKEGO_E2E_ERROR_PROTOCOL,
                                     EMSGSIZE, "frame length was outside protocol bounds");
    }
    size_t wire_length = LIKEGO_E2E_FRAME_PREFIX_SIZE + (size_t)body_length;
    if (controller->input_length < wire_length) {
      break;
    }
    if (process_one_wire_frame(controller,
                               controller->input + LIKEGO_E2E_FRAME_PREFIX_SIZE,
                               body_length) < 0) {
      return -1;
    }
    ...
    controller->input_length -= wire_length;
    if (controller->state == LIKEGO_E2E_STATE_CLOSED) {
      return 0;
    }
  }
  if (*saw_eof) {
    if (controller->input_length != 0u) {
      return terminal_protocol_error(controller, 0u, LIKEGO_E2E_ERROR_TRUNCATED_FRAME,
                                     EPROTO, "control channel ended inside a frame");
    }
    return terminal_protocol_error(controller, 0u, LIKEGO_E2E_ERROR_PROTOCOL, EPIPE,
...
static int send_controller_ready(struct controller *controller) {
  ...
  if (byte_buffer_append_u32(&payload, controller->platform) < 0 ||
      ...
      byte_buffer_append_u32(&payload, LIKEGO_E2E_MAX_FRAME_BODY) < 0 ||
      byte_buffer_append_u32(&payload, LIKEGO_E2E_HARD_BUDGET_MS) < 0 ||
      byte_buffer_append_u32(&payload, LIKEGO_E2E_MACOS_MAX_KILL_ROUNDS) < 0) {
    goto done;
  }
  result = send_frame(controller, LIKEGO_E2E_RESPONSE_CONTROLLER_READY,
                      LIKEGO_E2E_RESPONSE_FLAG_EVENT, 0u, payload.data, payload.length);
done:
  byte_buffer_free(&payload);
  return result;
}
...
static int controller_loop(struct controller *controller) {
  while (controller->state != LIKEGO_E2E_STATE_CLOSED &&
         controller->state != LIKEGO_E2E_STATE_FAILED) {
    if (shutdown_signal != 0) {
      int signal_number = shutdown_signal;
      emergency_cleanup(controller);
      (void)send_error(controller, 0u, LIKEGO_E2E_ERROR_INTERNAL, EINTR, true,
    ...
    nfds_t count = 0u;
    poll_fds[count].fd = LIKEGO_E2E_CONTROL_FD;
    ...
    if (result < 0) {
      ...
      return terminal_protocol_error(controller, 0u, LIKEGO_E2E_ERROR_INTERNAL,
    ...
    if (count > 1u && poll_fds[1].revents != 0) {
      if (drain_anchor_events(controller) < 0) {
        return terminal_protocol_error(controller, 0u, LIKEGO_E2E_ERROR_ANCHOR,
...
static int validate_controller_fds(void) {
  const int fds[] = {LIKEGO_E2E_CONTROL_FD, LIKEGO_E2E_RESPONSE_FD,
                     LIKEGO_E2E_TARGET_STDOUT_FD, LIKEGO_E2E_TARGET_STDERR_FD};
  for (size_t index = 0u; index < ARRAY_LENGTH(fds); index += 1u) {
    if (fcntl(fds[index], F_GETFD) < 0 || set_cloexec(fds[index]) < 0) {
      return -1;
    }
  }
  return set_nonblocking(LIKEGO_E2E_CONTROL_FD);
}
...
static void controller_initialize(struct controller *controller) {
  ...
  controller->target_pidfd = -1;
#if defined(__APPLE__)
  controller->platform = LIKEGO_E2E_PLATFORM_MACOS;
  controller->capabilities =
      LIKEGO_E2E_CAPABILITY_ANCHORED_GROUP | LIKEGO_E2E_CAPABILITY_MACOS_LIBPROC;
#elif defined(__linux__)
  controller->platform = LIKEGO_E2E_PLATFORM_LINUX;
  controller->capabilities = LIKEGO_E2E_CAPABILITY_ANCHORED_GROUP |
                             LIKEGO_E2E_CAPABILITY_LINUX_CGROUP_V2 |
                             LIKEGO_E2E_CAPABILITY_LINUX_PIDFD;
...
static int run_self_test(void) {
  ...
  struct target_spec parsed;
  memset(&parsed, 0, sizeof(parsed));
  enum likego_e2e_process_mode mode = 0;
  int result = 1;
  if (byte_buffer_append_u32(&prepare, LIKEGO_E2E_MODE_ANCHORED_MANAGED) < 0 ||
  ...
  if (parse_prepare_payload(prepare.data, prepare.length, &mode, &parsed) < 0 ||
      mode != LIKEGO_E2E_MODE_ANCHORED_MANAGED || parsed.argc != 2u ||
      parsed.envc != 1u || strcmp(parsed.cwd, "/") != 0 ||
      strcmp(parsed.argv[0], "/usr/bin/printf") != 0 ||
      strcmp(parsed.envp[0], "PATH=/usr/bin:/bin") != 0) {
    goto done;
  }
  uint8_t body[LIKEGO_E2E_FRAME_HEADER_SIZE];
  memset(body, 0, sizeof(body));
  store_u32_be(body + LIKEGO_E2E_HEADER_MAGIC_OFFSET, LIKEGO_E2E_FRAME_MAGIC);
  store_u16_be(body + LIKEGO_E2E_HEADER_VERSION_OFFSET, LIKEGO_E2E_PROTOCOL_VERSION);
  store_u16_be(body + LIKEGO_E2E_HEADER_TYPE_OFFSET, LIKEGO_E2E_REQUEST_QUERY);
  store_u64_be(body + LIKEGO_E2E_HEADER_REQUEST_ID_OFFSET, UINT64_C(42));
  struct request_frame frame;
  memset(&frame, 0, sizeof(frame));
  if (parse_request_body(body, sizeof(body), &frame) < 0 ||
      frame.type != LIKEGO_E2E_REQUEST_QUERY || frame.request_id != UINT64_C(42) ||
      frame.payload_length != 0u || load_u32_be(body) != LIKEGO_E2E_FRAME_MAGIC) {
    goto done;
  }
  result = 0;
done:
  ...
  if (result == 0) {
    puts("likego_e2e_posix_controller self-test: PASS");
  } else {
    fputs("likego_e2e_posix_controller self-test: FAIL\n", stderr);
  }
  return result;
}
...
static int run_cgroup_preflight(struct controller *controller) {
  ...
  puts("likego_e2e_posix_controller cgroup preflight: PASS");
  return 0;
#else
  (void)controller;
...
#if defined(__linux__)
  const char *environment_parent = getenv("LIKEGO_E2E_CGROUP_PARENT");
...
int main(int argc, char **argv) {
  ...
  if (parse_startup_arguments(&controller, argc, argv, &self_test, &cgroup_preflight) < 0) {
    fprintf(stderr, "likego_e2e_posix_controller: invalid startup arguments: %s\n",
            strerror(errno));
  ...
  if (prctl(PR_SET_CHILD_SUBREAPER, 1, 0, 0, 0) < 0) {
    fprintf(stderr, "likego_e2e_posix_controller: PR_SET_CHILD_SUBREAPER failed: %s\n",
            strerror(errno));
    controller_destroy(&controller);
    return 69;
  }
  ...
  if (validate_controller_fds() < 0 || install_controller_signal_handlers() < 0 ||
    ...
    fprintf(stderr, "likego_e2e_posix_controller: startup failed: %s\n", strerror(errno));
    controller_destroy(&controller);
    return 69;
  }
